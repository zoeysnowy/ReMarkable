...
export class ElectronMicrosoftAuth {
  ...
  async authenticateWithBrowser(): Promise<ElectronAuthResult> {
    return new Promise(async (resolve, reject) => {
      ...
      // 启动本地服务器监听回调并等待授权码
      try {
        const result = await this.startLocalServer();
        resolve(result);
      } catch (error) {
        reject(error);
      }
   ...
   * 启动本地服务器监听OAuth回调
   */
  private async startLocalServer(): Promise<ElectronAuthResult> {
    // 这里需要在主进程中实现HTTP服务器
    ...
    if (!electronAPI || typeof electronAPI.startAuthServer !== 'function') {
      throw new Error('Electron API不可用');
    }

    try {
      const authCode: string = await electronAPI.startAuthServer(this.redirectUri);
      if (!authCode) throw new Error('未收到授权码');
