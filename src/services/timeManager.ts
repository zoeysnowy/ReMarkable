...
export function getEventTime(eventId: string, fallback?: Partial<EventTime>): EventTime {
  ...
  if (snapshot.start || snapshot.end) {  // 🔧 改为：只要有任一时间就优先使用 TimeHub
  ...
  if (event?.startTime || event?.endTime) {  // 🔧 只要有任一时间就使用（接受 null）
    ...
    return {
      ...
      end: event.endTime ?? null,      // 🔧 允许 null
