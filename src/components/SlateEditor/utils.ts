...
const deserializeElement = (el: HTMLElement | ChildNode): any[] => {
  if (el.nodeType === 3) { // Node.TEXT_NODE
    return [{ text: el.textContent || '' }];
  }

  if (el.nodeType !== 1) { // Not an Element node
    return [];
  }

  const element = el as HTMLElement;
  const nodeName = element.nodeName.toLowerCase();
  let children: any[] = Array.from(element.childNodes).flatMap(node => 
    deserializeElement(node)
  );
  ...
  // 处理特殊节点
  if (element.getAttribute('data-type') === 'tag') {
    return [{
      type: 'tag',
      tagId: element.getAttribute('data-tag-id') || '',
      tagName: element.getAttribute('data-tag-name') || '',
      tagColor: element.getAttribute('data-tag-color') || '#666',
      tagEmoji: element.getAttribute('data-tag-emoji') || '',
      mentionOnly: element.getAttribute('data-mention-only') === 'true',
      children: [{ text: '' }],
    }];
  }

  if (element.getAttribute('data-type') === 'date-mention') {
    return [{
      type: 'dateMention',
