...
const PlanDemoLogger = logger.module('PlanDemo');
const PlanItemEditorDemo: React.FC = () => {
  ...
  useEffect(() => {
    const handleEventsChanged = (e: CustomEvent) => {
      PlanDemoLogger.log('📢 [Demo] 接收到事件变更通知:', e.detail);
      // 刷新事件列表
  ...
  const loadEventsFromStorage = () => {
    try {
      ...
      PlanDemoLogger.log('📖 [Demo] 已加载事�?', taskEvents.length);
    } catch (error) {
      PlanDemoLogger.error('�?[Demo] 加载事件失败:', error);
    }
  };
  ...
  const sampleItems: PlanItem[] = [
    {
      id: 'sample-1',
      title: '完成项目报告',
      content: '需要整理本季度的数据，制作PPT，准�?分钟演讲',
      tags: ['工作', '文档编辑'],
      color: '#000000',
  ...
  const handleSaveItem = (item: PlanItem) => {
    ...
    PlanDemoLogger.log('💾 [Demo] 保存计划�?', item);
    
    // 如果有时间，提示用户
    if (item.startTime) {
      PlanDemoLogger.log('�?[Demo] 计划项包含时间，已创�?UnifiedTimeline 事件');
    }
  };

  const handleEventCreated = (event: Event) => {
    PlanDemoLogger.log('🎉 [Demo] 事件已创建并同步�?UnifiedTimeline:', event);
  ...
  const handleCreateTag = (tagName: string) => {
    PlanDemoLogger.log('🏷�?创建新标�?', tagName);
  };
    ...
    <div className="plan-editor-demo">
      ...
      <div className="demo-header">
        ...
        <p className="demo-subtitle">
          参�?TagManager 设计，支持富文本编辑、标签、计时器等功�?
        </p>
      </div>
      ...
      <div className="demo-content">
        ...
        <section className="demo-section">
          <h2>🗓�?UnifiedTimeline 事件 ({createdEvents.length})</h2>
          <div className="timeline-info">
            <p>
              ℹ️ 当计划项包含明确的时间点时，会自动创�?UnifiedTimeline 事件�?
              可以�?TimeCalendar �?DesktopCalendarWidget 中查看�?
            </p>
          </div>
          {createdEvents.length === 0 ? (
            <div className="empty-state">
              <p className="empty-icon">🗓�?/p>
              <p className="empty-text">还没有创建带时间的事�?/p>
                ...
                <div key={event.id} className="event-card">
                  ...
                  <div className="event-sync-status">
                    <span className={`sync-badge sync-${event.syncStatus}`}>
                      {event.syncStatus === 'pending' && '�?待同�?}
                      {event.syncStatus === 'synced' && '�?已同�?}
  ...
  return (
    <div>
      ...
      <PlanItemEditor
        ...
        onCreateTag={(tag) => PlanDemoLogger.log('新标�?', tag)}
      />
    </div>
  );
}`}</code>
