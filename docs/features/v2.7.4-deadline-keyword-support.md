# v2.7.4: 截止时间关键词支持 + timeFieldState 优化

## 功能概述

1. **截止时间关键词支持**：识别"截止"、"ddl"、"deadline"等关键词，自动将时间设置为**结束时间**而非开始时间
2. **timeFieldState 优化**：从布尔标记改为存储实际时间值，确保重新打开 Picker 时精确恢复用户设置

## 需求来源

用户输入：
- "周五晚上10点截止" → 应设置 `endTime=22:00`（而非 `startTime=22:00`）
- "下周二中午12点ddl" → 应设置 `endTime=12:00`
- "最晚10点前完成" → 应设置 `endTime=22:00`

痛点：
- 重新打开 Picker 时，即使只设置了 `startTime`，`endTime` 也被错误地填充了相同的值
- 原因：旧的 `timeFieldState=[1,0,0,0]` 只记录了"是否设置"，重新加载时需要从 ISO 字符串解析，导致误判

## 核心改动

### 1. timeFieldState 重新设计

#### v2.5 旧设计（已废弃）
```typescript
timeFieldState: [number, number, number, number]  // [startTime?, endTime?, dueDate?, allDay?]
// 1 = 用户设置了，0 = 未设置
// 例如：[1, 0, 0, 0] 表示只设置了 startTime
```

**问题**：
- 重新加载时需要从 `start/end` ISO 字符串解析时间值
- 无法区分"单一开始时间（start=15:00, end=15:00）"和"时间段（start=13:00, end=18:00）"
- 导致 `endTime` 被错误地初始化

#### v2.7.4 新设计
```typescript
timeFieldState: [number | null, number | null, number | null, number | null]
// [startHour, startMinute, endHour, endMinute]
// null = 未设置，数字 = 实际值
// 例如：[15, 0, null, null] 表示只设置了 startTime=15:00
```

**优势**：
- ✅ 直接存储实际值，无需解析
- ✅ 精确恢复用户设置，`null` 明确表示未设置
- ✅ 降级兼容：如果没有 `timeFieldState`，回退到时间判断逻辑

### 2. 接口扩展

新增 `TimeType` 类型和 `timeType` 字段：

```typescript
/**
 * 时间类型：开始时间 vs 截止时间
 */
export type TimeType = 'start' | 'due' | 'none';

export interface TimePeriod {
  name: string;
  startHour: number;
  startMinute: number;
  endHour: number;
  endMinute: number;
  isFuzzyTime: boolean;
  timeType?: TimeType;  // 🆕 新增字段
}

export interface ParseResult {
  dateRange?: DateRange;
  timePeriod?: TimePeriod;
  pointInTime?: PointInTime;
  matched: boolean;
  timeType?: TimeType;  // 🆕 全局时间类型
}

// 🆕 v2.7.4: timeFieldState 改为存储实际值
export interface TimeGetResult {
  timeSpec?: TimeSpec;
  start?: string;
  end?: string;
  displayHint?: string | null;
  isFuzzyDate?: boolean;
  timeFieldState?: [number | null, number | null, number | null, number | null];  // [startHour, startMinute, endHour, endMinute]
  isFuzzyTime?: boolean;
  fuzzyTimeName?: string;
}
```

### 3. 截止关键词库

支持以下关键词（大小写不敏感）：

#### 中文核心词
- **截止**、**结束**、**终止**、**完成**
- **最晚**、**不晚于**

#### 场景词
- **ddl**、**deadline**、**due**
- **闭馆**、**散会**、**下班**

#### 英文词
- **before**、**by**、**until**
- **no later than**

#### 特殊模式
- **"X前"** 模式：如"10点前"、"22:00前"（通过正则 `/\d+[：:点]\s*前/` 检测）

### 3. 解析逻辑

在 `parseNaturalLanguage` 函数开头优先检测：

```typescript
// 🆕 v2.7.4: 检测截止关键词（优先级最高）
const deadlineKeywords = [
  '截止', '结束', '终止', '完成', '最晚', '不晚于', 
  'ddl', 'deadline', 'due', '闭馆', '散会', '下班',
  'before', 'by', 'until', 'no later than',
];
const hasDueKeyword = deadlineKeywords.some(kw => trimmedInput.includes(kw));
const hasBeforePattern = /\d+[：:点]\s*前/.test(trimmedInput);
const isDueTime = hasDueKeyword || hasBeforePattern;
```

返回结果时设置 `timeType`：

```typescript
return {
  matched: true,
  timePeriod: {
    // ... 其他字段
    timeType: isDueTime ? 'due' : 'start'
  },
  timeType: isDueTime ? 'due' : 'start'
};
```

### 4. 应用逻辑

在 `UnifiedDateTimePicker` 中根据 `timeType` 决定设置哪个时间字段：

```typescript
const timeType = customParsed.timePeriod.timeType || customParsed.timeType || 'start';

if (timeType === 'due') {
  // 截止时间：只设置结束时间
  setStartTime(null);
  setEndTime({
    hour: customParsed.timePeriod.startHour,
    minute: customParsed.timePeriod.startMinute
  });
  setFuzzyTimeName(null);
} else if (customParsed.timePeriod.isFuzzyTime) {
  // 模糊时间段：设置开始和结束时间
  setStartTime(...);
  setEndTime(...);
  setFuzzyTimeName(...);
} else {
  // 精确开始时间：只设置开始时间
  setStartTime(...);
  setEndTime(null);
  setFuzzyTimeName(null);
}
```

### 5. Picker 保存和读取逻辑

#### 保存时（handleApply）

```typescript
// 🆕 v2.7.4: timeFieldState 存储实际的时间值
const timeFieldState: [number | null, number | null, number | null, number | null] = [
  startTime?.hour ?? null,    // startHour
  startTime?.minute ?? null,  // startMinute
  endTime?.hour ?? null,      // endHour
  endTime?.minute ?? null     // endMinute
];

await TimeHub.setEventTime(eventId, {
  start: startIso,
  end: endIso,
  allDay: allDaySelected,
  displayHint: finalDisplayHint,
  timeFieldState,  // 保存实际值
  isFuzzyDate,
  isFuzzyTime,
  fuzzyTimeName
});
```

#### 读取时（useEffect 初始化）

```typescript
// 🆕 v2.7.4: 直接使用 timeFieldState 中存储的实际值
const savedFieldState = eventTime.timeFieldState;
if (savedFieldState) {
  const [startHour, startMinute, endHour, endMinute] = savedFieldState;
  setStartTime(startHour !== null && startMinute !== null 
    ? { hour: startHour, minute: startMinute } 
    : null);
  setEndTime(endHour !== null && endMinute !== null 
    ? { hour: endHour, minute: endMinute } 
    : null);
} else {
  // 降级：如果没有 timeFieldState，根据时间是否为 00:00 判断
  const hasSpecificStart = start.hour() !== 0 || start.minute() !== 0;
  const hasSpecificEnd = end ? (end.hour() !== 0 || end.minute() !== 0) : false;
  setStartTime(hasSpecificStart ? { hour: start.hour(), minute: start.minute() } : null);
  setEndTime(end && hasSpecificEnd ? { hour: end.hour(), minute: end.minute() } : null);
}
```

**关键优势**：
- ✅ 无需从 ISO 字符串解析时间
- ✅ `null` 明确表示未设置，避免误判
- ✅ 兼容旧数据（无 `timeFieldState` 时降级）

### 6. PlanManager 显示逻辑

### 6. PlanManager 显示逻辑

更新判断条件以适配新的 `timeFieldState` 格式：

```typescript
// 🆕 v2.7.4: timeFieldState 存储实际值，null 表示未设置
const hasStartTimeField = timeFieldState && timeFieldState[0] !== null && timeFieldState[1] !== null;
const hasEndTimeField = timeFieldState && timeFieldState[2] !== null && timeFieldState[3] !== null;
```

PlanManager 已支持截止时间显示（红色"截止"标签），当只有 `endTime` 时自动触发：

```tsx
{/* 单一结束时间：红色截止标签 */}
{hasEndTimeField && !hasStartTimeField && (
  <Tooltip title="截止时间">
    <span className="time-label end-label">截止</span>
  </Tooltip>
)}

{/* 单一开始时间：绿色开始标签 */}
{hasStartTimeField && !hasEndTimeField && (
  <Tooltip title="开始时间">
    <span className="time-label start-label">开始</span>
  </Tooltip>
)}
```

## 测试案例

| 输入示例 | 预期结果 | timeFieldState | 时间字段 | 显示 |
|---------|---------|---------------|---------|------|
| "周五晚上10点截止" | endTime=22:00 | `[null,null,22,0]` | endTime only | 🔴 截止 22:00 |
| "下周二中午12点ddl" | endTime=12:00 | `[null,null,12,0]` | endTime only | 🔴 截止 12:00 |
| "最晚明天10点前完成" | endTime=10:00 | `[null,null,10,0]` | endTime only | 🔴 截止 10:00 |
| "周五晚上10点开始" | startTime=22:00 | `[22,0,null,null]` | startTime only | 🟢 开始 22:00 |
| "下午3点" | startTime=15:00 | `[15,0,null,null]` | startTime only | 🟢 开始 15:00 |
| "下午" | startTime=13:00, endTime=18:00 | `[13,0,18,0]` | both | 13:00 → 18:00 |

**重新打开 Picker 验证**：
- "下午3点" → Picker 显示 startHour=15, startMinute=00, endHour=--, endMinute=-- ✅
- "下午3点截止" → Picker 显示 startHour=--, startMinute=--, endHour=15, endMinute=00 ✅

## 版本对比

### v2.5（旧版）
- ❌ `timeFieldState=[1,0,0,0]` 只记录"是否设置"
- ❌ 重新打开 Picker 时需要从 ISO 解析时间
- ❌ 无法区分单一开始时间和时间段（都是 start=15:00, end=15:00）
- ❌ 导致 `endTime` 被错误填充

### v2.7.3 之前
- ❌ "周五晚上10点截止" → startTime=22:00（错误）
- ❌ 无法区分开始时间和截止时间
- ❌ 缺少截止关键词检测

### v2.7.4 之后
- ✅ **timeFieldState 优化**：`[15,0,null,null]` 直接存储实际值
- ✅ **精确恢复**：重新打开 Picker 时，`null` 字段不会被填充
- ✅ **截止关键词**：支持 15+ 关键词（中英文）
- ✅ **智能设置**：
  - "后天下午3点" → `[15,0,null,null]` → 只设置 startTime
  - "后天下午3点截止" → `[null,null,15,0]` → 只设置 endTime
- ✅ **显示优化**：红色"截止"标签 + 绿色"开始"标签
- ✅ **降级兼容**：无 `timeFieldState` 时回退到时间判断

## 调试日志

启用 `dict` 和 `picker` 调试日志：

```
localStorage.setItem('DEBUG', 'dict,picker');
```

预期输出：
```
🔍 检测截止关键词 { isDueTime: true, hasDueKeyword: true, input: "周五晚上10点截止" }
⏰ 识别为截止时间（只设置结束时间） { timePeriod: "晚上10点", endTime: "22:0", keywords: "截止/ddl/deadline/due/最晚/不晚于" }
```

## 相关文件

- `src/utils/naturalLanguageTimeDictionary.ts` - 接口定义、截止关键词检测
- `src/components/FloatingToolbar/pickers/UnifiedDateTimePicker.tsx` - 时间字段设置逻辑
- `src/components/PlanManager.tsx` - 红色"截止"标签显示

## 技术债务

1. **"X前" 模式扩展**：当前仅支持 `/\d+[：:点]\s*前/`，未来可扩展为：
   - "周五前" → 周五全天截止
   - "本周前" → 本周日截止
   - "明年前" → 明年第一天截止

2. **模糊时间段+截止**：
   - "下午截止" 应设置 endTime=18:00（下午结束时间）
   - 当前逻辑仅处理精确时间+截止关键词

3. **多语言支持**：
   - 当前仅支持中英文
   - 未来可添加日文（締切）、韩文（마감）等

## 更新日志

- **2025-01-XX**: v2.7.4 初始发布
  - 新增 `TimeType` 类型和 `timeType` 字段
  - 支持 15+ 截止关键词
  - 根据 `timeType` 智能设置开始/结束时间
  - 完善调试日志
