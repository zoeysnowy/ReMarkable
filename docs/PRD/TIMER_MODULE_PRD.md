...
interface GlobalTimer {
  ...
  eventId?: string;            // Timer 事件的真实 ID（用于更新本地事件）
}
```

**存储位置**: 
- 内存: `useState<GlobalTimer | null>(null)`
- 持久化: `localStorage['remarkable-global-timer']`
...
```typescript
// 运行中
totalElapsed = elapsedTime + (Date.now() - startTime)
...
- ✅ **保留用户输入**: 用户可能在运行中编辑了 description，必须保留
...
sequenceDiagram
    ...
    Logic->>Logic: 清空 globalTimer
    Logic->>LS: 删除 timer 状态
    Logic->>UI: 切换到"时光"页面
```
...
**保存逻辑**: `App.tsx L613-736 handleTimerEditSave`
