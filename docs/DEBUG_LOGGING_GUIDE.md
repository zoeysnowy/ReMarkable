...
### 问题 2: 时间修改后 Event 被更新两次，第二次覆盖正确时间

**查看日志：**
```
[EventService] 更新字段: { startTime: '12:00', endTime: '18:00' }  ← ✅ 第一次正确
[EventService] 更新字段: { startTime: '10:00', endTime: '15:59' }  ← ❌ 第二次被旧值覆盖
```

**原因：** `handleLinesChange` 在每次编辑器内容变化时都调用 `syncToUnifiedTimeline`，用 item 的旧时间覆盖 TimeHub 刚写入的新时间。

**已修复：** 在 `handleLinesChange` 中检查 `item.eventId`：
- 如果有 `eventId`：跳过 `syncToUnifiedTimeline`（时间由 TimeHub 管理）
- 如果无 `eventId`：正常调用 `syncToUnifiedTimeline`

**新的日志流程：**
```
[picker] 💾 onTimeApplied: 保存 item (仅非时间字段)
[picker] ⏭️ handleLinesChange: 跳过 syncToUnifiedTimeline (item 有 eventId，时间由 TimeHub 管理)
[EventService] ✅ 更新成功 (只有一次更新，时间正确)
```

---

### 问题 3: 右侧时间显示为 12:00→12:00（应该是 12:00→18:00）
[picker] 🆕 创建新 Event (item 没有 eventId)
[EventService] ✅ 创建成功
[picker] ✅ TimeHub 写入成功，回写 eventId 到 item
```
