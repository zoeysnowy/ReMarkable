...
**问题描述**：运行中的Timer被意外同步，导致产生重复事件

**复现场景**：
1. 启动Timer → 创建event1 (syncStatus='local-only') ✅
2. 编辑Timer的description → 触发同步 ❌ **Bug：意外同步**
3. 停止Timer → 创建event2 (syncStatus='pending') ✅
4. 结果：远程出现2个事件（event1的早期版本 + event2的最终版本）

**根本原因**：
- `EventEditModal` 在编辑事件时，没有检查 `syncStatus='local-only'`
- 调用 `EventHub.updateFields` 时未传递 `skipSync: true`
- 导致运行中的Timer在编辑时触发了同步

**修复方案** (EventEditModal.tsx L350-380)：
```typescript
// 🎯 检查是否是运行中的Timer
...
// 更新时传递 skipSync 参数
await EventHub.setEventTime(event.id, {...}, { skipSync: shouldSkipSync });
await EventHub.updateFields(event.id, {...}, { skipSync: shouldSkipSync });
```
