...
(async function diagnoseAndFixCalendar() {
  ...
  try {
    if (window.syncManager?.microsoftService) {
      // 尝试多种方法获取日历
      if (typeof window.syncManager.microsoftService.fetchCalendars === 'function') {
        availableCalendars = await window.syncManager.microsoftService.fetchCalendars();
      } else if (typeof window.syncManager.microsoftService.getCalendars === 'function') {
        ...
        tags.forEach(tag => {
          if (tag.calendarMapping?.calendarId) {
            calendarMap.set(tag.calendarMapping.calendarId, {
              id: tag.calendarMapping.calendarId,
              name: tag.calendarMapping.calendarName,
              isDefaultCalendar: false
            });
          }
        });
        availableCalendars = Array.from(calendarMap.values());
        console.log('⚠️ 使用从标签映射提取的日历列表');
      }
    ...
    if (availableCalendars.length > 0) {
      console.log('');
      console.log(`✓ 从标签映射提取了 ${availableCalendars.length} 个日历:`);
      console.log('');
      availableCalendars.forEach((cal, index) => {
        console.log(`  ${index + 1}. ${cal.name}`);
        console.log(`     ID: ${cal.id.substring(0, 40)}...`);
