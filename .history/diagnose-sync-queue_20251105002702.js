...
if (syncMgr || debugSyncMgr) {
  console.log('✅ SyncManager 已初始化');
  console.log(`   类型: ${syncMgr?.constructor?.name || 'ActionBasedSyncManager'}`);
  
  // 获取队列（优先使用 debugSyncManager）
  const queue = debugSyncMgr?.getActionQueue?.() || syncMgr?.getActionQueue?.();
  if (queue) {
  ...
  // 最后同步时间
  const lastSyncTime = debugSyncMgr?.getLastSyncTime?.() || syncMgr?.getLastSyncTime?.();
  if (lastSyncTime) {
    console.log('');
  ...
  // 其他调试信息
  const isRunning = debugSyncMgr?.isRunning?.();
  const isSyncInProgress = debugSyncMgr?.isSyncInProgress?.();
  if (isRunning !== undefined || isSyncInProgress !== undefined) {
    console.log('');
    console.log('   🔧 同步状态:');
    if (isRunning !== undefined) console.log(`      运行中: ${isRunning ? '是' : '否'}`);
    if (isSyncInProgress !== undefined) console.log(`      同步进行中: ${isSyncInProgress ? '是' : '否'}`);
  }
...
console.log('---');
const syncActions = localStorage.getItem('remarkable-sync-actions');
if (syncActions) {
  try {
    const actions = JSON.parse(syncActions);
    console.log(`   remarkable-sync-actions 条目数: ${actions.length}`);
  } catch (e) {
    console.log('   ⚠️ remarkable-sync-actions 解析失败');
  }
} else {
  console.log('   remarkable-sync-actions 不存在');
}
...
if (events) {
  try {
    ...
    console.log(`   remarkable-events 条目数: ${evts.length}`);
  } catch (e) {
    console.log('   ⚠️ remarkable-events 解析失败');
  }
} else {
  console.log('   remarkable-events 不存在');
}
console.log('');
...
const queue = debugSyncMgr?.getActionQueue?.() || syncMgr?.getActionQueue?.();
if (queue && queue.filter(a => !a.synchronized && a.source === 'local').length > 0) {
  console.log('✅ 有待同步的本地操作');
  console.log('   操作: 手动触发同步:');
  console.log('   ```javascript');
  console.log('   await window.debugSyncManager.triggerSync()');
  console.log('   ```');
}
...
const diagnosticResult = {
  msAuth: msService?.isSignedIn?.() || false,
  syncManagerReady: !!(syncMgr || debugSyncMgr),
