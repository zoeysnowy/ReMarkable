...
function App() {
  ...
  const handleTimerStop = async () => {
    ...
    try {
      ...
      // � [FIX] Timer 事件暂时不同步到 Outlook，仅保存到本地
      // 用户可以在 TimeCalendar 中手动编辑并选择是否同步
      console.log('💾 [Timer Stop] 事件已保存到本地，暂不同步到 Outlook');
  ...
  const handleTimerEditSave = (updatedEvent: Event) => {
    ...
    try {
      ...
      // 🔄 [FIX] 仅在 Timer 停止后同步到 Outlook（运行中不同步）
      const isTimerRunning = globalTimer && globalTimer.isRunning && !globalTimer.isPaused;
      
      if (syncManager && !isTimerRunning) {
        try {
          if (isNewEvent) {
        ...
        console.log('⏸️ [App Timer] Timer is running, skipping Outlook sync (will sync when stopped)');
      }
    } catch (error) {
