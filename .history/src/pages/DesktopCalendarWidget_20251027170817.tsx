...
const DesktopCalendarWidget: React.FC = () => {
  ...
  // 🆕 Widget 从 localStorage 读取 globalTimer 和 availableTags
  ...
  // 🔄 监听 localStorage 变化，实时更新 globalTimer 和 availableTags
  useEffect(() => {
    // 初始加载
    const loadGlobalTimer = () => {
      try {
        const saved = localStorage.getItem('remarkable-global-timer');
        if (saved) {
          const timer = JSON.parse(saved);
          setGlobalTimer(timer);
          console.log('⏰ [Widget] Loaded globalTimer:', timer);
        } else {
          setGlobalTimer(null);
        }
      } catch (error) {
        console.error('❌ [Widget] Failed to load globalTimer:', error);
      }
    };

    const loadTags = () => {
      try {
        const saved = localStorage.getItem('remarkable-dev-persistent-remarkable-hierarchical-tags');
        if (saved) {
          const data = JSON.parse(saved);
          const tags = data.value || [];
          setAvailableTags(tags);
          console.log('🏷️ [Widget] Loaded tags:', tags.length);
        }
      } catch (error) {
        console.error('❌ [Widget] Failed to load tags:', error);
      }
    };
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === 'remarkable-global-timer') {
        loadGlobalTimer();
      } else if (e.key === 'remarkable-dev-persistent-remarkable-hierarchical-tags') {
        loadTags();
      }
    };
      ...
      >
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <MemoizedTimeCalendar
            ...
            availableTags={availableTags} // 🔧 传递标签
            globalTimer={globalTimer} // 🔧 传递计时器状态
            isWidgetMode={true}
