...
const WidgetPage_v3: React.FC = () => {
  ...
  const handleDragMove = useCallback(async (e: MouseEvent) => {
    ...
    console.log('🚚 [Renderer] 拖动中:', { 
      ...
      dragStartRef: { x: dragStartRef.current.x, y: dragStartRef.current.y },
    ...
    if (Math.abs(deltaX) > 0 || Math.abs(deltaY) > 0) {
      if (window.electronAPI?.widgetMove) {
        try {
          ...
          console.log('📡 [Renderer] 调用 widgetMove BEFORE:', { 
            x: deltaX, 
            y: deltaY,
          ...
          if (result.success) {
            // 关键：更新参考点为当前鼠标位置
            const oldRef = { ...dragStartRef.current };
            dragStartRef.current = {
              x: e.screenX,
              y: e.screenY
            };
            console.log('🔄 [Renderer] 更新参考点:', {
              old: oldRef,
              new: dragStartRef.current,
              moved: { x: e.screenX - oldRef.x, y: e.screenY - oldRef.y }
            });
          }
        } catch (error) {
