...
const WidgetPage: React.FC = () => {
  ...
  // 🎨 样式控制状态 - 完全复刻 DesktopCalendarWidgetV3
  const [opacity, setOpacity] = useState(0.95); // 整体透明度
  const [bgOpacity, setBgOpacity] = useState(0.0); // 背景透明度
  ...
  // 🎨 动态样式生成 - 完全复刻 DesktopCalendarWidgetV3
  // 将背景颜色转换为 rgba 格式以支持透明度
  const bgColorRgba = (() => {
  ...
  const widgetStyle: React.CSSProperties = {
    ...
    cursor: 'default',
    userSelect: 'none',
    backgroundColor: bgColorRgba, // 使用 rgba 而非 hex+alpha
  ...
  // 🎛️ 控制栏样式 - 完全复刻 DesktopCalendarWidgetV3
      ...
      {showControls && (
        <div style={controlBarStyle}>
          ...
          <div style={{
            ...
            📅 桌面日历 
            {isLocked && <span style={{ fontSize: '12px', opacity: 0.8 }}>🔒</span>}
            {!isLocked && <span style={{ fontSize: '11px', opacity: 0.7 }}>(拖拽此处移动)</span>}
          </div>
