...
const DesktopCalendarWidget: React.FC = () => {
  ...
  useEffect(() => {
    const checkAuthAndInitSync = () => {
      ...
      if (storedAuthState && !syncManager && microsoftService) {
        console.log('🚀 [Widget] 用户已登录，尝试使用全局同步管理器...');
        
        // 只使用全局 syncManager，不再创建新实例
        if (typeof window !== 'undefined' && (window as any).syncManager) {
          ...
          console.warn('⚠️ [Widget] 全局 syncManager 不存在，等待主应用初始化...');
        }
      } else if (!storedAuthState && syncManager) {
        console.log('⏸️ [Widget] 用户已登出，清除同步管理器引用...');
        setSyncManager(null);
  ...
  useEffect(() => {
    const updateSyncTime = () => {
      try {
        ...
        console.log('⚠️ [Widget] 无法获取同步时间 (syncManager 和 localStorage 都为空)');
      } catch (error) {
