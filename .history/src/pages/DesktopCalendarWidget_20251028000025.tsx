...
const DesktopCalendarWidget: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log('🔍 [Widget] Loading settings from localStorage:', savedSettings);
    
    if (savedSettings) {
      try {
        const settings = JSON.parse(savedSettings);
        console.log('📦 [Widget] Parsed settings:', settings);
        ...
        console.log('🎨 [Widget] Applying settings:', { opacity, color, locked });
      ...
      console.log('🔓 No saved settings, using defaults: bgOpacity=0.95, bgColor=#ffffff');
      // 确保主进程也是解锁状态
      if (window.electronAPI?.widgetLock) {
