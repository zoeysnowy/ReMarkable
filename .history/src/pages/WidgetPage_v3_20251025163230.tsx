  ...
  useEffect(() => {
    const checkAuthAndInitSync = () => {
      ...
      if (isAuthenticated && !syncManager) {
        console.log('🚀 [Widget] 用户已登录，初始化同步管理器...');
        try {
    ...
    >
      ...
      >
        <MemoizedTimeCalendar
          ...
          calendarProviderType={microsoftService ? 'outlook' : undefined} // 📅 根据连接状态确定日历类型
          // Widget 控制回调
        ...
        {/* 📊 Widget 专属状态栏 - 与 App 的 app-statusbar 一致 */}
        <div style={{
          position: 'absolute',
          bottom: 0,
          left: 0,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <img src={SyncIcon} alt="Sync" style={{ width: 16, height: 16 }} />
            <span>
              {lastSyncTime ? (
                <>
                  最后同步：<strong style={{ color: 'rgba(0, 0, 0, 0.87)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <img src={OutlookIcon} alt="Outlook" style={{ width: 16, height: 16 }} />
            <div
              style={{
                width: '8px',
                height: '8px',
