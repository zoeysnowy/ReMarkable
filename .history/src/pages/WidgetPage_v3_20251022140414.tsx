...
const WidgetPage_v3: React.FC = () => {
  ...
  const handleResizeMove = useCallback((e: MouseEvent) => {
    ...
    // 使用 requestAnimationFrame 来节流，避免频繁调用导致抖动
    if (!resizeStartRef.current.rafId) {
      resizeStartRef.current.rafId = requestAnimationFrame(() => {
        ...
        if (resizeStartRef.current) {
          resizeStartRef.current.rafId = null;
        }
      });
    }
  }, [isResizing]);
