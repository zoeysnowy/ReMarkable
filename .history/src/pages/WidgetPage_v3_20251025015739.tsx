...
import DesktopTimeCalendar from '../components/DesktopTimeCalendar'; // 🎨 改用 Desktop 专用日历
import { MicrosoftCalendarService } from '../services/MicrosoftCalendarService';
import '../components/DesktopCalendarWidget.css'; // 导入桌面日历 CSS

// 包装 DesktopTimeCalendar，只在必要的 props 变化时重渲染
const MemoizedTimeCalendar = memo(DesktopTimeCalendar, (prevProps, nextProps) => {
  // 只有这些 props 变化时才重渲染
  return (
    prevProps.microsoftService === nextProps.microsoftService
    // DesktopTimeCalendar 不接受 calendarBackgroundColor, calendarOpacity, widgetLocked
    // 它从 localStorage 读取自己的设置
  );
});
    ...
    >
      ...
      >
        <MemoizedTimeCalendar
          events={[]} // DesktopTimeCalendar 需要 events 数组
          tags={[]}    // DesktopTimeCalendar 需要 tags 数组
