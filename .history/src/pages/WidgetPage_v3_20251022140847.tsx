    ...
    if (deltaX !== 0 || deltaY !== 0) {
      ...
      if (window.electronAPI?.widgetMove) {
        window.electronAPI.widgetMove({ x: deltaX, y: deltaY })
          ...
          .catch((error: any) => {
            console.error('❌ widgetMove IPC 调用失败:', error);
          });
      } else {
      ...
      {!isLocked && (
        <div
          ...
          style={{
            ...
            backgroundColor: isDragging 
              ...
              : (showControls ? 'rgba(100, 150, 255, 0.3)' : 'rgba(100, 150, 255, 0.15)'), // 始终显示提示
            backdropFilter: showControls || isDragging ? 'blur(8px)' : 'blur(3px)',
            display: 'flex',
            alignItems: 'center',
            ...
            WebkitAppRegion: 'no-drag', // 确保不使用 Electron 的原生拖动
            boxShadow: (showControls || isDragging) ? '0 2px 8px rgba(100, 150, 255, 0.3)' : 'none'
        ...
        >
          <span style={{
            ...
            opacity: showControls || isDragging ? 1 : 0.6, // 根据状态显示
