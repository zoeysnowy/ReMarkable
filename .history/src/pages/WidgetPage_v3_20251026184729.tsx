...
const WidgetPage_v3: React.FC = () => {
  ...
  useEffect(() => {
    const checkAuthAndInitSync = () => {
      ...
      console.log('🔍 [Widget] 检查认证状态:', isAuthenticated);
  ...
  const handleResizeStart = useCallback((edge: string, e: React.MouseEvent) => {
    console.log('🎯 [Resize] handleResizeStart 被调用!', { edge, isLocked, clientX: e.clientX, clientY: e.clientY });
    if (isLocked) {
  ...
  return (
    ...
    >
      ...
      {!isLocked && !isDragging && (
        <>
          <div className="resize-handle-bottom" onMouseDown={(e) => { console.log('🖱️ Bottom handle mousedown!'); handleResizeStart('bottom', e); }} onMouseEnter={handleResizeHover} onMouseLeave={handleResizeLeave} />
          <div className="resize-handle-left" onMouseDown={(e) => { console.log('🖱️ Left handle mousedown!'); handleResizeStart('left', e); }} onMouseEnter={handleResizeHover} onMouseLeave={handleResizeLeave} />
          <div className="resize-handle-right" onMouseDown={(e) => { console.log('🖱️ Right handle mousedown!'); handleResizeStart('right', e); }} onMouseEnter={handleResizeHover} onMouseLeave={handleResizeLeave} />
          <div className="resize-handle-bottomleft" onMouseDown={(e) => { console.log('🖱️ BottomLeft handle mousedown!'); handleResizeStart('bottomleft', e); }} onMouseEnter={handleResizeHover} onMouseLeave={handleResizeLeave} />
          <div className="resize-handle-bottomright" onMouseDown={(e) => { console.log('🖱️ BottomRight handle mousedown!'); handleResizeStart('bottomright', e); }} onMouseEnter={handleResizeHover} onMouseLeave={handleResizeLeave} />
        </>
      )}
      ...
      >
        ...
        <div 
          style={{
            flexShrink: 0,
            margin: '0 8px 20px 8px', // 🎯 左右 8px，底部 20px 间距（为 resize handle 留出空间）
