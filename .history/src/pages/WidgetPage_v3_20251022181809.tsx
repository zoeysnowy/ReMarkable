...
const WidgetPage_v3: React.FC = () => {
  ...
  // 拖动状态 - 现在使用原生 -webkit-app-region: drag，不需要自定义拖动逻辑
  // const [isDragging, setIsDragging] = useState(false);
  // const dragStartRef = useRef<{ x: number; y: number } | null>(null);
  // const dragThrottleRef = useRef<number>(0); // 拖动节流用的时间戳
  ...
  const handleResizeMove = useCallback((e: MouseEvent) => {
    if (!isResizing || !resizeStartRef.current || isDragging) return; // 🔒 拖动时禁止调整大小
  ...
  }, [isResizing, isDragging]); // 添加 isDragging 依赖
  ...
  const handleDragStart = useCallback((e: React.MouseEvent) => {
    console.log('🎯 handleDragStart 被调用:', { isLocked, isResizing, screenX: e.screenX, screenY: e.screenY });
    
    if (isLocked) {
      console.warn('⚠️ Widget 已锁定，拒绝拖动');
      return;
    }
    
    if (isResizing) {
      console.warn('⚠️ 正在调整大小，拒绝拖动');
      return;
    }
  ...
  }, [isLocked, isResizing]); // 添加 isResizing 依赖

  const handleDragMove = useCallback(async (e: MouseEvent) => {
    if (!isDragging || !dragStartRef.current || isResizing) return; // 🔒 调整大小时禁止拖动
    
    // 节流：每 32ms (约30fps) 最多执行一次 - 降低频率以确保 IPC 不会堵塞
    const now = Date.now();
    ...
    if (Math.abs(deltaX) > 0 || Math.abs(deltaY) > 0) {
      ...
      console.log(`🎯 [Renderer] 拖动中: delta=(${deltaX}, ${deltaY}), screen=(${e.screenX}, ${e.screenY})`);
      console.log('📐 [Renderer] 拖动前尺寸:', beforeMove);
      ...
      if (window.electronAPI?.widgetMove) {
        try {
          ...
          console.log('📐 [Renderer] 拖动后尺寸:', afterMove);
          
          // 检查尺寸是否变化
          if (beforeMove.innerWidth !== afterMove.innerWidth || 
              beforeMove.innerHeight !== afterMove.innerHeight) {
            console.error('⚠️ [Renderer] 窗口尺寸在拖动中发生了变化！', {
              widthChange: afterMove.innerWidth - beforeMove.innerWidth,
              heightChange: afterMove.innerHeight - beforeMove.innerHeight
            });
          }
        ...
        console.warn('⚠️ electronAPI.widgetMove 不可用');
      }
    }
  }, [isDragging, isResizing]); // 添加 isResizing 依赖
  ...
  return (
    ...
    >
      ...
      {/* 顶部拖动条 - 整个顶部10px区域都可以拖动 */}
      {/* 顶部拖动条 - 使用原生 Electron 拖动，避免 setPosition 的尺寸 bug */}
      {!isLocked && (
        <div
          className="drag-bar"
          ...
          style={{
            ...
            cursor: 'grab',
            backgroundColor: showControls ? 'rgba(100, 150, 255, 0.5)' : 'rgba(100, 150, 255, 0.25)', // 加深颜色使其更明显
      ...
      {/* 调整大小手柄 - 只保留底部和侧边（禁用所有顶部相关手柄，避免与拖动条冲突） */}
      {/* 拖动时隐藏 resize handles，防止意外触发 */}
      {!isLocked && !isDragging && (
        <>
          {/* 禁用所有顶部相关手柄（top、topleft、topright），整个顶部区域专用于拖动 */}
