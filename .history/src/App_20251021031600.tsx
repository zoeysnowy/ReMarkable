...
function App() {
  ...
  const handleStartTimeChange = (newStartTime: number) => {
    ...
    const now = Date.now();
    const oldStartTime = globalTimer.startTime;
    const timeDiff = oldStartTime - newStartTime; // 时间差（正数=提前开始，负数=延后开始）
    
    console.log('⏰ [App] 修改开始时间:', {
      旧开始时间: new Date(oldStartTime).toLocaleString(),
      新开始时间: new Date(newStartTime).toLocaleString(),
      时间差分钟: Math.round(timeDiff / 60000),
      当前elapsedTime: globalTimer.elapsedTime,
      isRunning: globalTimer.isRunning
    });
    
    if (globalTimer.isRunning) {
      // 计时中：重新计算 elapsedTime
      // 当前总时长 = elapsedTime + (now - oldStartTime)
      // 新的 elapsedTime = 当前总时长 + timeDiff
      const currentTotalElapsed = globalTimer.elapsedTime + (now - oldStartTime);
      const newElapsedTime = currentTotalElapsed + timeDiff;
      
      setGlobalTimer({
        ...globalTimer,
        startTime: now, // 重置startTime为当前时间
        elapsedTime: newElapsedTime // 更新elapsedTime以反映新的开始时间
      });
      
      console.log('⏰ [App] 计时中修改:', {
        当前总时长毫秒: currentTotalElapsed,
        新elapsedTime毫秒: newElapsedTime,
        增加的时长分钟: Math.round(timeDiff / 60000)
      });
    } else {
      // 暂停中：直接调整 elapsedTime
      const newElapsedTime = globalTimer.elapsedTime + timeDiff;
      
      setGlobalTimer({
        ...globalTimer,
        elapsedTime: Math.max(0, newElapsedTime) // 确保不为负数
      });
      
      console.log('⏰ [App] 暂停中修改:', {
