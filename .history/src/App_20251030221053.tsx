...
function App() {
    ...
    const loadPlanItems = () => {
      try {
        ...
        if (saved) {
          ...
          console.log('📋 [App] 计划项详情:', items.map((i: PlanItem) => ({ id: i.id, title: i.title, tags: i.tags })));
        }
      } catch (error) {
  ...
  const handleSavePlanItem = useCallback((item: PlanItem) => {
    console.log('💾 [App] 准备保存计划项:', { id: item.id, title: item.title, tags: item.tags });
    setPlanItems(prev => {
      ...
      console.log('💾 [App] 已保存到 localStorage');
      console.log('💾 [App] 更新后的所有计划项:', updated.map(i => ({ id: i.id, title: i.title, tags: i.tags })));
