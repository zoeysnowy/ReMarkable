...
private getUserSettings(): any {
  ...
  private async applyRemoteActionToLocal(
    ...
    switch (action.type) {
      case 'create':
        ...
        // 🔧 [FIX] 检查三种可能的匹配方式：
        // 1. 通过 Outlook ID（rawRemoteId）
        // 2. 通过本地生成的 ID（newEvent.id）
        // 3. 通过 externalId 反向查找（防止 Timer 事件重复）
        const existingByRawId = this.eventIndexMap.get(rawRemoteId);
        const existingByNewId = this.eventIndexMap.get(newEvent.id);
        const existingByExternalId = this.eventIndexMap.get(newEvent.externalId);
        const existingEvent = existingByRawId || existingByNewId || existingByExternalId;
        
        // 🔍 [DEBUG] 详细日志
        if (newEvent.title?.includes('ReMarkable')) {
          console.log('🔍 [REMOTE → LOCAL] Duplicate check:', {
            remoteId: rawRemoteId,
            newEventId: newEvent.id,
            newEventExternalId: newEvent.externalId,
            existingByRawId: existingByRawId?.id,
            existingByNewId: existingByNewId?.id,
            existingByExternalId: existingByExternalId?.id,
            willSkip: !!existingEvent,
            indexMapSize: this.eventIndexMap.size
          });
        }
        ...
        if (!existingEvent) {
          ...
          console.log('✅ [RemoteToLocal] Event already exists, skipping duplicate:', {
            remoteId: rawRemoteId,
  ...
  private getLocalEvents() {
    try {
      ...
      if (this.eventIndexMap.size === 0 && events.length > 0) {
        console.log('🔧 [IndexMap] Initial build on first load');
  ...
  private updateEventInIndex(event: any, oldEvent?: any) {
    ...
    if (event) {
      ...
      // 🔍 [DEBUG] Timer 事件索引日志
      if (event.title?.includes('ReMarkable') || event.id?.startsWith('timer-')) {
        console.log('📇 [IndexMap] Updated event index:', {
          eventId: event.id,
          externalId: event.externalId,
          title: event.title,
          indexMapSize: this.eventIndexMap.size,
          bothIndexed: !!(event.id && event.externalId)
        });
      }
    }
  }
