...
private getUserSettings(): any {
  ...
  private rebuildEventIndexMap(events: any[]) {
    ...
    events.forEach(event => {
      ...
      if (event.externalId) {
        // 🔧 [FIX] 优先保留 Timer 事件的 externalId 索引
        // 如果已经有一个事件索引了这个 externalId，检查优先级
        const existing = newMap.get(event.externalId);
        if (!existing || event.id.startsWith('timer-')) {
          // 如果没有现有事件，或者当前是 Timer 事件，则使用当前事件
          newMap.set(event.externalId, event);
        }
        // 否则保留现有的索引（Outlook 事件不覆盖 Timer 事件）
      }
    });
