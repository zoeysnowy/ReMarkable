    ...
    try {
      ...
      if (duplicates.length > 0) {
        console.warn(`⚠️ [Sync] Found ${duplicates.length} duplicate externalIds in local events:`, 
          duplicates.slice(0, 5).map(([id, count]) => `${id.substring(0, 20)}...: ${count} copies`));
      }
...
private getUserSettings(): any {
  ...
  private rebuildEventIndexMap(events: any[]) {
    events.forEach(event => {
      ...
      if (event.externalId) {
        // 🔧 [CRITICAL FIX] 不要用 externalId 作为唯一键！
        // 因为多个本地事件可能有相同的 externalId（重复创建或系列事件）
        // externalId 只应该用于查找"是否存在对应的远程事件"
        // 解决方案：只保留 event.id 作为索引键
        // 
        // 旧逻辑会导致：如果5个事件有相同的 externalId，只有最后一个被保留
        // 其他4个在删除检测时会找不到，被误判为"远程已删除"
        
        // ❌ 不再使用 externalId 作为 Map 的键
        // const existing = this.eventIndexMap.get(event.externalId);
        // if (!existing || event.id.startsWith('timer-')) {
        //   this.eventIndexMap.set(event.externalId, event);
        // }
    ...
    console.log(`🚀 [IndexMap] Rebuilt index with ${this.eventIndexMap.size} entries for ${events.length} events (id-only)`);
  }
