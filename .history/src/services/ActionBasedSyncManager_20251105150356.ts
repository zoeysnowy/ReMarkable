...
export class ActionBasedSyncManager {
  ...
  private async performSync() {
    ...
    try {
      ...
      // 🔧 [OPTIMIZED] 智能同步：根据待同步操作类型决定是否需要拉取远程数据
      ...
      const pendingRemoteActions = this.actionQueue.filter(
        action => action.source === 'remote' && !action.synchronized
      );
      
      const hasLocalActions = pendingLocalActions.length > 0;
      const hasRemoteActions = pendingRemoteActions.length > 0;
      const needsRemoteSync = hasRemoteActions || !hasLocalActions; // 有远程待同步，或没有任何待同步（定期检查）
      
      console.log(`📊 [Sync] Pending actions: ${pendingLocalActions.length} local, ${pendingRemoteActions.length} remote`);
      
      // 只在必要时拉取远程数据（避免不必要的429错误）
      if (needsRemoteSync) {
        console.log('🔄 [Sync] Fetching remote changes (has remote actions or regular check)...');
        await this.fetchRemoteChanges();
      } else {
        console.log('⏩ [Sync] Skipping remote fetch (only local actions to sync)');
        // 但需要验证目标日历是否存在
        await this.validateCalendarsForLocalActions(pendingLocalActions);
      }
