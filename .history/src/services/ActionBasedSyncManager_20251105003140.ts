...
private getUserSettings(): any {
  ...
  private fixOrphanedPendingEvents() {
    ...
    try {
      ...
      // 2. remarkableSource = true 或 undefined（本地创建，undefined 是历史数据）
      ...
      const pendingEvents = events.filter((event: any) => {
        // 🔧 [FIX] 兼容历史数据：remarkableSource 为 undefined 时也视为本地创建
        const isLocalEvent = event.remarkableSource === true || event.remarkableSource === undefined;
