  ...
  private async fetchRemoteChanges() {
    try {
      ...
      if (duplicates.length > 0) {
        console.warn(`⚠️ [Sync] Found ${duplicates.length} duplicate externalIds in local events`);
        
        // 🔍 详细列出前3个重复的事件信息
        duplicates.slice(0, 3).forEach(([externalId, count]) => {
          console.warn(`🔍 [Sync] Duplicate externalId (${count} copies): ${externalId.substring(0, 30)}...`);
