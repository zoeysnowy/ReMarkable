  ...
  private async fetchRemoteChanges() {
    try {
      ...
      remarkableEvents.forEach((event: any) => {
        ...
        // 🔧 [FIX] 需要检查三种可能的匹配：
        // 1. event.id（带 outlook- 前缀，如 'outlook-AAMkAD...'）
        // 2. 兼容旧数据的 'outlook-' + event.id
        // 3. 纯 Outlook ID（去掉 outlook- 前缀），用于匹配 Timer 事件的 externalId
        const pureOutlookId = event.id.replace(/^outlook-/, '');
        const existingLocal = this.eventIndexMap.get(event.id) || 
                             this.eventIndexMap.get(`outlook-${event.id}`) ||
                             this.eventIndexMap.get(pureOutlookId);
