...
export class ActionBasedSyncManager {
  ...
  private async fetchRemoteChanges() {
    try {
      ...
      if (isFullSync) {
        // 全量同步：上次同步时间的前7天 → 现在 + 未来 3 个月
        // 🔧 [FIX] 向前多查7天，避免漏掉同步时间附近的事件
        const baseStartDate = this.lastSyncTime || new Date(now.getTime() - 365 * 24 * 60 * 60 * 1000);
        startDate = new Date(baseStartDate);
        startDate.setDate(startDate.getDate() - 7); // 向前7天
        ...
        console.log('📅 [Sync] FULL sync from last sync time -7 days to now + 3 months:', {
      ...
      localEventsWithExternalId.forEach((localEvent: any) => {
        ...
        if (isInSyncWindow) {
          ...
          if (!isFoundInRemote) {
            ...
            // 🔍 [DEBUG] 只在删除"🧨刷小红书"时详细记录
            if (deletionFoundCount < 5 && localEvent.title && localEvent.title.includes('🧨刷小红书')) {
              console.warn(`🗑️ [Sync] Deleting "🧨刷小红书":`, {
                externalId: cleanExternalId.substring(0, 50) + '...',
                startTime: localEventTime.toISOString(),
                inRemoteIds: remoteEventIds.has(cleanExternalId)
              });
            } else if (deletionFoundCount < 5) {
              console.log(`🗑️ [Sync] Event deleted remotely: "${localEvent.title}"`);
            }
