  ...
  private async fetchRemoteChanges() {
    try {
      ...
      localEventsWithExternalId.forEach((localEvent: any) => {
        ...
        // 🔧 [NEW] 跳过 local-only 事件（例如运行中的 Timer）
        if (localEvent.syncStatus === 'local-only') {
          console.log(`⏭️ [Sync] Skipping local-only event from deletion check: "${localEvent.title}"`);
          return;
        }
        
        // 🔧 [NEW] 跳过 remarkableSource 且没有 externalId 的事件（纯本地事件）
        if (localEvent.remarkableSource && !localEvent.externalId) {
          console.log(`⏭️ [Sync] Skipping pure local event: "${localEvent.title}"`);
          return;
        }
        ...
        if (isInSyncWindow) {
          ...
          if (!isFoundInRemote) {
            ...
            // 🔧 [NEW] 延长保护窗口到 60 秒（原来是 30 秒）
            if (recentlyUpdated && timeSinceUpdate < 60000) {
...
private getUserSettings(): any {
  ...
  private rebuildEventIndexMap(events: any[]) {
    ...
    console.log(`🚀 [IndexMap] Built index with ${this.eventIndexMap.size} entries for ${events.length} events`);
  }
