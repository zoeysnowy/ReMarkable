...
export class ActionBasedSyncManager {
  ...
  private async performSync() {
    ...
    // 🔧 防止短时间内重复同步（最小间隔 15 秒）
    // 原因：每次同步会向10个日历发起请求，短时间内多次同步会触发Microsoft Graph API速率限制(429)
    const now = Date.now();
    const timeSinceLastSync = this.lastSyncTime ? (now - this.lastSyncTime.getTime()) : Infinity;
    if (timeSinceLastSync < 15000) {
      console.log(`⏸️ [performSync] Last sync was ${Math.round(timeSinceLastSync / 1000)}s ago, skipping (minimum 15s interval)`);
      return;
    }
