...
private getUserSettings(): any {
    ...
    switch (action.type) {
      ...
      case 'delete':
        ...
        if (eventToDeleteIndex !== -1) {
          ...
          if (!isBatchMode) {
            this.saveLocalEvents(events);
            console.log('✅ [RemoteToLocal] Event deleted from local storage, remaining events:', events.length);
          }
      ...
      if (eventIndex !== -1) {
        ...
        if (oldEventId !== updatedEvent.id) {
          ...
          if (existingIndex === -1) {
            ...
            console.log(`✅ [ActionBasedSyncManager] Replaced event: removed ${oldEventId}, added ${updatedEvent.id}`);
          } else {
            // 如果新ID已存在，更新现有事件
            events[existingIndex] = updatedEvent;
            console.log(`🔀 [ActionBasedSyncManager] Updated existing event: ${updatedEvent.id}`);
          }
          ...
          // ID没有变化，直接更新
          events[eventIndex] = updatedEvent;
          console.log(`📝 [ActionBasedSyncManager] Updated local event: ${oldEventId}`);
        }
  ...
  /**
   * 🔧 启动完整性检查调度器
   * 使用定时器 + 空闲检测，在不阻塞用户操作的前提下执行检查
   */
  private startIntegrityCheckScheduler() {
    // 每 5 分钟尝试一次完整性检查
    this.indexIntegrityCheckInterval = setInterval(() => {
      this.scheduleIdleIntegrityCheck();
    }, 300000); // 5 分钟

    // 立即调度第一次检查（延迟 30 秒启动，避免干扰初始化）
    setTimeout(() => {
      this.scheduleIdleIntegrityCheck();
    }, 30000);

    console.log('✅ [Integrity] Scheduler started (5-minute interval)');
  }
  ...
  private scheduleIdleIntegrityCheck() {
    // 检查是否应该运行（避免与同步冲突）
    if (this.syncInProgress) {
      console.log('⏸️ [Integrity] Sync in progress, skipping check');
      return;
    }

    // 检查距离上次检查的时间（避免频繁检查）
    const now = Date.now();
    if (now - this.lastIntegrityCheck < 60000) {
      console.log('⏸️ [Integrity] Last check was recent, skipping');
      return;
    }
  ...
  /**
   * 🔧 增量完整性检查（分批执行，不阻塞 UI）
   * @param deadline requestIdleCallback 提供的截止时间对象
   */
  private runIncrementalIntegrityCheck(deadline?: IdleDeadline) {
    const startTime = performance.now();
    this.lastIntegrityCheck = Date.now();

    console.log('🔍 [Integrity] Starting incremental check...');

    try {
      // 获取 localStorage 中的事件
      const stored = localStorage.getItem(STORAGE_KEYS.EVENTS);
      if (!stored) {
        console.log('✅ [Integrity] No events in storage, skipping check');
        return;
      }

      const events = JSON.parse(stored);
      console.log(`🔍 [Integrity] Checking ${events.length} events`);

      const issues: any[] = [];
      let checked = 0;
      const maxBatchSize = 100; // 每批最多检查 100 个事件
      const maxDuration = 50; // 最多执行 50ms

      // 检查 1: IndexMap 一致性
      const indexSize = this.eventIndexMap.size;
      const expectedSize = events.length * 2; // 每个事件最多 2 个索引（id + externalId）

      if (indexSize === 0 && events.length > 0) {
        issues.push({
          type: 'missing-index',
          severity: 'warning',
          message: `IndexMap is empty but ${events.length} events exist`,
          autoFix: true
        });
        // 自动修复：重建索引
        console.warn('⚠️ [Integrity] IndexMap is empty, rebuilding...');
        this.rebuildEventIndexMap(events);
      }

      // 检查 2: 逐个验证事件
      for (let i = 0; i < events.length && i < maxBatchSize; i++) {
        ...
        if (!event.id) {
          issues.push({
            ...
            severity: 'critical',
            eventIndex: i,
            message: 'Event missing required field: id'
          });
          continue;
        }
        ...
        if (!indexedEvent) {
          issues.push({
            ...
            severity: 'warning',
            eventId: event.id,
            message: `Event ${event.id} not in IndexMap`,
            autoFix: true
          });
          // 自动修复：添加到索引
          this.updateEventInIndex(event);
        } else if (indexedEvent !== event) {
          // 引用不一致（可能是过期数据）
          issues.push({
            type: 'stale-reference',
            severity: 'warning',
            eventId: event.id,
            message: `IndexMap reference is stale for ${event.id}`,
            autoFix: true
          });
          // 自动修复：更新索引
          this.updateEventInIndex(event, indexedEvent);
        }

        // 检查 externalId 唯一性
        if (event.externalId) {
          const duplicates = events.filter((e: any) => 
            e.externalId === event.externalId && e.id !== event.id
          );
          if (duplicates.length > 0) {
            issues.push({
              type: 'duplicate-external-id',
              severity: 'critical',
              eventId: event.id,
              externalId: event.externalId,
              duplicateCount: duplicates.length,
              message: `Duplicate externalId: ${event.externalId} (${duplicates.length + 1} events)`
            });
          }
        }
        ...
        if (event.startTime && event.endTime) {
          ...
          if (end < start) {
            issues.push({
              ...
              severity: 'warning',
              eventId: event.id,
              message: `Invalid time range: endTime < startTime`
            });
          }
        }
        ...
        if (deadline && deadline.timeRemaining() < 5) {
          console.log(`⏸️ [Integrity] Timeout reached, checked ${checked}/${events.length} events`);
          break;
        }
        
        const elapsed = performance.now() - startTime;
        if (elapsed > maxDuration) {
          console.log(`⏸️ [Integrity] Duration limit reached (${elapsed.toFixed(0)}ms), checked ${checked}/${events.length} events`);
          break;
        }
      }
      ...
      // 检查 3: 孤立索引（IndexMap 中有但 localStorage 中没有）
      let orphanedCount = 0;
      const eventIds = new Set(events.map((e: any) => e.id));
      this.eventIndexMap.forEach((value, key) => {
        if (!eventIds.has(key) && !key.startsWith('outlook-')) {
          orphanedCount++;
          if (orphanedCount <= 10) { // 只报告前 10 个
            issues.push({
              type: 'orphaned-index',
              severity: 'info',
              indexKey: key,
              message: `Orphaned index entry: ${key}`,
              autoFix: true
            });
          }
        }
      ...
      // 输出检查结果
      if (issues.length === 0) {
        console.log(`✅ [Integrity] Check completed: ${checked} events, no issues found (${duration.toFixed(0)}ms)`);
      } else {
        console.warn(`⚠️ [Integrity] Check completed: ${checked} events, ${issues.length} issues found (${duration.toFixed(0)}ms)`);
        ...
        if (critical.length > 0) {
          console.error(`🔴 [Integrity] ${critical.length} critical issues:`, critical);
        }
        if (warnings.length > 0) {
          console.warn(`🟡 [Integrity] ${warnings.length} warnings:`, warnings);
        }
        if (info.length > 0) {
          console.log(`🟢 [Integrity] ${info.length} info messages:`, info);
        }

        // 自动修复可修复的问题
        const autoFixable = issues.filter(i => i.autoFix);
        if (autoFixable.length > 0 && autoFixable.length <= 20) {
          console.log(`🔧 [Integrity] Auto-fixing ${autoFixable.length} issues...`);
          // 已经在检查过程中修复了
        }
      }

      // 计算健康评分
      const healthScore = this.calculateHealthScore(events.length, issues);
      console.log(`📊 [Integrity] Health score: ${healthScore}/100`);

      // 如果健康评分低于 80，触发警告
      if (healthScore < 80) {
        console.warn(`⚠️ [Integrity] Low health score: ${healthScore}/100, consider running full check`);
      }
