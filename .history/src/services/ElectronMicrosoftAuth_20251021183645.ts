...
export class ElectronMicrosoftAuth {
  ...
  private startLocalServer(
    resolve: (value: ElectronAuthResult) => void, 
    reject: (reason: any) => void
  ) {
    // 这里需要在主进程中实现HTTP服务器
    // 由于这是在渲染进程中，我们通过IPC与主进程通信
    
    if (typeof window !== 'undefined' && window.electronAPI && window.electronAPI.startAuthServer) {
      window.electronAPI.startAuthServer(this.redirectUri)
        .then((authCode: string) => {
          // 收到授权码后，交换访问令牌
          return this.exchangeCodeForToken(authCode);
        })
        .then((result: ElectronAuthResult) => resolve(result))
