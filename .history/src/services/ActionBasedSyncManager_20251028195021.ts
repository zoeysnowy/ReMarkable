...
export class ActionBasedSyncManager {
  ...
  private async fetchRemoteChanges() {
    try {
      ...
      // 🔍 [DEBUG] 记录删除检测的基本信息
      console.log(`🔍 [Sync] Deletion check setup:`, {
        combinedEventsCount: combinedEvents.length,
        remoteEventIdsCount: remoteEventIds.size,
        localEventsCount: localEvents.length,
        syncWindow: `${startDate.toLocaleDateString()} - ${endDate.toLocaleDateString()}`
      });
      
      // 🔍 [DEBUG] 检查远程是否有"🧨刷小红书"事件
      const remoteTargetEvents = combinedEvents.filter((e: any) => 
        e.subject && e.subject.includes('🧨刷小红书')
      );
      if (remoteTargetEvents.length > 0) {
        console.warn(`🔍 [Sync] Found ${remoteTargetEvents.length} "🧨刷小红书" events in REMOTE (combinedEvents):`);
        remoteTargetEvents.forEach((evt: any, idx: number) => {
          const rawId = evt.id.startsWith('outlook-') ? evt.id.replace('outlook-', '') : evt.id;
          console.warn(`  [${idx + 1}/${remoteTargetEvents.length}]`, {
            id: evt.id.substring(0, 70) + '...',
            rawId: rawId.substring(0, 70) + '...',
            startTime: evt.start?.dateTime || evt.start || evt.startTime,
            inRemoteEventIds: remoteEventIds.has(rawId)
          });
        });
      } else {
        console.warn(`⚠️ [Sync] NO "🧨刷小红书" events found in REMOTE (combinedEvents)`);
      }
      ...
      console.log(`🔍 [Sync] Local events with externalId: ${localEventsWithExternalId.length}`);
      ...
      localEventsWithExternalId.forEach((event: any) => {
        ...
        if (!externalIdToEvents.has(cleanId)) {
          externalIdToEvents.set(cleanId, []);
        }
        externalIdToEvents.get(cleanId)!.push(event);
      });
      ...
      if (duplicates.length > 0) {
        ...
        duplicates.slice(0, 3).forEach(([externalId, count]) => {
          console.warn(`🔍 [Sync] Duplicate externalId (${count} copies): ${externalId.substring(0, 40)}...`);
          const duplicateEvents = externalIdToEvents.get(externalId);
          if (duplicateEvents) {
            duplicateEvents.forEach((evt, idx) => {
              console.warn(`  [${idx + 1}/${count}]`, {
                localId: evt.id.substring(0, 60),
                title: evt.title,
                startTime: evt.startTime || evt.start,
        ...
        console.log(`✅ [Sync] No duplicate externalIds found in localStorage`);
      }

      // 🔍 [DEBUG] 专门检查"🧨刷小红书"事件
      const targetEvents = localEvents.filter((e: any) => e.title && e.title.includes('🧨刷小红书'));
      if (targetEvents.length > 0) {
        console.warn(`🔍 [Sync] Found ${targetEvents.length} "🧨刷小红书" events in localStorage:`);
        targetEvents.forEach((evt: any, idx: number) => {
          console.warn(`  [${idx + 1}/${targetEvents.length}]`, {
            id: evt.id.substring(0, 70) + '...',
            externalId: evt.externalId ? evt.externalId.substring(0, 70) + '...' : 'none',
            startTime: evt.startTime || evt.start,
            createdAt: evt.createdAt,
            updatedAt: evt.updatedAt,
            source: evt.source
      ...
      localEventsWithExternalId.forEach((localEvent: any) => {
        ...
        if (isInSyncWindow) {
          ...
          if (!isFoundInRemote) {
            ...
            if (deletionFoundCount < 5) {
              // 🔍 额外检查：看看远程是否有类似的ID
              const similarIds = Array.from(remoteEventIds).filter(id => 
                id.includes(cleanExternalId.substring(0, 50)) || 
                cleanExternalId.includes(id.substring(0, 50))
              );
              ...
              console.log(`🗑️ [Sync] Event deleted remotely, removing locally: "${localEvent.title}"`, {
                ...
                cleanExternalId: cleanExternalId.substring(0, 60) + '...',
                eventTime: localEventTime.toISOString(),
                isInWindow: isInSyncWindow,
                foundInRemote: isFoundInRemote,
                remoteEventIdsSize: remoteEventIds.size,
                similarIdsInRemote: similarIds.length > 0 ? similarIds.map(id => id.substring(0, 60) + '...') : 'none'
              });
            } else if (deletionFoundCount === 5) {
              console.log(`🗑️ [Sync] Event deleted remotely (and more)...`);
            }
