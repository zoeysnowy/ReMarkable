...
export const TimeCalendar: React.FC<TimeCalendarProps> = ({
  ...
  // 📅 为 Week/Day 视图注入星期行（类似 Month 视图）
  useEffect(() => {
    if (!isCalendarReady || !calendarRef.current) return;
    if (currentView === 'month') return; // 月视图已有星期行
    
    // 等待 DOM 渲染完成
    const timer = setTimeout(() => {
      const container = document.querySelector('.toastui-calendar-week-view-day-names');
      if (!container) return;
      
      // 检查是否已存在自定义星期行
      const existing = container.querySelector('.custom-weekday-row');
      if (existing) return;
      
      // 创建星期行
      const weekdayRow = document.createElement('div');
      weekdayRow.className = 'custom-weekday-row';
      
      const dayNames = ['日', '一', '二', '三', '四', '五', '六'];
      const startDay = currentView === 'day' ? new Date(currentDate).getDay() : 0;
      const displayDays = currentView === 'day' ? [dayNames[startDay]] : dayNames;
      
      weekdayRow.innerHTML = displayDays
        .map(day => `<span class="custom-weekday-item">${day}</span>`)
        .join('');
      
      // 插入到容器顶部
      container.insertBefore(weekdayRow, container.firstChild);
    }, 100);
    
    return () => clearTimeout(timer);
  }, [isCalendarReady, currentView, currentDate]);
        ...
        <ToastUIReactCalendar
          ...
          template={{
            ...
            weekDayName(model: any) {
              ...
              // 返回两行结构：星期 + 日期
              return `<div class="week-day-name-wrapper ${todayClass}"><div class="week-day-label">${dayName}</div><div class="week-date-label">${dateDisplay}</div></div>`;
            },
                  dateDisplay = `${month}/${date}`;
                }
              }
              
              // 只返回日期（星期行通过DOM操作单独添加）
              return `<span class="${todayClass}">${dateDisplay}</span>`;
            },
