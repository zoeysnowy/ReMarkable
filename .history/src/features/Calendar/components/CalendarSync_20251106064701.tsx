...
const CalendarSync: React.FC<CalendarSyncProps> = ({ 
  ...
  const handleForceSync = async () => {
    ...
    try {
      ...
      // 获取默认日历ID
      const calendarId = microsoftService.getSelectedCalendarId();
      if (!calendarId) {
        setSyncMessage('❌ 未找到默认日历');
        return;
      }
      ...
      const events = await microsoftService.getEventsFromCalendar(calendarId, startDate, endDate);
      console.log(`✅ [CalendarSync] 获取到 ${events.length} 个远程事件`);
      
      setSyncMessage(`✅ 同步完成！获取到 ${events.length} 个远程事件`);
