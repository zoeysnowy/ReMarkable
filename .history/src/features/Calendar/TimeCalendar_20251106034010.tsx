          ...
          template={{
            ...
            weekDayName(model: any) {
              ...
              if (dateInstance) {
                ...
                // 为每月1号显示 "星期 月/日" 格式
                if (date === 1) {
                  return `<span class="toastui-calendar-template-weekDayName ${todayClass}"><span style="font-size: 11px; opacity: 0.7; margin-right: 4px;">${dayName}</span>${month}/${date}</span>`;
                }
              }
              
              // 其他日期显示 "星期 日期" 格式
              return `<span class="toastui-calendar-template-weekDayName ${todayClass}"><span style="font-size: 11px; opacity: 0.7; margin-right: 4px;">${dayName}</span>${date}</span>`;
            },
