...
function App() {
    ...
    if (currentAuthState !== lastAuthState) {
      ...
      if (currentAuthState) {
        ...
        if (!syncManager) {
          ...
          AppLogger.log('ℹ️ 同步管理器已存在，无需重新创建');
          // 不再重复调用 start()，避免重复同步
        }
      } else {
  ...
  }, [microsoftService, lastAuthState, syncManager]);  // ✅ 只依赖对象引用，不要调用函数
  ...
  const renderCurrentPage = useMemo(() => {
    ...
    switch (currentPage) {
      ...
      case 'plan':
        // 🔧 过滤 Plan Items：TimeCalendar 创建的 event 只显示未来和进行中的事件
        const filteredPlanItems = planItems.filter(item => {
          // 如果没有关联 eventId，保留所有项（纯 PlanItem）
          if (!item.eventId) {
            return true;
          }
          ...
          // 如果 Event 不存在，保留该项（可能是数据不一致）
          if (!event) {
            return true;
          }
          
          // 如果不是 TimeCalendar 创建的 event（remarkableSource !== true），保留
          if (event.remarkableSource !== true) {
            return true;
          }
          
          // TimeCalendar 创建的 event：只显示未过期的（当前时间 < 结束时间）
          const now = new Date();
