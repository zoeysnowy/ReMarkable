  ...
  useEffect(() => {
    const initializeApp = async () => {
      ...
      if (typeof window !== 'undefined') {
        ...
        // 暴露恢复工具到全局，便于在控制台手动触发
        (window as any).ReMarkableRecovery = {
          fromSnapshots: (date?: string) => {
            const d = date || new Date().toISOString().split('T')[0];
            const items = snapshotService.restoreSnapshot(d);
            if (items && items.length > 0) {
              localStorage.setItem(STORAGE_KEYS.PLAN_ITEMS, JSON.stringify(items));
            }
            return items;
          },
          fromEvents: () => RecoveryService.recoverPlanItemsFromEvents(),
          ensureDefaultTags: () => RecoveryService.ensureDefaultTags(),
        };
      }
    };
    ...
    const loadPlanItems = () => {
      try {
        ...
        if (saved) {
          ...
          if (!items || items.length === 0) {
            // 尝试自动恢复：优先使用快照，其次从事件恢复
            const today = new Date().toISOString().split('T')[0];
            const recoveredFromSnapshot = snapshotService.restoreSnapshot(today);
            if (recoveredFromSnapshot && recoveredFromSnapshot.length > 0) {
              localStorage.setItem(STORAGE_KEYS.PLAN_ITEMS, JSON.stringify(recoveredFromSnapshot));
              setPlanItems(recoveredFromSnapshot);
              AppLogger.log('🩺 [App] 已从快照自动恢复计划项:', recoveredFromSnapshot.length);
            } else {
              const recoveredFromEvents = RecoveryService.recoverPlanItemsFromEvents();
              if (recoveredFromEvents && recoveredFromEvents.length > 0) {
                setPlanItems(recoveredFromEvents);
                AppLogger.log('🩺 [App] 已从事件自动恢复计划项:', recoveredFromEvents.length);
              }
            }
          }
        } else {
