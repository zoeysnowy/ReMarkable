...
function parseChineseDate(text: string, refDate: Date): ParsedDate | null {
  ...
  // 仅“下周/下週”（未指定星期）：解析为下周一
  else if (/^下周(?![一二三四五六日天])/.test(trimmed) || /^下週(?![一二三四五六日天])/.test(trimmed)) {
    const currentDay = result.start.getDay(); // 0(日)~6(六)
    const daysUntilNextMonday = ((1 - currentDay + 7) % 7) || 7; // 下一个周一
    result.start.setDate(result.start.getDate() + daysUntilNextMonday);
  }
  // 仅“本周/这周/這週”（未指定星期）：解析为本周一
  else if (/^(本周|本週|这周|這週)$/.test(trimmed)) {
    const currentDay = result.start.getDay();
    const daysFromMonday = (currentDay + 6) % 7; // 距离周一的天数
    result.start.setDate(result.start.getDate() - daysFromMonday);
  }
  // 仅“上周/上週”（未指定星期）：解析为上周一
  else if (/^上周$/.test(trimmed) || /^上週$/.test(trimmed)) {
    const currentDay = result.start.getDay();
