...
function parseChineseDate(text: string, refDate: Date): ParsedDate | null {
  ...
  // 1. 上午/下午/中午/早上/晚上 + 时间 (支持中文和阿拉伯数字)
  const periodTimeMatch = trimmed.match(/(上午|下午|中午|早上|晚上)([\d一二三四五六七八九十两兩]+)[点點时時:：]?([\d一二三四五六七八九十两兩]+)?[分]?/);
  if (periodTimeMatch) {
    console.log('[dateParser] 匹配到时间段:', periodTimeMatch);
    ...
    console.log('[dateParser] 原始小时:', hour, '分钟:', minute, '时间段:', period);
    ...
    console.log('[dateParser] 调整后小时:', hour, '分钟:', minute);
    hasTime = true;
  }
  // 2. 纯数字时间 (如果没有上午/下午等标识)
    ...
    if (pureTimeMatch) {
      console.log('[dateParser] 匹配到纯数字时间:', pureTimeMatch);
  ...
  if (hasTime) {
    ...
    console.log('[dateParser] 最终日期时间:', result.start.toLocaleString('zh-CN'));
  }
 ...
 */
export function formatDateDisplay(date: Date, hasTime?: boolean): string {
  const now = new Date();
