...
export function convertToCalendarEvent(
  ...
  if (isWidgetMode) {
    // 🆕 Widget模式：简化的前缀同步逻辑
    // Widget的策略：如果从localStorage读取的事件已经有前缀，就保持；如果没有前缀但检测到是运行中的timer，就添加前缀
    if (event.title.startsWith('[专注中]')) {
      displayTitle = event.title; // 保持已有前缀
      console.log('🔍 [WIDGET PREFIX] Event already has prefix, keeping it:', event.title);
    } else if (event.id && event.id.includes('timer-') && isCurrentlyRunningTimer) {
      // Widget检测到这是运行中的timer事件，主动添加前缀
      displayTitle = `[专注中] ${event.title}`;
      console.log('🔍 [WIDGET PREFIX] Adding prefix to running timer:', displayTitle);
    } else {
      displayTitle = event.title; // 其他情况保持原title
      console.log('🔍 [WIDGET PREFIX] No prefix needed for:', event.title);
    }
  } else {
