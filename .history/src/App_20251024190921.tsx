...
// 🚀 性能优化：懒加载大型组件，减少初始加载和页面切换时间
const FigmaTagManagerV3 = lazy(() => {
  console.log('⏳ [Lazy] Starting to load FigmaTagManagerV3...');
  const startTime = performance.now();
  return import('./components/FigmaTagManagerV3').then(module => {
    console.log(`✅ [Lazy] FigmaTagManagerV3 loaded in ${(performance.now() - startTime).toFixed(2)}ms`);
    return module;
  });
});

const TimeCalendar = lazy(() => {
  console.log('⏳ [Lazy] Starting to load TimeCalendar...');
  const startTime = performance.now();
  return import('./components/TimeCalendar').then(module => {
    console.log(`✅ [Lazy] TimeCalendar loaded in ${(performance.now() - startTime).toFixed(2)}ms`);
    return module;
  });
});
...
function App() {
  ...
  const renderCurrentPage = useMemo(() => {
    ...
    if (renderDuration > 100) {
      console.warn(`⚠️ [App] 页面 ${currentPage} 渲染耗时过长: ${renderDuration.toFixed(2)}ms`);
    }
