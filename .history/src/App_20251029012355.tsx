...
function App() {
  ...
  // 🔧 [NEW] 同步globalTimer状态到localStorage（供Widget读取）
  useEffect(() => {
    if (globalTimer) {
      // 将当前timer状态保存到localStorage，供Widget读取
      localStorage.setItem('remarkable-global-timer', JSON.stringify(globalTimer));
      console.log('💾 [Timer] Synced timer state to localStorage for Widget');
    } else {
      // 清除localStorage中的timer状态
      localStorage.removeItem('remarkable-global-timer');
      console.log('🗑️ [Timer] Cleared timer state from localStorage');
    }
  }, [globalTimer]);
