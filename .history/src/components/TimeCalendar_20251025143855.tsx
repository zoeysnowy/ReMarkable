...
import GoogleIcon from '../assets/icons/Google_Calendar.svg';
...
interface TimeCalendarProps {
  ...
  calendarProviderType?: 'outlook' | 'google' | 'icloud'; // 📅 日历提供商类型
...
export const TimeCalendar: React.FC<TimeCalendarProps> = ({
  ...
  const CalendarIcon = calendarProviderType ? calendarIcons[calendarProviderType] : null;
    ...
    styleElement.textContent = `
      ...
      .toastui-calendar-view-controls {
        ...
        background-color: ${getAdaptiveColors.isDark 
          ? 'rgba(255,255,255,0.28)' 
          : 'rgba(255,255,255,0.35)'} !important;
        border: 1px solid ${getAdaptiveColors.borderLight} !important;
      ...
      .toastui-calendar-panel-header {
        color: ${getAdaptiveColors.textPrimary} !important;
        font-weight: 500 !important;
      }
      
      /* 时间轴标签 - 首列的时间 (1am, 2am等) */
      .toastui-calendar-time,
      .toastui-calendar-time-date,
      .toastui-calendar-time-label,
      .toastui-calendar-timegrid-hourmarker-time,
      .toastui-calendar-week-view-time-column {
        color: ${getAdaptiveColors.textPrimary} !important;
      }
      ...
      .toastui-calendar-nav-button:hover {
        ...
        border-color: ${getAdaptiveColors.borderMedium} !important;
        color: ${getAdaptiveColors.textPrimary} !important;
      }
      ...
      .toastui-calendar-nav-button.active {
        ...
        border-color: ${getAdaptiveColors.accentColor} !important;
        font-weight: 600 !important;
    ...
    <>
      ...
      <div className={`time-calendar-container ${className}`} style={{ 
        ...
        <div style={{
          ...
          padding: '8px 12px',
          background: bgRgba,
          borderRadius: isWidgetMode ? '6px' : '4px', // 🎨 与 controller 一致的圆角
          border: isWidgetMode ? 'none' : `1px solid ${getAdaptiveColors.borderMedium}`,
          fontSize: '12px',
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            {lastSyncTime ? (
              <>
                � 最后同步: <strong style={{ color: getAdaptiveColors.textPrimary }}>
                ...
                <span style={{ color: getAdaptiveColors.textSecondary }}>
                  更新 {events.length} 个事件
                </span>
              </>
            ) : (
              <span style={{ color: getAdaptiveColors.textDisabled }}>未同步</span>
            )}
          </span>
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            {CalendarIcon ? (
              <img 
                src={CalendarIcon} 
                alt={`${calendarProviderType} calendar`}
                style={{ 
                  width: 16, 
                  height: 16,
                  filter: getAdaptiveColors.isDark ? 'brightness(1.2)' : 'none',
            ...
            <span 
              style={{ 
                ...
                height: '8px', 
                borderRadius: '50%',
                backgroundColor: microsoftService ? '#4ade80' : '#ef4444',
                boxShadow: microsoftService 
                  ? '0 0 8px rgba(74, 222, 128, 0.6)' 
                  : '0 0 8px rgba(239, 68, 68, 0.6)',
                transition: 'all 0.3s ease'
              }}
              title={microsoftService ? '已连接' : '未连接'}
            />
          </span>
        </div>
      </div>
