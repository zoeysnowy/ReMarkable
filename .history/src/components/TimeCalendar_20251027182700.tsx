...
export const TimeCalendar: React.FC<TimeCalendarProps> = ({
  ...
  // 🎨 设置面板初始高度（不使用 !important，允许用户拖动调整）
  useEffect(() => {
    if (!isCalendarReady) return;
    
    const timer = setTimeout(() => {
      // Task 面板
      if (calendarSettings.taskHeight) {
      ...
      // AllDay 面板
      if (calendarSettings.allDayHeight) {
      ...
      // Milestone 面板
      if (calendarSettings.milestoneHeight) {
    ...
    return () => clearTimeout(timer);
  }, [isCalendarReady, calendarSettings.taskHeight, calendarSettings.allDayHeight, calendarSettings.milestoneHeight, currentView]);
      ...
      mutations.forEach((mutation) => {
        if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
          ...
          if (target.classList.contains('toastui-calendar-task') || 
            ...
            if (!isNaN(newHeight) && newHeight !== calendarSettings.taskHeight) {
              console.log('📏 [拖动] Task高度变化:', newHeight);
              setCalendarSettings(prev => ({ ...prev, taskHeight: newHeight }));
            }
          } else if (target.classList.contains('toastui-calendar-panel-allday')) {
            const newHeight = parseInt(target.style.height);
            if (!isNaN(newHeight) && newHeight !== calendarSettings.allDayHeight) {
              console.log('📏 [拖动] AllDay高度变化:', newHeight);
              setCalendarSettings(prev => ({ ...prev, allDayHeight: newHeight }));
            }
          } else if (target.classList.contains('toastui-calendar-milestone') || 
                     target.classList.contains('toastui-calendar-panel-milestone')) {
            const newHeight = parseInt(target.style.height);
            if (!isNaN(newHeight) && newHeight !== calendarSettings.milestoneHeight) {
              console.log('📏 [拖动] Milestone高度变化:', newHeight);
              setCalendarSettings(prev => ({ ...prev, milestoneHeight: newHeight }));
            }
          }
        }
      });
  ...
  }, [isCalendarReady, currentView, calendarSettings.taskHeight, calendarSettings.allDayHeight, calendarSettings.milestoneHeight]);
