...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  // 同步到UnifiedTimeline
  const syncToUnifiedTimeline = async (item: PlanItem) => {
    ...
    // 🆕 如果 item 已有 eventId，从 TimeHub 读取最新时间（而不是使用 item 的旧时间字段）
    let finalStartTime = item.startTime || item.dueDate || new Date().toISOString();
    let finalEndTime = item.endTime || item.dueDate || new Date().toISOString();
    
    if (item.eventId) {
      try {
        const { TimeHub } = await import('../services/TimeHub');
        const snapshot = await TimeHub.getSnapshot(item.eventId);
        if (snapshot.start && snapshot.end) {
          ...
          console.log('%c[🔴 SYNC] 使用 TimeHub 的最新时间', 'color: green; font-size: 14px', {
            eventId: item.eventId,
            TimeHub时间: { start: snapshot.start, end: snapshot.end },
            item旧时间: { start: item.startTime, end: item.endTime }
          });
        }
      } catch (err) {
        console.warn('%c[🔴 SYNC] TimeHub 读取失败，使用 item 时间', 'color: orange; font-size: 14px', err);
      }
    }
