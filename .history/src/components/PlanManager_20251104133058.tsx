...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  // 将文本格式命令路由到当前编辑器（UnifiedSlateEditor - Slate only）
  const handleTextFormat = useCallback((command: string) => {
    ...
    switch (command) {
      ...
      case 'removeFormat':
        // 移除所有格式
        ['bold', 'italic', 'underline', 'strikethrough'].forEach(mark => {
          SlateEditor.removeMark(editor, mark);
        });
        break;
      default:
        console.warn(`[handleTextFormat] Unsupported command: ${command}`);
        break;
    }
  }, []);

  // 插入 Tag 到编辑器（支持 Slate 和 Tiptap）
  const insertTag = useCallback((editor: any, tagId: string, isDescriptionMode: boolean) => {
    const tag = TagService.getTagById(tagId);
    if (!tag) return false;

    const isSlateEditor = !(editor as any).chain;
    console.log('[insertTag] 检测编辑器类型:', { isSlateEditor, tagId, tagName: tag.name });
    ...
    if (isSlateEditor) {
      ...
      try {
        ...
        console.log('[insertTag] 准备插入 Slate tag 节点');
        ...
        console.log('[insertTag] Tag 节点:', tagNode);
        ...
        console.log('[insertTag] Tag 节点插入成功');
        
        // 插入空格
        Transforms.insertText(editor, ' ');
        console.log('[insertTag] 空格插入成功');
        
        return true;
      } catch (err) {
        console.error('[insertTag] Slate insertion failed:', err);
        return false;
      }
    } else {
      // Tiptap 编辑器：使用原有逻辑
      console.log('[insertTag] 使用 Tiptap API');
  ...
  // 插入 Emoji 到编辑器（支持 Slate 和 Tiptap）
  const insertEmoji = useCallback((editor: any, emoji: string) => {
    const isSlateEditor = !(editor as any).chain;
    console.log('[insertEmoji] 检测编辑器类型:', { isSlateEditor, emoji });
    ...
    if (isSlateEditor) {
      ...
      try {
        ...
        console.log('[insertEmoji] 准备插入 emoji 到 Slate');
        
        // 确保编辑器聚焦
        ReactEditor.focus(editor);
        
        Transforms.insertText(editor, emoji + ' ');
        console.log('[insertEmoji] Emoji 插入成功');
        return true;
      } catch (err) {
        console.error('[insertEmoji] Slate insertion failed:', err);
        return false;
      }
    } else {
      // Tiptap 编辑器：使用原有逻辑
  ...
  // 插入日期提及到编辑器（支持 Slate 和 Tiptap）
  const insertDateMention = useCallback((editor: any, startDate: Date, endDate?: Date, isDescriptionMode: boolean = false) => {
    const isSlateEditor = !(editor as any).chain;
    console.log('[insertDateMention] 检测编辑器类型:', { isSlateEditor, startDate, endDate });

    if (isSlateEditor) {
      // Slate 编辑器：插入 dateMention 元素
      try {
        const { Transforms } = require('slate');
        const { ReactEditor } = require('slate-react');
        
        console.log('[insertDateMention] 准备插入 Slate dateMention 节点');
        
        const dateMentionNode = {
          type: 'dateMention',
        ...
        console.log('[insertDateMention] DateMention 节点:', dateMentionNode);
        ...
        console.log('[insertDateMention] DateMention 节点插入成功');
        
        // 插入空格
        Transforms.insertText(editor, ' ');
        console.log('[insertDateMention] 空格插入成功');
        
        return true;
      } catch (err) {
        console.error('[insertDateMention] Slate insertion failed:', err);
        return false;
      }
    } else {
      // Tiptap 编辑器：使用原有逻辑
      console.log('[insertDateMention] 使用 Tiptap API');
      editor
        .chain()
        .focus()
        .insertContent({
  ...
  // 从编辑器获取 HTML（支持 Slate 和 Tiptap）
  const getEditorHTML = useCallback((editor: any): string => {
    const isSlateEditor = !(editor as any).chain;
    
    if (isSlateEditor) {
      // Slate 编辑器：需要序列化
      const { serializeToHtml } = require('./SlateEditor/utils');
      try {
        return serializeToHtml(editor.children);
      } catch (err) {
        console.error('[getEditorHTML] Slate serialization failed:', err);
        return '';
      }
    } else {
      // Tiptap 编辑器
    ...
    <div className="plan-manager">
      ...
      <div className="plan-list-scroll-container" ref={editorContainerRef}>
        <SlateFreeFormEditor
            ...
            onLineFocus={(lineId) => {
              console.log('[PlanManager] Line focused:', lineId);
              // 更新当前聚焦的行 ID
              setCurrentFocusedLineId(lineId);
            ...
            onEditorReady={(lineId, editor) => {
              // console.log('[PlanManager] Editor ready:', lineId);
              editorRegistryRef.current.set(lineId, editor);
            }}
            onEditorDestroy={(lineId) => {
              // console.log('[PlanManager] Editor destroy:', lineId);
      ...
      <HeadlessFloatingToolbar
        ...
        onTimeApplied={(startIso, endIso) => {
          ...
          const insertSuccess = insertDateMention(editor, startDate, endDate, isDescriptionMode);
          
          if (!insertSuccess) {
            warn('picker', '⚠️ onTimeApplied: 插入日期提及失败');
            return;
          }
          
          dbg('picker', '✅ onTimeApplied: 日期提及插入成功');
        ...
        onTagSelect={(tagIds: string[]) => {
          console.log('[onTagSelect] 被调用', { tagIds, currentFocusedLineId });
          
          // 计算新增标签（与上一次所选差集）
          const addedIds = tagIds.filter(id => !currentSelectedTagsRef.current.includes(id));
          console.log('[onTagSelect] 新增标签:', addedIds);

          // ⚠️ 注意：不要在这里提前更新状态，等插入成功后再更新

          // 仅插入最新新增的那一个，避免多次插入造成重复
          if (!currentFocusedLineId || addedIds.length === 0) {
            console.log('[onTagSelect] 跳过：没有 focusedLineId 或没有新增标签');
            // 即使没有新增，也要更新UI状态（可能是取消选择）
            currentSelectedTagsRef.current = tagIds;
            setCurrentSelectedTags(tagIds);
            return;
          }
          ...
          const item = items.find(i => i.id === actualItemId);
          if (!item) {
            console.log('[onTagSelect] 跳过：找不到 item');
            return;
          }

          const editor = editorRegistryRef.current.get(currentFocusedLineId);
          if (!editor) {
            console.log('[onTagSelect] 跳过：找不到 editor');
            return;
          }
          
          console.log('[onTagSelect] 找到编辑器，准备插入标签', { insertId });

          const isDescriptionMode = currentFocusedMode === 'description';

          // 使用统一的插入函数（支持 Slate 和 Tiptap）
          const success = insertTag(editor, insertId, isDescriptionMode);
          if (!success) {
            console.log('[onTagSelect] 插入失败');
            return;
          }
          
          console.log('[onTagSelect] 插入成功，准备保存');

          // 保存（Title: 提取标签并更新元数据；Description: 仅更新 description HTML）
          const updatedContent = getEditorHTML(editor);
          console.log('[onTagSelect] 获取到内容:', updatedContent.substring(0, 100));
          ...
          if (isDescriptionMode) {
            ...
            // ✅ 同步更新 currentSelectedTags（解决 FloatingBar 勾选状态不同步问题）
            // 将 tagNames 转换为 tagIds
            const updatedTagIds = extractedTags
              .map(tagName => {
                const tag = TagService.getFlatTags().find(t => t.name === tagName);
                return tag?.id;
              })
              .filter(Boolean) as string[];
            currentSelectedTagsRef.current = updatedTagIds;
            setCurrentSelectedTags(updatedTagIds);
            console.log('[onTagSelect] 同步 currentSelectedTags:', updatedTagIds);
          }
