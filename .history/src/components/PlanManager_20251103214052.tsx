...
const PlanItemTimeDisplay: React.FC<{
  ...
  // 观察订阅的时间变化，输出调试日志
  useEffect(() => {
    dbg('ui', 'PlanItemTimeDisplay snapshot update', {
      eventId: item.eventId,
      start: eventTime.start,
      end: eventTime.end,
      allDay: eventTime.timeSpec?.allDay,
    });
  }, [item.eventId, eventTime.start, eventTime.end, eventTime.timeSpec?.allDay]);
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 0, fontSize: '14px', justifyContent: 'flex-end' }}>
        ...
        <PlanItemTimeDisplay
          item={item}
          onEditClick={(anchor) => {
            dbg('ui', 'Open UnifiedDateTimePicker from suffix click', { eventId: item.eventId, itemId: item.id });
            dateAnchorRef.current = anchor;
    ...
    <div className="plan-manager">
      ...
      <HeadlessFloatingToolbar
        ...
        onTimeApplied={(startIso, endIso) => {
          dbg('picker', 'HeadlessFloatingToolbar onTimeApplied (TimeHub updated)', { startIso, endIso, focusedLineId: currentFocusedLineId });
            ...
            (async () => {
              try {
                if (updatedItem.eventId) {
                  ...
                  dbg('picker', 'Updated existing event (non-time fields) after picker apply', { eventId: updatedItem.eventId });
                } else if (startIso) {
                  ...
                  if (createRes.success && createRes.event) {
                    ...
                    dbg('picker', 'Created new eventId after picker apply', { eventId: newId });
                  }
                }
              } catch {}
            })();
        ...
        onDateRangeSelect={(start: Date, end: Date) => {
          // 统一日期时间选择器（无 eventId 的降级路径）：不再往标题插入 📅 mention
          // 直接更新当前项的时间字段，保持内容不变；如果没有 eventId，则创建事件并回写
          if (!currentFocusedLineId) return;
          const actualItemId = currentFocusedLineId.replace('-desc', '');
          const item = items.find(i => i.id === actualItemId);
          const editor = editorRegistryRef.current.get(currentFocusedLineId);
          ...
          // 保存当前编辑内容（不插入任何日期mention），并写入时间字段
          const updatedItem: PlanItem = {
            ...item,
            ...(isDescriptionMode
              ? { description: updatedHTML }
          ...
          dbg('picker', 'onDateRangeSelect (no eventId path): update item times only', { itemId: item.id, startIso: updatedItem.startTime, endIso: updatedItem.endTime });
          onSave(updatedItem);
          syncToUnifiedTimeline(updatedItem);

          // 统一到 Event：若已有 eventId 更新；没有则创建
          (async () => {
            try {
              if (updatedItem.eventId) {
                await EventService.updateEvent(updatedItem.eventId, {
                ...
                dbg('picker', 'Updated existing event from onDateRangeSelect', { eventId: updatedItem.eventId });
              } else {
                const newId = generateEventId();
                const createRes = await EventService.createEvent({
                  id: newId,
                ...
                if (createRes.success && createRes.event) {
                  ...
                  onSave(withEvent);
                  syncToUnifiedTimeline(withEvent);
                  dbg('picker', 'Created new event from onDateRangeSelect', { eventId: newId });
                }
              }
            } catch {}
          })();
