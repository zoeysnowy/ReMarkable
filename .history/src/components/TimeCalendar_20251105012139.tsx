...
export const TimeCalendar: React.FC<TimeCalendarProps> = ({
  ...
  const handleSaveEventFromModal = async (updatedEvent: Event) => {
    ...
    try {
      ...
      // 检查是否是新事件
      const existingSnapshot = EventHub.getSnapshot(updatedEvent.id);
      const isNewEvent = !existingSnapshot;
      
      let result;
      if (isNewEvent) {
        // 新事件：直接创建
        console.log('➕ [TimeCalendar] Creating new event via EventHub:', updatedEvent.id);
        result = await EventHub.createEvent(updatedEvent);
      } else {
        // 现有事件：计算需要更新的字段
        console.log('✏️ [TimeCalendar] Updating existing event via EventHub:', updatedEvent.id);
        
        const updates: Partial<Event> = {};
        
        // 比较并收集变化的字段
        if (updatedEvent.title !== existingSnapshot.title) updates.title = updatedEvent.title;
        if (updatedEvent.description !== existingSnapshot.description) updates.description = updatedEvent.description;
        if (updatedEvent.location !== existingSnapshot.location) updates.location = updatedEvent.location;
        if (updatedEvent.isAllDay !== existingSnapshot.isAllDay) updates.isAllDay = updatedEvent.isAllDay;
        if (updatedEvent.startTime !== existingSnapshot.startTime) updates.startTime = updatedEvent.startTime;
        if (updatedEvent.endTime !== existingSnapshot.endTime) updates.endTime = updatedEvent.endTime;
        if (JSON.stringify(updatedEvent.tags) !== JSON.stringify(existingSnapshot.tags)) updates.tags = updatedEvent.tags;
        if (updatedEvent.tagId !== existingSnapshot.tagId) updates.tagId = updatedEvent.tagId;
        if (JSON.stringify(updatedEvent.calendarIds) !== JSON.stringify(existingSnapshot.calendarIds)) updates.calendarIds = updatedEvent.calendarIds;
        if (updatedEvent.calendarId !== existingSnapshot.calendarId) updates.calendarId = updatedEvent.calendarId;
        
        // 增量更新
        result = await EventHub.updateFields(updatedEvent.id, updates, { source: 'TimeCalendar' });
      }
      
      if (!result.success) {
        console.error('❌ [TimeCalendar] Save failed:', result.error);
        return;
      }
      
      const finalEvent = result.event!;
      
      // 🎨 立即更新 UI - 从 localStorage 重新加载所有事件
      const saved = localStorage.getItem(STORAGE_KEYS.EVENTS);
