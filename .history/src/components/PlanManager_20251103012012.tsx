        ...
        {(item.dueDate || item.startTime) && (() => {
          ...
          if (item.startTime && item.endTime) {
            ...
            const InlineDurationArrow: React.FC<{ text: string }> = ({ text }) => {
              ...
              useLayoutEffect(() => {
                if (spanRef.current) {
                  setW(Math.ceil(spanRef.current.offsetWidth));
                }
              }, [text]);
              // 稍微放宽箭头宽度，并在文字与箭头之间保留最小间距
              const width = Math.max(10, w + 8);
              return (
                <div className="preview-arrow-section" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', margin: '0 6px' }}>
                  <span
                    ref={spanRef}
