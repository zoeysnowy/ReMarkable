    ...
    if (!globalTimer) { // 只在 Widget 场景（没有 prop）时轮询
      ...
      const checkTimer = () => {
        ...
        if (eventsData !== lastEventsStateRef.current) {
          console.log('🔍 [TIMER] Events data changed, checking for timer updates');
    ...
    try {
      ...
      if (eventsData) {
        ...
        if (todayTimerEvents.length > 0) {
          ...
          // 检查最新timer是否可能还在运行（结束时间在未来或很接近现在）
          const timeDiff = endTime.getTime() - now.getTime();
          const isLikelyRunning = timeDiff > -120000; // 2分钟容差
        ...
        console.log('⚠️ [TIMER] No recent running timer events found');
      }
    } catch (error) {
