...
export const TimeCalendar: React.FC<TimeCalendarProps> = ({
  ...
  // 🔄 初始加载时检查 localStorage（Widget 场景）
  useEffect(() => {
    if (!globalTimer) {
      console.log('🔍 [TIMER] Initial check for localStorage timer');
      const saved = localStorage.getItem('remarkable-global-timer');
      if (saved) {
        console.log('✅ [TIMER] Found timer in localStorage on mount');
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      ...
      if (e.key === 'remarkable-global-timer') {
        console.log('🔄 [TIMER] Timer storage changed, triggering recalculation');
        // 触发重新计算 runningTimerEventId
        setLocalStorageTimerTrigger(prev => prev + 1);
      }
    };
  ...
  // 🔧 计算当前运行中的 Timer 事件 ID
  // 优先使用 prop，如果没有则从 localStorage 读取（支持 Widget 场景）
  const runningTimerEventId = useMemo(() => {
    console.log('🔍 [TIMER] Recalculating runningTimerEventId', {
      hasGlobalTimerProp: !!globalTimer,
    ...
    try {
      const saved = localStorage.getItem('remarkable-global-timer');
      console.log('🔍 [TIMER] localStorage content:', saved);
      ...
      if (saved) {
        ...
        if (timer && timer.isRunning) {
          ...
          console.log('⚠️ [TIMER] Timer exists but not running');
        }
      } else {
    ...
    console.log('⚠️ [TIMER] No running timer found');
    return null;
  }, [globalTimer, localStorageTimerTrigger]); // 依赖 prop 和 localStorage 触发器
  ...
  const calendarEvents = useMemo(() => {
    ...
    const calendarEventsWithStats = uniqueFiltered
      .map(event => {
        const calendarEvent = convertToCalendarEvent(event, hierarchicalTags, runningTimerEventId);
  ...
  }, [events, hierarchicalTags, visibleTags, visibleCalendars, eventOpacity, currentDate, runningTimerEventId]);
  // 📅 创建日历分组配置
