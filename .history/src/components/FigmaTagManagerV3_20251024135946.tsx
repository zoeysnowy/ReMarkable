...
const FigmaTagManagerV3: React.FC<FigmaTagManagerProps> = ({
  ...
  useEffect(() => {
    const handlePaste = (e: ClipboardEvent) => {
      ...
      // 解析标签格式：# emoji 名称 (可能有前导空格表示层级)
      const lines = pastedText.split('\n').filter(line => line.trim());
      const tagPattern = /^(\s*)#\s*([^\s]*)\s+(.+)$/;
      ...
      for (const line of lines) {
        ...
        if (match) {
          const [, spaces, emoji, name] = match;
          const level = Math.floor(spaces.length / 2); // 每2个空格 = 1级缩进
          parsedTags.push({
            name: name.trim(),
      ...
      if (parsedTags.length > 0) {
        console.log('📋 [Paste] Detected tags to import:', parsedTags);
        e.preventDefault();
          ...
          parsedTags.forEach((parsedTag, index) => {
            ...
            if (parsedTag.level > 0) {
              for (let i = newTags.length - 1; i >= 0; i--) {
                if ((newTags[i].level || 0) < parsedTag.level) {
                  parentId = newTags[i].id;
                  break;
                }
              }
            }
