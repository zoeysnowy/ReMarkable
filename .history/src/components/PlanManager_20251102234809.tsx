      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '14px', justifyContent: 'flex-end' }}>
        ...
        {(item.dueDate || item.startTime) && (() => {
          ...
          if (item.startTime && item.endTime) {
            const { parseLocalTimeString, formatDisplayTime, getTimeDifferenceInSeconds, formatDuration } = require('../utils/timeUtils');
            ...
            return (
              <span style={{ color: '#6b7280', whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                {monthStart}月{dayStart}日 {formatDisplayTime(start)}
                ...
                {sameDay ? (
                  ...
                  <>
                    {monthEnd}月{dayEnd}日 {formatDisplayTime(end)}
                  </>
                )}
                <span style={{ opacity: 0.6, marginLeft: 6 }}>· {formatDuration(durationSec)}</span>
              </span>
            );
          }
          return null;
