      ...
      {editor && mode === 'description' && (
        <BubbleMenu editor={editor} tippyOptions={{ duration: 100 }}>
          <div style={{ display: 'flex', gap: 6, padding: '6px 8px', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, boxShadow: '0 4px 14px rgba(0,0,0,0.08)' }}>
            ...
            {/* 高亮 */}
            <button onMouseDown={(e) => e.preventDefault()} onClick={() => editor.chain().focus().toggleHighlight({ color: '#fde68a' }).run()} style={{ background: '#fde68a' }}>HL</button>
            <button onMouseDown={(e) => e.preventDefault()} onClick={() => editor.chain().focus().toggleHighlight({ color: '#a7f3d0' }).run()} style={{ background: '#a7f3d0' }}>HL</button>
          </div>
        </BubbleMenu>
      )}
