...
interface TiptapLineProps {
  ...
  onShiftEnter?: (lineId: string) => void;
  onTab?: (lineId: string) => void;
  onShiftTab?: (lineId: string) => void;
  onArrowUp?: (lineId: string) => void;
  onArrowDown?: (lineId: string) => void;
  onFocus?: () => void;
  onBlur?: () => void;
  className?: string;
}
      ...
      handleKeyDown: (view, event) => {
        ...
        if (event.key === 'Enter' && !event.shiftKey) {
          ...
          if (mode === 'title') {
            ...
            console.log('[TiptapLine] Calling onEnter with lineId:', lineId);
            onEnter?.(lineId);
            return true;
          }
        ...
        // Shift+Enter: 模式切换
        if (event.key === 'Enter' && event.shiftKey) {
          console.log('[TiptapLine] Shift+Enter pressed');
          event.preventDefault();
          console.log('[TiptapLine] Calling onShiftEnter with lineId:', lineId);
          onShiftEnter?.(lineId);
          return true;
        }
