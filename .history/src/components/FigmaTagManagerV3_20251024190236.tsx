...
const FigmaTagManagerV3: React.FC<FigmaTagManagerProps> = ({
  ...
  useEffect(() => {
    ...
    const handleCopy = (e: ClipboardEvent) => {
      ...
      // 同时写入两种格式到剪贴板
      e.clipboardData?.setData('text/plain', textFormat);
      ...
      console.log('📋 [Copy] Copied to clipboard:', {
        ...
        selectedTags: selectedTags.map(t => ({ name: t.name, color: t.color, level: t.level }))
      });
    };
    ...
    const handlePaste = (e: ClipboardEvent) => {
      ...
      // 🔧 优先尝试读取JSON格式（包含完整的层级和颜色信息）
      const jsonData = e.clipboardData?.getData('application/json');
      console.log('📋 [Paste] JSON data from clipboard:', jsonData);
      
      if (jsonData) {
        try {
          const parsedData = JSON.parse(jsonData);
          console.log('📋 [Paste] Parsed JSON successfully:', parsedData);
