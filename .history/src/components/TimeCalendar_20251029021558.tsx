...
export const TimeCalendar: React.FC<TimeCalendarProps> = ({
  ...
  const getRunningTimerEventId = () => {
    ...
    // 3. 🆕 Widget回退方案：从remarkable-events中识别可能正在运行的timer
    // 🔧 重要：只有在Widget场景下才使用此回退方案
    console.log('🔍 [TIMER] Fallback: checking recent timer events...');
    try {
      const eventsData = localStorage.getItem('remarkable-events');
      if (eventsData) {
        ...
        // 查找今天的timer事件，按开始时间排序
        const todayTimerEvents = events
          .filter((e: any) => e.isTimer && new Date(e.startTime).toDateString() === now.toDateString())
          .sort((a: any, b: any) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime());
        
        if (todayTimerEvents.length > 0) {
          const latestTimer = todayTimerEvents[0];
          const endTime = new Date(latestTimer.endTime);
          
          // 🔧 修复：使用更严格的判断 - 结束时间必须在未来
          const timeDiff = endTime.getTime() - now.getTime();
          const isActuallyRunning = timeDiff > 0; // 必须在未来，不允许容差
          
          console.log('🔍 [TIMER] Latest timer event analysis:', {
            id: latestTimer.id,
            title: latestTimer.title,
          ...
          if (isActuallyRunning) {
            console.log('✅ [TIMER] Using latest timer event as running timer:', latestTimer.id);
            return latestTimer.id;
          } else {
            console.log('⚠️ [TIMER] Latest timer has ended, not considered running');
          }
        }
        
        console.log('⚠️ [TIMER] No active running timer events found');
      }
    } catch (error) {
      console.error('❌ [TIMER] Failed to analyze timer events:', error);
    }
  ...
  const generateRealtimeTimerEvent = () => {
    ...
    const realtimeEvent: Event = {
      id: timerEventId,
      title: currentTimer.taskTitle || currentTimer.eventTitle || '计时中...',
