...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  const floatingToolbar = useFloatingToolbar({
    ...
    onMenuSelect: (menuIndex: number) => {
      // 设置活动的 picker 索引，保持打开状态
      // 不要自动重置，让用户手动关闭或选择其他 picker
      setActivePickerIndex(menuIndex);
    },
  });

  // 🆕 当 FloatingBar 隐藏时，重置 activePickerIndex
  useEffect(() => {
    if (!floatingToolbar.position.show) {
      setActivePickerIndex(null);
    }
  }, [floatingToolbar.position.show]);

  // 将文本格式命令路由到 UnifiedSlateEditor
  ...
  const renderLinePrefixAdapter = useCallback((node: { eventId?: string; lineId: string; level: number; mode: 'title' | 'description' }) => {
    // 通过 lineId 查找对应的 item（去掉可能的 -desc 后缀）
    const itemId = node.lineId.replace('-desc', '');
    ...
    <div className="plan-manager">
        ...
        <UnifiedSlateEditor
            ...
            onChange={(updatedItems) => {
              ...
              updatedItems.forEach(updatedItem => {
                ...
                // 🆕 同步到 TimeCalendar：
                // 1. 如果有 eventId，说明已经是 Event，需要更新
                // 2. 如果有时间信息（startTime/endTime/dueDate），需要创建或更新 Event
                const hasTimeInfo = updatedItem.startTime || updatedItem.endTime || updatedItem.dueDate;
                
                if (updatedItem.eventId || hasTimeInfo) {
                  syncToUnifiedTimeline(updatedItem);
                }
              });
            }}
            onFocus={(lineId) => {
              // 更新当前聚焦的行 ID（用于 FloatingBar 插入定位）
              const actualItemId = lineId.replace('-desc', '');
              const item = items.find(i => i.id === actualItemId);
              if (item) {
                setCurrentFocusedLineId(lineId);
                setCurrentFocusedMode(lineId.endsWith('-desc') ? 'description' : 'title');
            ...
            renderLinePrefix={renderLinePrefixAdapter}
            renderLineSuffix={renderLineSuffixAdapter}
      ...
      <HeadlessFloatingToolbar
        ...
        onTagSelect={(tagIds: string[]) => {
          ...
          // 🆕 不自动关闭 FloatingBar，让用户可以继续选择其他标签
          // 用户可以按 Escape 或点击外部关闭
        }}
        onEmojiSelect={(emoji: string) => {
          ...
          // 使用 insertEmoji helper 插入 emoji
