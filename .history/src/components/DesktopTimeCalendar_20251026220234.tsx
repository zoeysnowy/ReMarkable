...
import './DesktopTimeCalendar.css'; // 🎨 Desktop Widget 专用样式
...
const DesktopTimeCalendar: React.FC<DesktopTimeCalendarProps> = ({
  ...
  // 🎨 强制移除 Toast UI Calendar 内部设置的背景色
  useEffect(() => {
    const removeInlineBackgroundColor = () => {
      // 查找所有 Toast UI Calendar 的 layout 容器
      const layouts = document.querySelectorAll('.desktop-calendar-main-body .toastui-calendar-layout');
      layouts.forEach(layout => {
        if (layout instanceof HTMLElement) {
          // 移除内联样式中的 background-color
          layout.style.backgroundColor = 'transparent';
        }
      });
      
      // 同时处理其他可能有背景色的容器
      const panels = document.querySelectorAll('.desktop-calendar-main-body .toastui-calendar-panel');
    ...
    // 使用 MutationObserver 监听 DOM 变化，Toast UI Calendar 可能会动态修改样式
    const observer = new MutationObserver(removeInlineBackgroundColor);
    const targetNode = document.querySelector('.desktop-calendar-main-body');
  ...
  return (
    ...
    >
      ...
      {/* 日历主体 */}
      <div 
        className={`desktop-calendar-main-body ${adaptiveColors.isDark ? 'dark-mode' : 'light-mode'}`}
        style={{ 
          flex: 1, 
          overflow: 'hidden',
          position: 'relative',
          backgroundColor: settings.backgroundColor, // 🔧 修复：跟随设置的背景色
          borderRadius: '20px', // 🎨 圆角边框设计
          border: `2px solid ${adaptiveColors.borderColorMedium}`, // 🎨 自适应边框
          boxShadow: adaptiveColors.isDark 
            ? '0 4px 12px rgba(0,0,0,0.3)' 
            : '0 2px 8px rgba(0,0,0,0.1)' // 🎨 添加阴影增强立体感
        }}
