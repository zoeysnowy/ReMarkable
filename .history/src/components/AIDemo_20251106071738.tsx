...
export const AIDemo: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (window.electron) {
      // 监听代理状态
      window.electron.on('ai-proxy-status', (status: any) => {
        setProxyStatus(status.running ? 'running' : 'stopped');
      });
      
      // 监听代理日志
      window.electron.on('ai-proxy-log', (log: any) => {
    ...
    return () => {
      if (window.electron) {
        window.electron.removeAllListeners?.('ai-proxy-status');
        window.electron.removeAllListeners?.('ai-proxy-log');
      }
    };
  }, []);
  ...
  const checkProxyStatus = async () => {
    ...
    if (window.electron?.invoke) {
      try {
        const result = await window.electron.invoke('check-ai-proxy-status');
        if (result.running) {
  ...
  const handleStartProxy = async () => {
    ...
    try {
      const result = await window.electron.invoke('start-ai-proxy');
