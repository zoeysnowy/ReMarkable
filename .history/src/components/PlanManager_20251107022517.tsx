...
// 🔍 调试开关 - 通过 window.SLATE_DEBUG = true 开启，自动持久化到 localStorage
const isDebugEnabled = () => {
  if (typeof window === 'undefined') return false;
  
  // 检查 window.SLATE_DEBUG
  if ((window as any).SLATE_DEBUG === true) {
    try {
...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  useEffect(() => {
    ...
    if (isDebugEnabled()) {
      ...
      window.console.log(`%c[🚀 ${timestamp}] PlanManager - 调试模式已开启`, 
