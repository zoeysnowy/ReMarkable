    ...
    <div className="plan-manager">
      ...
      <HeadlessFloatingToolbar
        ...
        onTagSelect={(tagIds: string[]) => {
          ...
          tagSpan.setAttribute('data-tag-id', insertId);
          ...
          const selection = window.getSelection();
          if (selection) {
            // 如果当前选区不在该编辑器内，强制将光标移动到末尾
            const anchorNode = selection.anchorNode as Node | null;
            const selectionInside = anchorNode && editableElement.contains(anchorNode);
            ...
            // 使用最新 range 进行插入
            const useRange = selection.getRangeAt(0);
            useRange.deleteContents();
            useRange.insertNode(tagSpan);

            // 在标签后添加一个空格，方便继续输入
            const space = document.createTextNode(' ');
            useRange.collapse(false);
            useRange.insertNode(space);

            // 光标移动到空格后
            useRange.setStartAfter(space);
            useRange.setEndAfter(space);
            selection.removeAllRanges();
            selection.addRange(useRange);
          }
        ...
        onEmojiSelect={(emoji: string) => {
          ...
          if (!item || !editableElement || !editableElement.isContentEditable) return;
