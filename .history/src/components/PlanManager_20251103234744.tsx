...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  const syncToUnifiedTimeline = (item: PlanItem) => {
    ...
    if (type === 'todo') {
      console.log('%c[🔴 SYNC] 跳过: type=todo', 'color: orange; font-size: 14px');
      return;
    }
    
    // 🆕 确定最终时间（根据 type 和时间缺省情况）
    ...
    if (item.eventId) {
      ...
      if (snapshot.start && snapshot.end) {
        ...
        // TimeHub 无数据，使用 item 字段
        finalStartTime = item.startTime || item.dueDate || item.createdAt || new Date().toISOString();
        finalEndTime = item.endTime || item.dueDate || item.createdAt || new Date().toISOString();
      ...
      if (type === 'task') {
        ...
        if (item.startTime && !item.endTime) {
          ...
          // 完全没有时间 → task 日期 = 创建日期
          const createdDate = item.createdAt || new Date().toISOString();
          finalStartTime = createdDate;
          finalEndTime = createdDate;
          console.log('%c[🔴 SYNC] 📋 Task: 无约定时间，使用创建日期', 'color: blue; font-size: 14px', { date: createdDate });
        } else {
          // 两者都有（理论上不会到这里，因为会被判断为 event）
          finalStartTime = item.startTime!;
          finalEndTime = item.endTime!;
        }
      } else {
        // Event 类型：使用原有逻辑
        finalStartTime = item.startTime || item.dueDate || new Date().toISOString();
        finalEndTime = item.endTime || item.dueDate || new Date().toISOString();
      }
    }
    ...
    const event: Event = {
      ...
      startTime: finalStartTime,  // 🆕 使用 TimeHub 的最新时间
      endTime: finalEndTime,      // 🆕 使用 TimeHub 的最新时间
      // 全天：显式勾选优先；否则当起止为同一天且均为 00:00 视为全天
      isAllDay: (() => {
        if (item.isAllDay) return true;
        if (finalStartTime && finalEndTime) {  // 🆕 使用 finalStartTime/finalEndTime
