      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 0, fontSize: '14px', justifyContent: 'flex-end' }}>
        ...
        {(item.dueDate || item.startTime) && (() => {
          ...
          if (item.startTime && item.endTime) {
            ...
            // 紧凑布局：移除所有间距
            return (
              <div className="preview-time-display" style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
                ...
                <span className="preview-end-time" style={{ fontSize: 14, fontWeight: 500, color: '#374151', margin: 0, padding: 0 }}>{endTimeStr}</span>
              </div>
            );
          }
          return null;
            ...
            <div style={{ padding: 0 }}>
              <UnifiedDateTimePicker
                onSelect={(start, end) => {
                  ...
                  const item = items.find(i => i.id === currentFocusedLineId.replace('-desc',''));
                  const editableElement = document.querySelector(
                    `[data-line-id="${currentFocusedLineId}"] .ProseMirror`
                  ) as HTMLElement | null;
                  const isDescriptionMode = currentFocusedMode === 'description';

                  // 不插入文本，仅更新元数据的起止时间，并保存当前编辑的HTML
                  if (item) {
                    const updatedItem: PlanItem = {
                      ...item,
                      startTime: start ? new Date(start).toISOString() : item.startTime,
                      endTime: end ? new Date(end).toISOString() : (start ? new Date(start).toISOString() : item.endTime),
                      ...(isDescriptionMode
