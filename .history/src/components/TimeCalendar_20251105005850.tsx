  ...
  const handleSaveEventFromModal = async (updatedEvent: Event) => {
    ...
    try {
      ...
      if (eventIndex === -1) {
        ...
        // 🔒 Bug Fix: 合并更新而不是覆盖，保留未修改的字段（如 description）
        existingEvents[eventIndex] = {
          ...originalEvent,
          ...updatedEvent,
          // 特别保护 description：如果更新的 description 为空但原有的不为空，保留原有的
          description: updatedEvent.description || originalEvent.description || '',
        };
      }
      localStorage.setItem(STORAGE_KEYS.EVENTS, JSON.stringify(existingEvents));
