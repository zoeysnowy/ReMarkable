...
export const EventEditModal: React.FC<EventEditModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (showTagDropdown || showCalendarDropdown) {
  ...
  }, [showTagDropdown, showCalendarDropdown]);
  ...
  const filteredCalendars = useMemo(() => {
    if (!calendarSearchQuery.trim()) return availableCalendars;
  ...
  }, [availableCalendars, calendarSearchQuery]);
  ...
  useEffect(() => {
    if (formData.tags.length > 0) {
      ...
      if (firstTag && firstTag.calendarMapping && firstTag.calendarMapping.calendarId) {
        const mappedCalendarId = firstTag.calendarMapping.calendarId;
        
        // 只有当前calendarId为空或者与映射的不同时才更新
        if (!formData.calendarId || formData.calendarId !== mappedCalendarId) {
          console.log('🗓️ [EventEditModal] Auto-mapping calendar from tag:', {
            tagId: firstTag.id,
            tagName: firstTag.name,
            mappedCalendarId,
            calendarName: firstTag.calendarMapping.calendarName
          });
  ...
  }, [formData.tags]); // 只监听tags变化，避免循环依赖
  ...
  const handleSave = () => {
    ...
    const updatedEvent: Event = {
      ...
      calendarId: formData.calendarId || undefined, // 日历分组
          ...
          <div className="form-group">
            ...
            <div className="calendar-selector" ref={calendarSelectorRef}>
              ...
              <div className="selected-calendar">
                {formData.calendarId ? (() => {
                  const calendar = availableCalendars.find(cal => cal.id === formData.calendarId);
                  return calendar ? (
                    <span className="calendar-chip" style={{ backgroundColor: calendar.color || '#3498db' }}>
                      📅 {calendar.name || calendar.displayName}
                      <button onClick={() => setFormData({ ...formData, calendarId: '' })}>✕</button>
                    </span>
                  ) : (
                    <span className="calendar-chip-invalid">
                      ⚠️ 未知日历 ({formData.calendarId})
                      <button onClick={() => setFormData({ ...formData, calendarId: '' })}>✕</button>
                    </span>
                  );
                })() : (
                  <span className="placeholder">选择日历分组...</span>
                )}
              </div>
              ...
              {showCalendarDropdown && (
                <div className="calendar-dropdown">
                  ...
                  <div className="calendar-dropdown-list">
                    {/* 清空选择选项 */}
                    <div
                      className={`calendar-option ${!formData.calendarId ? 'selected' : ''}`}
                      onClick={() => {
                        setFormData({ ...formData, calendarId: '' });
                        setShowCalendarDropdown(false);
                      }}
                    >
                      <input
                        type="radio"
                        checked={!formData.calendarId}
                        readOnly
                      />
                      <span className="calendar-icon">🚫</span>
                      <span className="calendar-name">不指定日历分组</span>
                    </div>
                    ...
                    {filteredCalendars.length > 0 ? (
                      filteredCalendars.map(calendar => (
                        <div
                          key={calendar.id}
                          className={`calendar-option ${formData.calendarId === calendar.id ? 'selected' : ''}`}
                          onClick={() => {
                            setFormData({ ...formData, calendarId: calendar.id });
                            setShowCalendarDropdown(false);
                          }}
                        >
                          <input
                            type="radio"
                            checked={formData.calendarId === calendar.id}
                            readOnly
                          />
                          <span className="calendar-color" style={{ backgroundColor: calendar.color || '#3498db' }}></span>
                          <span className="calendar-icon">📅</span>
                          <span className="calendar-name">{calendar.name || calendar.displayName}</span>
