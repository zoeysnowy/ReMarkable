...
const FigmaTagManagerV3: React.FC<FigmaTagManagerProps> = ({
  ...
  useEffect(() => {
    ...
    const handlePaste = (e: ClipboardEvent) => {
      // 只在非输入区域处理粘贴（避免干扰contentEditable）
      const target = e.target as HTMLElement;
      if (target.contentEditable === 'true' || target.tagName === 'INPUT' || target.tagName === 'TEXTAREA') {
        console.log('📋 [Paste] Skipping - target is editable:', target.tagName);
        return;
      }

      console.log('📋 [Paste] Paste event triggered on:', target.className);
      
      // 🔧 Step 1: 优先尝试从文本中提取 Base64 编码的 JSON（主要方案）
      ...
      console.log('📋 [Paste] Text starts with marker?', pastedText?.startsWith('__REMARKABLE_TAGS_JSON__'));
