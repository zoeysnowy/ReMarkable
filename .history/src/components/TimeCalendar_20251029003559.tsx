...
export const TimeCalendar: React.FC<TimeCalendarProps> = ({
  ...
  const generateRealtimeTimerEvent = () => {
    // 获取当前timer状态
    const currentTimer = globalTimer || (() => {
      try {
        const saved = localStorage.getItem('remarkable-global-timer');
    ...
    const timerEventId = `timer-${currentTimer.tagId}-${currentTimer.originalStartTime || currentTimer.startTime}`;
    
    // 查找已保存的timer事件（最新的一个）
    const savedTimerEvents = events.filter(e => e.id === timerEventId);
    const latestSavedEvent = savedTimerEvents.length > 0 ? savedTimerEvents[0] : null;

    // 计算当前时间
    const now = new Date();
    const timerStartTime = new Date(currentTimer.originalStartTime || currentTimer.startTime);
    
    // 如果有已保存的事件，从其结束时间开始；否则从timer开始时间开始
    const eventStartTime = latestSavedEvent 
      ? new Date(latestSavedEvent.endTime)
      : timerStartTime;

    // 🔧 确保计算正确：实时事件应该显示完整的timer时长
    const realtimeEvent: Event = {
      id: timerEventId,
      title: currentTimer.currentTask || '计时中...',
      startTime: formatTimeForStorage(timerStartTime), // 使用timer的真实开始时间
      endTime: formatTimeForStorage(now), // 使用当前时间作为结束时间
      location: '',
      description: '实时计时事件',
