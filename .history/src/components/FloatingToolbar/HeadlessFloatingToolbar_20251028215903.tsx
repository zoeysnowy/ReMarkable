...
export const HeadlessFloatingToolbar: React.FC<FloatingToolbarProps> = ({
  ...
  const renderQuickActionButton = (feature: ToolbarFeatureType) => {
    ...
    // DateRange 使用特殊处理 - 不用 Tippy.js 包装，让 Ant Design 自己处理弹出
    if (feature === 'dateRange') {
      return (
        <div key={feature} className="headless-date-range-wrapper">
          <button
            className={`headless-toolbar-btn headless-toolbar-action-btn ${
              activePicker === feature ? 'headless-toolbar-btn-active' : ''
            }`}
          ...
          {activePicker === feature && (
            <div className="headless-date-range-popup">
              <SimpleDatePicker
                onSelect={(start, end) => {
                  console.log('🆕 Date selected in simple picker:', start, end);
                  onDateRangeSelect?.(start, end);
                  setActivePicker(null);
                }}
                onClose={() => {
                  console.log('🆕 Simple picker closed');
                  setActivePicker(null);
                }}
              />
            </div>
