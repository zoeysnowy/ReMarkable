...
const TimeColumn: React.FC<{
  ...
  const handleChange = (item: number) => {
    console.log(`⏰ [TimeColumn] ${type} changed`, { from: value, to: item });
    if (!disabled) {
      onChange(item);
    }
  };
...
const UnifiedDateTimePicker: React.FC<UnifiedDateTimePickerProps> = ({
  ...
  useEffect(() => {
    const handleGlobalClick = (e: MouseEvent) => {
      ...
      console.log('🌍 [Global] Click detected', {
        ...
        path: e.composedPath().map((el: any) => el.tagName || el.nodeName).slice(0, 5)
      });
    };
  ...
  const handleDateClick = (date: Dayjs) => {
    console.log('📅 [UnifiedDateTimePicker] handleDateClick called', {
  ...
  const handleQuickSelect = (days: number) => {
    console.log('⚡ [UnifiedDateTimePicker] handleQuickSelect called', { days });
    const end = dayjs();
  ...
  const handleApply = (e?: React.MouseEvent) => {
    console.log('🟢 [UnifiedDateTimePicker] handleApply called', {
      event: e,
  ...
  const handleCancel = (e?: React.MouseEvent) => {
    console.log('🔴 [UnifiedDateTimePicker] handleCancel called', {
      event: e,
      stack: new Error().stack
    });
    e?.stopPropagation();
