...
const UnifiedDateTimePicker: React.FC<UnifiedDateTimePickerProps> = ({
  ...
  const renderPreviewText = () => {
    ...
    const hasEndDate = selectedDates.end;
    
    if (!hasStartDate) return '未选择时间';
    ...
    if (isSameDay) {
      return `${start.format('YYYY-MM-DD（ddd）HH:mm')} → ${end.format('HH:mm')} ${duration}`;
    } else {
      return `${start.format('YYYY-MM-DD（ddd）HH:mm')} → ${end.format('HH:mm')} ${duration} +${dayDiff}`;
    }
  };
    ...
    >
      ...
      <div className="quick-select-section">
        <Space size="small" wrap>
          <Button size="small" onClick={handleSelectTomorrow}>明天</Button>
          <Button size="small" onClick={handleSelectThisWeek}>本周</Button>
          <Button size="small" onClick={handleSelectNextWeek}>下周</Button>
          <Button size="small" onClick={handleSelectMorning}>上午</Button>
          <Button size="small" onClick={handleSelectAfternoon}>下午</Button>
          <Button size="small" onClick={handleSelectEvening}>晚上</Button>
        </Space>
      </div>
