...
const TimeColumn: React.FC<{
  ...
  disabled?: boolean;
  syncScrollTarget?: number; // 添加同步滚动目标
}> = ({ type, value, onChange, disabled, syncScrollTarget }) => {
  ...
  // 当值改变时，滚动到对应位置
  useEffect(() => {
    if (value !== null && columnRef.current && contentRef.current) {
      // 每个 time-cell 的高度是 28px
      const cellHeight = 28;
      const containerHeight = columnRef.current.clientHeight;
      
      // 如果有同步滚动目标，使用它；否则使用自己的值
      const targetValue = syncScrollTarget !== undefined ? syncScrollTarget : value;
      const selectedIndex = targetValue + 1; // +1 因为有 "不选择" 选项
        ...
        <div className="time-section">
          ...
          <div className="time-columns-container">
            <TimeColumn
              ...
              disabled={false}
              syncScrollTarget={Math.min(startTime?.hour ?? 0, endTime?.hour ?? 0)}
            />
            <TimeColumn
              type="minute"
