        ...
        <RangePicker
          ...
          onOpenChange={(open) => {
            ...
            if (open) {
              ...
              // 创建一个集合来跟踪需要保护的时间列元素
              const protectedColumns = new Set<Element>();
              ...
              setTimeout(() => {
                ...
                allDropdowns.forEach((dropdown, index) => {
                  ...
                  timeColumns.forEach((column) => {
                    ...
                    protectedColumns.add(col); // 添加到保护列表
                    
                    // 🔥 立即劫持这个特定列的所有滚动方法
                    const originalColScrollTo = col.scrollTo;
                    const originalColScrollBy = col.scrollBy;
                    const originalColScrollIntoView = col.scrollIntoView;
                    
                    // 重写 scrollTo 方法
                    col.scrollTo = function(x?: any, y?: any) {
                      console.log(`📅 BLOCKED scrollTo on time column:`, x, y);
                      // 强制保持在顶部
                      if (typeof y === 'number' && y !== 0) {
                        console.log(`📅 Force reset scrollTo to 0 instead of ${y}`);
                        originalColScrollTo.call(this, x, 0);
                      }
                      return;
                    };
                    
                    // 重写 scrollBy 方法
                    col.scrollBy = function(x?: any, y?: any) {
                      console.log(`📅 BLOCKED scrollBy on time column:`, x, y);
                      return; // 完全阻止相对滚动
                    };
                    
                    // 重写 scrollIntoView 方法
                    col.scrollIntoView = function(arg?: any) {
                      console.log(`📅 BLOCKED scrollIntoView on time column:`, arg);
                      return; // 完全阻止滚动到视图
                    };
                    
                    // 🔥 重写 scrollTop 属性的 setter
                    Object.defineProperty(col, 'scrollTop', {
                      ...
                      set: function(value: number) {
                        ...
                        // 在初始化后2秒内，强制阻止任何非零滚动
                        if (timeSinceInit < 2000 && value !== 0) {
                          console.log(`📅 BLOCKED scrollTop set to ${value} during init phase (${timeSinceInit}ms)`);
                          originalScrollTop?.set?.call(this, 0);
                          return;
                        }
                        
                        console.log(`📅 Setting scrollTop to ${value} after init phase`);
