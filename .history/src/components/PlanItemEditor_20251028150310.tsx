...
const PlanItemEditor: React.FC<PlanItemEditorProps> = ({
  ...
  const handleSave = () => {
    ...
    // 🔄 如果有明确的时间点，创建 UnifiedTimeline 事件
    if (startTime && syncManager) {
      createUnifiedTimelineEvent(planItem);
    }
  ...
  // 🆕 创建 UnifiedTimeline 事件
  const createUnifiedTimelineEvent = (planItem: PlanItem) => {
    if (!planItem.startTime) return;

    // 计算结束时间（如果没有指定）
    const calculatedEndTime = planItem.endTime || 
      formatTimeForStorage(
        new Date(parseLocalTimeString(planItem.startTime).getTime() + 3600000) // 默认1小时
      );

    const event: Event = {
      id: `event-${Date.now()}`,
      title: `${planItem.emoji || ''} ${planItem.title}`.trim(),
      description: planItem.content || '',
      startTime: planItem.startTime,
      endTime: calculatedEndTime,
      isAllDay: planItem.isAllDay || false,
      location: planItem.location || '',
      tags: planItem.tags,
      tagId: planItem.tags[0] || '', // 使用第一个标签作为主标签
      category: 'planning', // 默认为计划类型
      source: 'local',
      syncStatus: 'pending',
    ...
    // 📝 记录本地操作，准备同步
    if (syncManager && syncManager.recordLocalAction) {
      syncManager.recordLocalAction('create', 'event', event.id, event, null);
      console.log('🔄 [PlanItemEditor] 已创建事件并记录同步操作:', event);
    }
    ...
    try {
      ...
      console.log('💾 [PlanItemEditor] 事件已保存到 localStorage');

      // 📢 触发事件变更通知
      window.dispatchEvent(new CustomEvent('local-events-changed', {
        detail: { action: 'create', event }
      }));
      ...
      console.error('❌ [PlanItemEditor] 保存事件失败:', error);
    }
  };
