...
import { parseNaturalDate, formatDateDisplay, DATE_EXAMPLES } from '../../utils/dateParser';
...
export const DateMentionPicker: React.FC<DateMentionPickerProps> = ({
  ...
  const handleInputChange = useCallback((value: string) => {
    ...
    if (value.trim()) {
      ...
      if (result) {
        setParsedDate({ start: result.start, end: result.end });
      } else {
        setParsedDate(null);
        setError('无法识别日期');
      }
      ...
      {parsedDate && (
        ...
        >
          <div style={{ color: '#1e40af', fontWeight: '500' }}>
            📅 {formatDateDisplay(parsedDate.start, true)}
          </div>
          {parsedDate.end && (
            <div style={{ color: '#6b7280', fontSize: '12px', marginTop: '4px' }}>
              至 {formatDateDisplay(parsedDate.end, true)}
            </div>
          )}
        </div>
      )}
