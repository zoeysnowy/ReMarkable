  ...
  const createNewTag = (level: number = 0, afterTagId?: string) => {
    ...
    setTags(prevTags => {
      ...
      if (afterTagId) {
        ...
        // ✅ 正确逻辑：新标签紧跟在当前标签后面（position + 1）
        // 找到所有标签并按position排序
        const sortedTags = [...prevTags].sort((a, b) => (a.position || 0) - (b.position || 0));
        
        // 将所有position >= afterPosition + 1 的标签position加1，为新标签腾出空间
        const updatedTagsWithSpace = sortedTags.map(tag => {
        ...
        // 新标签的position就是 afterPosition + 1
        newPosition = afterPosition + 1;
        
        console.log('🎯 Creating new tag after tagId:', afterTagId, 'at position:', newPosition, 'level:', level);
        console.log('📊 Shifted', updatedTagsWithSpace.filter(t => t !== afterTag && (t.position || 0) > afterPosition).length, 'tags forward');
        ...
        // ✅ 使用已经调整好位置的标签列表，加上新标签
        updatedTags = [...updatedTagsWithSpace, newTag];
        
        console.log('✅ Created new tag with shifted positions');
      } else {
