...
export const EventEditModal: React.FC<EventEditModalProps> = ({
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (authenticated) {
        try {
          const calendars = await simplifiedMicrosoftCalendarService.getCalendars();
          setMsCalendars(calendars);
          console.log('✅ Microsoft日历已加载:', calendars.length, '个');
        } catch (error) {
          console.error('❌ 加载Microsoft日历失败:', error);
          setMsAuthError('加载日历失败');
        }
      }
    };
  ...
  // Microsoft认证成功处理
  const handleMsAuthSuccess = async (authResult: any) => {
    console.log('🎉 Microsoft认证成功:', authResult);
  ...
  // Microsoft认证错误处理
  const handleMsAuthError = (error: Error) => {
    console.error('❌ Microsoft认证失败:', error);
    setMsAuthError(`认证失败: ${error.message}`);
    ...
    <div className="event-edit-modal-overlay" onClick={onClose}>
      <div className="event-edit-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {/* Microsoft日历集成 */}
          <div className="form-group">
            <label>Microsoft日历集成</label>
            <div className="microsoft-calendar-section">
              {!isAuthenticated ? (
                <div className="ms-auth-prompt">
                  <div className="auth-status">
                    <span className="status-icon">🔗</span>
                    <span>未连接Microsoft账户</span>
                  </div>
                  {msAuthError && (
                  ...
                  <button
                    type="button"
                    className="btn-connect-ms"
                    onClick={() => setShowMsLogin(true)}
                  >
                    🔐 连接Microsoft日历
                  </button>
                </div>
                ...
                <div className="ms-connected">
                  <div className="auth-status success">
                    <span className="status-icon">✅</span>
                    <span>已连接Microsoft账户</span>
                    <span className="calendar-count">({msCalendars.length} 个日历)</span>
                  </div>
                  <div className="ms-actions">
                    <button
                      type="button"
                      className="btn-sync-ms"
                      onClick={async () => {
                        try {
                          const calendars = await simplifiedMicrosoftCalendarService.getCalendars();
                          setMsCalendars(calendars);
                          console.log('🔄 Microsoft日历已刷新');
                        } catch (error) {
                  ...
                  {msCalendars.length > 0 && (
                    <div className="ms-calendar-list">
                      <h5>可用的Microsoft日历:</h5>
                      <div className="calendar-chips">
                        {msCalendars.map((cal) => (
                          <div key={cal.id} className="calendar-chip">
                            <span className="calendar-name">{cal.name}</span>
                            <span className="calendar-id">{cal.id.substring(0, 8)}...</span>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          {/* Microsoft登录模态框 */}
          {showMsLogin && (
            <div className="ms-login-overlay">
              <div className="ms-login-modal">
                <div className="modal-header">
                  <h3>连接Microsoft账户</h3>
                  <button
                    className="close-button"
                    onClick={() => setShowMsLogin(false)}
                  >
                    ×
                  </button>
                </div>
                <SimpleMicrosoftLogin
                  onSuccess={handleMsAuthSuccess}
                  onError={handleMsAuthError}
                />
              </div>
            </div>
          )}
