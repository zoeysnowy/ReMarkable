      ...
      <div className="plan-list-scroll-container" ref={editorContainerRef}>
        <UnifiedSlateEditor
          ...
          onChange={(updatedItems) => {
            // 🔍 调试：查看 updatedItems 的内容
            console.log(`[🔍 onChange 触发] updatedItems 数量: ${updatedItems.length}`);
            ...
            // 将 PlanItem[] 转换回 Event[] 并批量保存
            const changedItems: Event[] = [];
            const itemsToDelete: string[] = [];
            
            updatedItems.forEach((updatedItem: any) => {
              // 🔧 [FIX] 跳过待删除的 items（防止删除后又被重新创建）
              if (pendingDeleteIdsRef.current.has(updatedItem.id)) {
                console.log(`[⏭️ 跳过待删除 event] ${updatedItem.id}`);
                return;
              }
              ...
              // 🔍 调试日志
              if (isEmpty) {
                console.log(`[🔍 空白检测] ${updatedItem.id}`, {
                  title: updatedItem.title,
                  content: updatedItem.content,
                  description: updatedItem.description,
                  startTime: mergedItem.startTime,
                  endTime: mergedItem.endTime,
                  dueDate: mergedItem.dueDate,
                  existingItem: !!existingItem
                });
              }
              
              // 🔧 [FIX] 如果是完全空的 event，直接跳过（不保存也不删除）
              // 对于已存在的空 event，延迟删除（避免 onChange 循环）
              if (isEmpty) {
                if (existingItem) {
                  console.log(`[⏳ 延迟删除空白 event] ${updatedItem.id}`);
                  // 标记为稍后删除
                  itemsToDelete.push(updatedItem.id);
                } else {
                  console.log(`[⏭️ 跳过空白新 event] ${updatedItem.id}`);
                }
                return; // 跳过后续处理
              }
              ...
              if (isChanged) {
                ...
                const eventItem: Event = {
                  ...
                  isAllDay: existingItem?.isAllDay || false,
            ...
            // 🆕 批量删除完全空的 event（延迟执行，避免触发 onChange 循环）
            if (itemsToDelete.length > 0) {
              console.log(`[🗑️ 标记待删除] ${itemsToDelete.length} 个`);
              // 先添加到 pending set，防止在删除期间被重新创建
              itemsToDelete.forEach(id => pendingDeleteIdsRef.current.add(id));
              
              setTimeout(() => {
                itemsToDelete.forEach(id => {
                  console.log(`[🗑️ 执行删除] ${id}`);
                  onDelete(id);
                  // 删除完成后从 pending set 中移除
                  setTimeout(() => {
            ...
            // 批量保存
            if (changedItems.length > 0) {
              console.log(`[⚡ UnifiedSlateEditor 批量更新] ${changedItems.length} 个变更`);
              changedItems.forEach(item => {
                onSave(item);
                
                // 🔧 只有当 item 有时间字段时才同步到 UnifiedTimeline/EventService
                // 避免空时间的 Plan item 被强制创建成带时间的 Event
                const hasAnyTime = !!(item.startTime || item.endTime || item.dueDate);
                if (hasAnyTime) {
                  console.log(`[✅ 同步到 Calendar] ${item.title}`, { 
                    startTime: item.startTime, 
                    endTime: item.endTime, 
                    dueDate: item.dueDate 
                  });
