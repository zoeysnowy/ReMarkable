...
import { deserializeFromHtml, serializeToHtml, createEmptyParagraph } from './utils';
...
export const SlateLine: React.FC<SlateLineProps> = ({
  ...
  // ✅ [IME 修复] 完全禁用 AndroidPlugin
  // 原因：slate-android-plugin 在 Windows 桌面环境下会干扰搜狗输入法等 IME
  // 即使使用平台检测，useAndroidPlugin Hook 的内部逻辑仍然会执行并干扰 IME
  // TODO: 如果需要移动端支持，需要创建单独的移动端版本组件
  const editor = baseEditor;
  
  // 🚫 以下代码会导致 Windows IME 问题，已禁用：
  // const isMobile = useMemo(() => {
  //   return /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
  // }, []);
  // const editorWithAndroid = useAndroidPlugin(baseEditor);
  // const editor = isMobile ? editorWithAndroid : baseEditor;
  ...
  const [value, setValue] = useState<Descendant[]>(() => {
    try {
      ...
      const parsed = deserializeFromHtml(safeContent);
      // 防御性检查：确保至少有一个节点
      if (!parsed || parsed.length === 0) {
        console.warn('[SlateLine] Parsed content is empty, using default paragraph');
        return [createEmptyParagraph()];
      }
      return parsed;
    } catch (e) {
      console.error('[SlateLine] Failed to parse content:', e, 'content:', content);
      return [createEmptyParagraph()];
    }
  });
  ...
  useEffect(() => {
    ...
    if (contentChanged) {
      try {
        const safeContent = content || '<p></p>';
        const newValue = deserializeFromHtml(safeContent);
        // 防御性检查：确保至少有一个节点
        if (!newValue || newValue.length === 0) {
          console.warn('[SlateLine] Parsed new content is empty, keeping current value');
          return;
        }
        // 🆕 强制重新挂载编辑器以避免状态冲突
  ...
  }, [lineId, editor, onEditorReady, onEditorDestroy]);
    ...
    >
      ...
      >
        <Editable
          ...
          onFocus={() => {
            // console.log('[SlateLine] Focus 事件触发', { lineId, mode });
            onFocus?.();
          }}
          onBlur={onBlur}
