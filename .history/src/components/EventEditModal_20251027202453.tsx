...
export const EventEditModal: React.FC<EventEditModalProps> = ({
  ...
  const flatTags = useMemo(() => {
    console.log('🔍 [EventEditModal] hierarchicalTags 输入:', hierarchicalTags);
    ...
    console.log('📊 [EventEditModal] flatTags 输出:', flattened.map(t => ({ name: t.name, level: t.level })));
                ...
                <div className="tag-dropdown">
                  ...
                  <div className="tag-dropdown-list">
                    {filteredTags.length > 0 ? (
                      (() => {
                        console.log('🏷️ [EventEditModal] 标签层级调试:', filteredTags.map(tag => ({
                          name: tag.name,
                          level: tag.level,
