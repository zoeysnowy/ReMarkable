  ...
  const createNewTag = (level: number = 0, afterTagId?: string) => {
    ...
    setTags(prevTags => {
      ...
      if (afterTagId) {
        ...
        const afterLevel = afterTag.level || 0;
        
        // 计算正确的插入位置：找到当前标签后面第一个同级或更高级别的标签
        const sortedTags = [...prevTags].sort((a, b) => (a.position || 0) - (b.position || 0));
        const afterIndex = sortedTags.findIndex(tag => tag.id === afterTagId);
        
        // 🔧 关键修复：找到下一个同级或更高级别的标签（跳过所有子标签）
        let nextSameLevelIndex = -1;
        for (let i = afterIndex + 1; i < sortedTags.length; i++) {
          const tagLevel = sortedTags[i].level || 0;
          // 找到第一个层级 <= 当前层级的标签（同级或更高级）
          if (tagLevel <= level) {
        ...
        if (nextSameLevelIndex !== -1) {
          // 在当前标签和下一个同级标签之间插入
          const nextSameLevelTag = sortedTags[nextSameLevelIndex];
          const nextPosition = nextSameLevelTag.position || 0;
          insertPosition = (afterPosition + nextPosition) / 2;
          console.log('🎯 Inserting between same-level tags:', {
            afterTag: afterTag.name,
            afterPosition,
            nextTag: nextSameLevelTag.name,
            nextPosition,
            insertPosition
          });
        } else {
          // 没有找到同级标签，说明是该层级的最后一个，直接在后面添加
          insertPosition = afterPosition + 1;
          console.log('🎯 Inserting as last tag at this level:', {
            afterTag: afterTag.name,
            afterPosition,
            insertPosition
          });
        ...
        console.log('🎯 Creating new tag after tagId:', afterTagId, 'at position:', newPosition, 'level:', level);
