...
export const TimeCalendar: React.FC<TimeCalendarProps> = ({
  ...
  useEffect(() => {
    ...
    // console.log(`⏱️ [TimeCalendar] Component mounted in ${mountDuration.toFixed(2)}ms`);
  }, []);
  ...
  // console.log(`📊 [RENDER] TimeCalendar render #${renderCountRef.current} started`);
  ...
  const calendarEvents = useMemo(() => {
    ...
    console.log(`📅 [USEMEMO] Date range filter: ${events.length} → ${filteredByDateRange.length} events (${viewStart.toLocaleDateString()} ~ ${viewEnd.toLocaleDateString()})`);
  ...
  }, [events, hierarchicalTags, calendarSettings.visibleTags, calendarSettings.visibleCalendars, calendarSettings.eventOpacity, currentDate]);
  // 📅 创建日历分组配置
  ...
  useEffect(() => {
    ...
    // console.log(`⏱️ [RENDER] TimeCalendar render #${renderCountRef.current} completed in ${renderDuration.toFixed(2)}ms`);
  });
