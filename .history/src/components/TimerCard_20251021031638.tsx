...
import './TimerCard.css';import { icons } from '../assets/icons';

import './TimerCard.css';

export interface TimerSession {

  id: string;interface TimerCardProps {

  startTime: Date;  onTimerStart: (tagId: string) => void;

  endTime?: Date;  globalTimer: {

  duration: number;    isRunning: boolean;

  description: string;    tagId: string;

  isActive: boolean;    tagName: string;

}    tagEmoji?: string;

    tagColor?: string;

interface TimerCardProps {    startTime: number;

  onSessionComplete?: (session: TimerSession) => void;    elapsedTime: number;

  className?: string;    isPaused: boolean;

}    eventEmoji?: string;

    eventTitle?: string;

export const TimerCard: React.FC<TimerCardProps> = ({   } | null;

  onSessionComplete,  onTimerPause: () => void;

  className = ''   onTimerResume: () => void;

}) => {  onTimerStop: () => void;

  const [isRunning, setIsRunning] = useState(false);  onTimerCancel: () => void;

  const [currentSession, setCurrentSession] = useState<TimerSession | null>(null);  onTimerEdit: () => void;

  const [elapsedTime, setElapsedTime] = useState(0);  onStartTimeChange: (newStartTime: number) => void;
