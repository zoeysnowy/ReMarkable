...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  const editorLines = useMemo<FreeFormLine<PlanItem>[]>(() => {
    console.log('🔄 [editorLines] Creating lines from items:', items.map(i => ({ id: i.id, title: i.title, tags: i.tags })));
        ...
        {item.tags && item.tags.length > 0 ? (
          <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', alignItems: 'center' }}>
            {item.tags.map((tagName: string) => {
              ...
              console.log('🏷️ [Render Tag]:', { tagName, tag, tagColor, tagEmoji, displayName });
              
              return (
                <span
                  key={tagName}
                  className="plan-tag-chip"
                  style={{
                    display: 'inline-flex',
                    alignItems: 'center',
                    gap: '4px',
                ...
                >
                  ...
                  <span
                    ...
                    onClick={(e) => {
                      ...
                      console.log('🗑️ [删除标签]:', { tagName, updatedTags });
                      onSave(updatedItem);
                    }}
                    onMouseEnter={(e) => {
    ...
    <div className="plan-manager">
      ...
      <HeadlessFloatingToolbar
        ...
        onTagSelect={(tagIds: string[]) => {
          ...
          // 将标签添加到当前聚焦的 PlanItem
          if (currentFocusedLineId) {
            console.log('🏷️ [onTagSelect] 当前聚焦行 ID:', currentFocusedLineId);
            const item = items.find(i => i.id === currentFocusedLineId);
            
            if (item) {
              console.log('🏷️ [onTagSelect] 找到计划项:', { id: item.id, title: item.title, currentTags: item.tags });
              ...
              console.log('🏷️ [onTagSelect] 转换后的标签名称:', tagNames);
              ...
              console.log('🏷️ [onTagSelect] 准备保存更新后的计划项:', updatedItem);
              onSave(updatedItem);
            } else {
              console.warn('⚠️ [onTagSelect] 未找到聚焦的计划项 ID:', currentFocusedLineId);
            }
          } else {
            console.warn('⚠️ [onTagSelect] 没有聚焦的行 ID');
          }
        }}
