...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      // Alt+Backspace: 删除当前行的最后一个标签
      if (e.altKey && e.key === 'Backspace' && currentFocusedLineId) {
        ...
        if (item && item.tags && item.tags.length > 0) {
          ...
          // 删除最后一个标签
          const updatedTags = item.tags.slice(0, -1);
          const updatedItem = {
            ...item,
            tags: updatedTags,
          };
          console.log('⌨️ [键盘删除标签]:', { 
            removedTag: item.tags[item.tags.length - 1], 
  ...
  }, [items, currentFocusedLineId, onSave]);
