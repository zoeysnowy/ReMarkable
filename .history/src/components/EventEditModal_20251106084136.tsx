...
export const EventEditModal: React.FC<EventEditModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    attendees: [] as Array<{ name: string; email: string; type?: string; status?: string }>,
  });
  ...
  useEffect(() => {
    if (event && isOpen) {
      ...
      setFormData({
        title: event.title || '',
        description: event.description ?? '', // 使用 ?? 而不是 ||，保留空字符串
        startTime,
        endTime,
  ...
  const handleSave = () => {
    ...
    // 🕐 通过 EventHub 写入时间（EventHub 内部会调用 TimeHub）
    if (event?.id) {
      import('../services/EventHub').then(async ({ EventHub }) => {
        try {
          // 🔍 检查事件是否已存在于 localStorage（区分新建 vs 编辑）
          const existingEvent = EventHub.getSnapshot(event.id);
          ...
          if (isNewEvent) {
            ...
            // 获取创建后的事件
            const createdEvent = EventHub.getSnapshot(event.id);
            if (createdEvent) {
              onSave(createdEvent);
            }
          } else {
            ...
            const finalEvent = EventHub.getSnapshot(event.id);
          ...
          console.error('❌ [EventEditModal] Save failed:', error);
        }
      ...
      const updatedEvent: Event = {
        ...event,
        title: finalTitle,
        description: formData.description, // 直接使用表单值，不做 fallback
        startTime: startISO,
        endTime: endISO,
        location: formData.location,
        isAllDay: formData.isAllDay,
    ...
    >
      ...
      >
        ...
        <div className="modal-body">
          ...
          <div className="form-group form-group-inline">
            <label>组织者</label>
            <div className="organizer-inputs">
              <input
                type="text"
                placeholder="姓名"
