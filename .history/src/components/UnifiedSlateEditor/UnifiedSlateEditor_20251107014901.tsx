...
export const UnifiedSlateEditor: React.FC<UnifiedSlateEditorProps> = ({
  ...
  React.useEffect(() => {
    if (DEBUG) {
      ...
      debugLog(`%c[🚀 ${timestamp}] UnifiedSlateEditor mounted with DEBUG LOGGING ENABLED`, 
        'background: #4CAF50; color: white; padding: 4px 8px; border-radius: 3px; font-weight: bold;');
      debugLog(`%c[📊 ${timestamp}] Initial items count: ${items.length}`, 
        'color: #2196F3; font-weight: bold;');
    }
    
    return () => {
      if (DEBUG) {
        debugLog(`%c[👋 ${new Date().toISOString().split('T')[1].slice(0, 12)}] UnifiedSlateEditor unmounted`, 
