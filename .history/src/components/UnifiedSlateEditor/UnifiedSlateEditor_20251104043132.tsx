...
export const UnifiedSlateEditor: React.FC<UnifiedSlateEditorProps> = ({
  ...
  const handleChange = useCallback((newValue: Descendant[]) => {
    setValue(newValue as EventLineNode[]);
    
    // 转换为 PlanItem 并通知外部
    const planItems = slateNodesToPlanItems(newValue as EventLineNode[]);
    onChange(planItems);
  }, [onChange]);
  ...
  const handleKeyDown = useCallback((event: React.KeyboardEvent) => {
    ...
    if (event.key === 'Enter' && !event.shiftKey) {
      ...
      const eventLine = currentNode as EventLineNode;
    ...
    if (event.key === 'Enter' && event.shiftKey) {
      ...
      if (eventLine.type === 'event-line') {
        if (eventLine.mode === 'title') {
          ...
          Transforms.insertNodes(editor, descLine, {
            at: [path[0] + 1],
          });
        } else {
          ...
          Transforms.setNodes(
            editor,
            { mode: 'title' } as Partial<EventLineNode>,
            { at: path }
          );
        }
      }
    ...
    if (event.key === 'Tab' && !event.shiftKey) {
      ...
      if (eventLine.type === 'event-line') {
        ...
        Transforms.setNodes(
          editor,
          { level: newLevel } as Partial<EventLineNode>,
          { at: path }
        );
      }
      return;
  ...
  const handleCopy = useCallback((event: React.ClipboardEvent) => {
    ...
    const richHtml = slateNodesToRichHtml(fragment as EventLineNode[]);
  ...
  return (
    <div className={`unified-slate-editor ${className}`}>
      <Slate editor={editor} initialValue={value as Descendant[]} onChange={handleChange}>
        <Editable
