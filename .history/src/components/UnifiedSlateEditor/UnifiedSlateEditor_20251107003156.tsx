...
export const UnifiedSlateEditor: React.FC<UnifiedSlateEditorProps> = ({
      ...
      setTimeout(() => {
        try {
          ...
          if (editor.children.length > 0) {
            ...
            if (SlateElement.isElement(firstNode)) {
              // 将光标移动到第一个节点的开始位置
          ...
          console.warn('[handleGrayTextClick] Failed to focus:', err);
          // 聚焦失败时，尝试简单聚焦
          try {
            ReactEditor.focus(editor);
          } catch (e) {
            console.error('[handleGrayTextClick] Fallback focus failed:', e);
          }
        }
      }, 50);
    } catch (err) {
  ...
  return (
    <div className={`unified-slate-editor ${className}`}>
      <Slate editor={editor} value={value as unknown as Descendant[]} onChange={handleChange}>
        <Editable
