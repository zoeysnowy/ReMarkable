...
// 🔍 调试日志开关 - 在浏览器控制台运行 `window.SLATE_DEBUG = true` 开启
const DEBUG = typeof window !== 'undefined' && (window as any).SLATE_DEBUG === true;
...
export const UnifiedSlateEditor: React.FC<UnifiedSlateEditorProps> = ({
  ...
  React.useEffect(() => {
    ...
    // 使用 window.console 绕过任何可能的 logger 包装
    const originalLog = window.console.log.bind(window.console);
    originalLog(`%c[🚀 ${timestamp}] UnifiedSlateEditor mounted with DEBUG LOGGING ENABLED`, 
      'background: #4CAF50; color: white; padding: 4px 8px; border-radius: 3px; font-weight: bold;');
    originalLog(`%c[📊 ${timestamp}] Initial items count: ${items.length}`, 
      'color: #2196F3; font-weight: bold;');
    
    return () => {
      originalLog(`%c[👋 ${new Date().toISOString().split('T')[1].slice(0, 12)}] UnifiedSlateEditor unmounted`, 
  ...
  const handleChange = useCallback((newValue: Descendant[]) => {
    ...
    console.log(`[🔄 ${timestamp}] handleChange triggered`, {
    ...
    console.log(`[📤 ${timestamp}] Calling onChange with planItems`, {
