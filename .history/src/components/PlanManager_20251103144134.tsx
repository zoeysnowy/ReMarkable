...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  // 既可保存真实元素，也可保存虚拟 reference（用于光标定位）
  const dateAnchorRef = useRef<any>(null);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === '@' || (e.shiftKey && e.key === '2')) {
        ...
        if (selection && selection.rangeCount > 0) {
          ...
          // 优先使用虚拟 reference 锚定到光标矩形，避免编辑器重绘导致的元素丢失
          const rect = range.getBoundingClientRect();
          if (rect && (rect.width > 0 || rect.height > 0)) {
            const virtualRef = {
              getBoundingClientRect: () => rect,
              contextElement: editorContainerRef.current || document.body,
            };
            dateAnchorRef.current = virtualRef;
          } else {
            // 兜底：插入 1px span 作为真实锚点
