    ...
    <>
      ...
      <div className={`time-calendar-container ${className}`} style={{ 
        ...
        <div style={{
          ...
          alignItems: 'center',
          padding: '10px 16px',
          background: bgRgba, // 🎨 使用统一的背景色
          borderRadius: isWidgetMode ? '20px' : '4px', // 🎨 与 Main/Controller 一致的圆角
          border: isWidgetMode ? `2px solid ${calendarBackgroundColor}` : `1px solid ${getAdaptiveColors.borderMedium}`, // 🎨 与 Main 一致的边框
          fontSize: '13px',
          fontWeight: 500, // 🎨 增加字体粗细
          color: getAdaptiveColors.textSecondary, // 🎨 自适应文字颜色
          flexShrink: 0,
          backdropFilter: calendarOpacity < 1 ? 'blur(10px)' : 'none', // 🎨 统一的毛玻璃效果
          boxShadow: isWidgetMode 
            ? (calendarOpacity > 0.5 
                ? '0 4px 12px rgba(0,0,0,0.15)' 
                : '0 2px 8px rgba(0,0,0,0.1)') 
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            � 共 <strong style={{ color: getAdaptiveColors.accentColor, fontSize: '14px' }}>{events.length}</strong> 个事件
          </span>
        </div>
      </div>
