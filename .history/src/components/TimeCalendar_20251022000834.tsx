...
export const TimeCalendar: React.FC<TimeCalendarProps> = ({
  ...
  console.log(`📊 [RENDER] TimeCalendar render #${renderCountRef.current}`);
  ...
  const calendarEvents = useMemo(() => {
    ...
    const hasTagFilter = calendarSettings.visibleTags.length > 0;
    const hasCalendarFilter = calendarSettings.visibleCalendars.length > 0;
    ...
    const filteredByTags = events.filter(event => {
      ...
      if (hasTagFilter) {
        ...
        // ✅ 新逻辑：支持特殊标签选项
        // "no-tag" - 显示未定义标签的事件
        const hasNoTagOption = calendarSettings.visibleTags.includes('no-tag');
        ...
        // 有标签的事件：检查是否与筛选条件匹配
        return eventTags.some(tagId => calendarSettings.visibleTags.includes(tagId));
      }
      return true;
    });
    ...
    const filteredByCalendars = filteredByTags.filter(event => {
      ...
      if (hasCalendarFilter) {
        ...
        const hasLocalCreatedOption = calendarSettings.visibleCalendars.includes('local-created');
        const hasNotSyncedOption = calendarSettings.visibleCalendars.includes('not-synced');
        ...
        return calendarSettings.visibleCalendars.includes(event.calendarId);
      }
      return true;
    });
    ...
    const calendarEventsWithStats = uniqueFiltered
      .map(event => {
        ...
        return {
          ...calendarEvent,
          backgroundColor: originalColor + Math.floor(opacity * 255).toString(16).padStart(2, '0'),
  ...
  return (
    <>
      ...
      <div className={`time-calendar-container ${className}`} style={{ 
        ...
        // 🎨 CSS 变量：将用户设置的透明度和颜色传递给所有子元素
        ['--calendar-opacity' as any]: calendarOpacity,
        ['--calendar-bg-r' as any]: r,
        ['--calendar-bg-g' as any]: g,
        ['--calendar-bg-b' as any]: b,
        // 预计算常用的颜色组合
        ['--calendar-color-10' as any]: `rgba(${r}, ${g}, ${b}, ${calendarOpacity * 0.1})`,
        ['--calendar-color-20' as any]: `rgba(${r}, ${g}, ${b}, ${calendarOpacity * 0.2})`,
        ['--calendar-color-30' as any]: `rgba(${r}, ${g}, ${b}, ${calendarOpacity * 0.3})`,
        ['--calendar-color-50' as any]: `rgba(${r}, ${g}, ${b}, ${calendarOpacity * 0.5})`,
        ['--calendar-color-70' as any]: `rgba(${r}, ${g}, ${b}, ${calendarOpacity * 0.7})`,
        ['--calendar-color-90' as any]: `rgba(${r}, ${g}, ${b}, ${calendarOpacity * 0.9})`,
        // 边框颜色（更深一些）
        ['--calendar-border-light' as any]: `rgba(0, 0, 0, ${calendarOpacity * 0.15})`,
        ['--calendar-border' as any]: `rgba(0, 0, 0, ${calendarOpacity * 0.2})`,
        ['--calendar-border-strong' as any]: `rgba(0, 0, 0, ${calendarOpacity * 0.3})`,
        ...style // 允许外部覆盖
      }}>
        {/* 🎛️ 控制工具栏 */}
        <div className="toastui-calendar-controls" style={{
          ...
          border: `1px solid rgba(0, 0, 0, ${calendarOpacity * 0.2})`
        }}>
        ...
        <div style={{
          ...
          <span>
            📈 共 <strong style={{ color: '#667eea' }}>{events.length}</strong> 个事件
          </span>
          {lastSyncTime && (
            <span>
              🔄 最后同步: <strong>{lastSyncTime.toLocaleString('zh-CN')}</strong>
            </span>
          )}
        </div>
      </div>
