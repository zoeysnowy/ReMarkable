        ...
        <UnifiedSlateEditor
          ...
          onChange={(updatedItems) => {
            ...
            updatedItems.forEach((updatedItem: any) => {
              ...
              // 🔧 [FIX] 把 "(无标题)" 也视为空白
              const isTitleEmpty = !updatedItem.title?.trim() || updatedItem.title?.trim() === '(无标题)';
              const isContentEmpty = !updatedItem.content?.trim() || updatedItem.content?.trim() === '(无标题)';
              const isDescriptionEmpty = !updatedItem.description?.trim();
              ...
              // 🔍 详细调试日志
              console.log(`[🔍 处理 item] ${updatedItem.id}`, {
                title: `"${updatedItem.title}"`,
                titleTrimmed: `"${updatedItem.title?.trim()}"`,
                content: `"${updatedItem.content}"`,
                contentTrimmed: `"${updatedItem.content?.trim()}"`,
              ...
              if (isChanged) {
                ...
                const eventItem: Event = {
                  ...
                  ...updatedItem,
                  id: updatedItem.id,
                  title: updatedItem.title || '',  // 🔧 [FIX] 空标题就是空标题，不要设置为"(无标题)"
                  content: updatedItem.content,
                  description: updatedItem.description,
            ...
            if (itemsToDelete.length > 0) {
              console.log(`[🗑️ 删除空白 event] ${itemsToDelete.length} 个`);
