...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  const handleLinesChange = (newLines: FreeFormLine<PlanItem>[]) => {
    ...
    itemGroups.forEach((group, itemId) => {
      ...
      if (titleLine.data) {
        // 如果 data 中缺少 id，使用从行 id 推导的 itemId 兜底，避免生成无 id 的条目
        const updatedItem: PlanItem = { 
          ...(titleLine.data as any),
          id: (titleLine.data as any)?.id ?? itemId,
          title: plainText,
          content: titleLine.content, // 保存 HTML 用于行内显示
          tags: extractedTags,
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '14px', justifyContent: 'flex-end' }}>
        ...
        {(item.dueDate || item.startTime) && (() => {
          // 任务（仅截止日期）
          if (!item.startTime && item.dueDate) {
          ...
          if (item.startTime && item.endTime) {
            const { parseLocalTimeString, getTimeDifferenceInSeconds } = require('../utils/timeUtils');
            const start = parseLocalTimeString(item.startTime);
            const end = parseLocalTimeString(item.endTime);
            const pad2 = (n: number) => String(n).padStart(2, '0');
            const weekday = ['周日','周一','周二','周三','周四','周五','周六'][start.getDay()];
            const startDateStr = `${start.getFullYear()}-${pad2(start.getMonth() + 1)}-${pad2(start.getDate())}（${weekday}） ${pad2(start.getHours())}:${pad2(start.getMinutes())}`;
            const endTimeStr = `${pad2(end.getHours())}:${pad2(end.getMinutes())}`;

            // All-day detection (same day and both 00:00)
            const isAllDayEvent = start.getHours() === 0 && start.getMinutes() === 0 && end.getHours() === 0 && end.getMinutes() === 0 && start.toDateString() === end.toDateString();
            if (isAllDayEvent) {
              const month = start.getMonth() + 1;
              const day = start.getDate();
              return (
                <span style={{ color: '#6b7280', whiteSpace: 'nowrap' }}>
                  全天 {month}月{day}日 {weekday}
                </span>
              );
            }

            const durationSec = getTimeDifferenceInSeconds(start, end);
            const hours = Math.floor(durationSec / 3600);
            const minutes = Math.floor((durationSec % 3600) / 60);
            const durationText = minutes === 0 ? `${hours}h` : `${hours}h${minutes}m`;

            // Styled time preview structure
            return (
              <div className="preview-time-display" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span className="preview-start-time" style={{ fontSize: 14, fontWeight: 500, color: '#374151' }}>{startDateStr}</span>
                <div className="preview-arrow-section" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <span
                  ...
                  <svg className="arrow-icon" width="52" height="9" viewBox="0 0 52 9" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: 51, height: 'auto', flexShrink: 0 }}>
                ...
                <span className="preview-end-time" style={{ fontSize: 14, fontWeight: 500, color: '#374151', padding: '16px 6px' }}>{endTimeStr}</span>
              </div>
            );
          }
          return null;
