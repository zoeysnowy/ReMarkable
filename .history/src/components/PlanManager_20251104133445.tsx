    ...
    <div className="plan-manager">
      ...
      <HeadlessFloatingToolbar
        ...
        onTimeApplied={(startIso, endIso) => {
          ...
          if (!editor) {
            warn('picker', '⚠️ onTimeApplied: 找不到编辑器');
            return;
          }

          // ✅ 插入日期提及元素到编辑器
          const insertSuccess = insertDateMention(editor, startIso, endIso, isDescriptionMode);
        ...
        onTagSelect={(tagIds: string[]) => {
          ...
          const editor = unifiedEditorRef.current;
          if (!editor) {
            console.log('[onTagSelect] 跳过：找不到编辑器');
            return;
          }
          ...
          if (!tag) {
            console.log('[onTagSelect] 跳过：找不到标签');
            return;
          }

          // 使用新的 API：insertTag(editor, tagId, tagName, tagColor?, tagEmoji?, mentionOnly?)
          const success = insertTag(editor, insertId, tag.name, tag.color, tag.emoji, isDescriptionMode);
          if (!success) {
        ...
        onDateRangeSelect={(start: Date, end: Date) => {
          ...
          if (currentFocusedLineId) {
            ...
            if (item && editor) {
              ...
              // 使用 insertDateMention helper 插入日期
              const success = insertDateMention(editor, start.toISOString(), end.toISOString(), isDescriptionMode);
              if (!success) return;
