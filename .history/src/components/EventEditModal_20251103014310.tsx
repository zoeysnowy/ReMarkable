...
import { formatTimeForStorage, parseLocalTimeString, formatDateTimeForInput, formatDateForInput } from '../utils/timeUtils';
...
export const EventEditModal: React.FC<EventEditModalProps> = ({
  ...
  useEffect(() => {
    if (event && isOpen) {
      // 使用 timeUtils 正确解析时间
      const startDateObj = event.startTime ? parseLocalTimeString(event.startTime) : undefined;
      const endDateObj = event.endTime ? parseLocalTimeString(event.endTime) : undefined;
      // 根据是否全天，选择不同的 input 值格式
  ...
  }, [event, isOpen, draggable, resizable]);
