  ...
  const handleSaveEventFromModal = async (updatedEvent: Event) => {
    ...
    try {
      ...
      if (eventIndex === -1) {
        ...
        // 🔒 Bug Fix: 增量更新 - 只更新 EditModal 实际修改的字段
        // 检测哪些字段真正变化了
        ...
        // 只有真正变化的字段才更新
        if (updatedEvent.title !== originalEvent.title) updates.title = updatedEvent.title;
        if (updatedEvent.location !== originalEvent.location) updates.location = updatedEvent.location;
        if (updatedEvent.isAllDay !== originalEvent.isAllDay) updates.isAllDay = updatedEvent.isAllDay;
        if (JSON.stringify(updatedEvent.tags) !== JSON.stringify(originalEvent.tags)) updates.tags = updatedEvent.tags;
        if (updatedEvent.tagId !== originalEvent.tagId) updates.tagId = updatedEvent.tagId;
        if (JSON.stringify(updatedEvent.calendarIds) !== JSON.stringify(originalEvent.calendarIds)) updates.calendarIds = updatedEvent.calendarIds;
        if (updatedEvent.calendarId !== originalEvent.calendarId) updates.calendarId = updatedEvent.calendarId;
        
        // ⚠️ 关键：description 只有在明确不同时才更新（不能用 || 运算符判断）
        // 如果 EditModal 传回的 description 和原始的不同，才更新
        if (updatedEvent.description !== originalEvent.description) {
          updates.description = updatedEvent.description;
          console.log('📝 [TimeCalendar] Description changed:', {
            old: originalEvent.description ? `${originalEvent.description.length} chars` : '(empty)',
            new: updatedEvent.description ? `${updatedEvent.description.length} chars` : '(empty)'
          });
        }
        
        // 时间字段特殊处理（通过 TimeHub 更新，这里只是同步状态）
        if (updatedEvent.startTime !== originalEvent.startTime) updates.startTime = updatedEvent.startTime;
        if (updatedEvent.endTime !== originalEvent.endTime) updates.endTime = updatedEvent.endTime;
