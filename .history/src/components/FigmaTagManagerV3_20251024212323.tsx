...
interface FigmaTagManagerProps {
...
const FigmaTagManagerV3: React.FC<FigmaTagManagerProps> = ({
  microsoftService,
  tagService,
  availableCalendars,
  globalTimer,
  ...
  const handleCalendarMappingClick = (tagId: string, event: React.MouseEvent) => {
    ...
    // 🔧 智能批量操作：如果当前标签在选中列表中，且选中了多个标签，则应用到所有选中的标签
    const isTagSelected = selectedTags.includes(tagId);
    const shouldBatchUpdate = isTagSelected && selectedTags.length > 1;
    
    if (shouldBatchUpdate) {
      console.log(`📅 [Smart Batch] Tag ${tagId} is selected with ${selectedTags.length - 1} other tags, enabling batch mode`);
      setShowCalendarPicker({
        show: true,
        tagId: `batch:${selectedTags.join(',')}`,
        position: { x: rect.left, y: rect.bottom + 5 }
      });
    } else {
      setShowCalendarPicker({
    ...
    <div>
      ...
      */}
        ...
        <div style={{
          ...
          <div>批量操作快捷键：</div>
          <div>• <kbd>Delete</kbd> / <kbd>Backspace</kbd> - 删除</div>
          <div>• <kbd>Shift+Alt+↑/↓</kbd> - 移动</div>
          <div>• <kbd>Shift+Alt+M</kbd> - 日历映射</div>
          <div style={{ marginTop: '8px', color: '#3b82f6' }}>💡 选中多个标签后，点击任意一个的日历映射即可批量设置</div>
        </div>
              ...
              <div style={{
                ...
                <div 
                  ...
                  style={{
                    ...
                    border: selectedTags.includes(tag.id) && selectedTags.length > 1 ? '2px solid #3b82f6' : 'none' // 🎯 批量模式边框
                  }}
                  onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#f1f5f9'}
                  onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
                  title={
                    selectedTags.includes(tag.id) && selectedTags.length > 1 
                      ? `批量设置 (${selectedTags.length}个标签)` 
                      : "点击设置日历映射"
                  }
                >
                  {/* 🎯 批量操作指示器 */}
...
export default FigmaTagManagerV3;
