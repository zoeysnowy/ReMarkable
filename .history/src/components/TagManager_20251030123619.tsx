  ...
  const createNewTag = (level: number = 0, afterTagId?: string) => {
    ...
    setTags(prevTags => {
      ...
      if (afterTagId) {
        ...
        console.log('🎯 Creating new tag after tagId:', afterTagId, 'at position:', newPosition, 'needsShifting:', needsShifting);
        ...
        // 如果需要移动后续标签，将所有 position >= insertPosition 的标签 +1
        if (needsShifting) {
          const shiftedTags = prevTags.map(tag => {
            if ((tag.position || 0) >= insertPosition) {
              return { ...tag, position: (tag.position || 0) + 1 };
            }
            return tag;
          });
          updatedTags = [...shiftedTags, newTag];
          console.log('🔄 Shifted subsequent tags forward by 1');
        } else {
