...
export const FreeFormEditor = <T,>({
  ...
  // 保存光标位置的 ref
  const cursorPositionRef = useRef<{ lineId: string; offset: number } | null>(null);
  ...
  // 保存光标位置
  const saveCursorPosition = useCallback((lineId: string) => {
    ...
    if (selection && selection.rangeCount > 0) {
      ...
      if (element && element.contains(range.startContainer)) {
        ...
        cursorPositionRef.current = { lineId, offset };
      }
    }
  }, []);
  
  // 恢复光标位置
  const restoreCursorPosition = useCallback((lineId: string) => {
    if (!cursorPositionRef.current || cursorPositionRef.current.lineId !== lineId) {
      return;
    }
      ...
      {lines.map((line) => (
        ...
        >
          ...
          <span
            ...
            ref={(el) => {
              // 当元素更新后，恢复光标位置
              if (el && cursorPositionRef.current?.lineId === line.id) {
                setTimeout(() => restoreCursorPosition(line.id), 0);
              }
            }}
            style={{
              outline: 'none',
