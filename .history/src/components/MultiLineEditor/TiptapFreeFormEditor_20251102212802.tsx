...
export const TiptapFreeFormEditor = <T,>({
  ...
  const handleLineEnter = useCallback((lineId: string) => {
    ...
    // 聚焦到新行
    console.log('[TiptapFreeFormEditor] Trying to focus new line:', newLine.id);
    setTimeout(() => {
      // Debug: 查看所有 data-line-id 元素
      const allLines = document.querySelectorAll('[data-line-id]');
      console.log('[TiptapFreeFormEditor] All line elements:', Array.from(allLines).map(el => el.getAttribute('data-line-id')));
      
      const wrapper = document.querySelector(`[data-line-id="${newLine.id}"]`) as HTMLElement;
      console.log('[TiptapFreeFormEditor] Found wrapper:', wrapper);
      
      const element = wrapper?.querySelector('.ProseMirror') as HTMLElement;
      console.log('[TiptapFreeFormEditor] Found ProseMirror element:', element);
      
      if (element) {
        element.focus();
        console.log('[TiptapFreeFormEditor] Focused successfully');
      } else {
        console.error('[TiptapFreeFormEditor] Element not found for lineId:', newLine.id);
      }
    }, 50);
  }, [lines, onLinesChange]);
  ...
  const handleLineShiftEnter = useCallback((lineId: string) => {
    ...
    if (!isDescriptionMode) {
      ...
      console.log('[TiptapFreeFormEditor] Trying to focus description line:', descLine.id);
      setTimeout(() => {
        const element = document.querySelector(`[data-line-id="${descLine.id}"] .ProseMirror`) as HTMLElement;
        console.log('[TiptapFreeFormEditor] Found desc element:', element);
        if (element) {
          element.focus();
          console.log('[TiptapFreeFormEditor] Focused desc successfully');
        }
      }, 50);
    } else {
      ...
      setTimeout(() => {
        const element = document.querySelector(`[data-line-id="${titleLineId}"] .tiptap-line-editor`) as HTMLElement;
        element?.focus();
      }, 10);
    }
  }, [lines, onLinesChange]);
  ...
  const handleArrowUp = useCallback((lineId: string) => {
    ...
    if (currentIndex > 0) {
      ...
      const prevElement = document.querySelector(`[data-line-id="${prevLineId}"] .tiptap-line-editor`) as HTMLElement;
  ...
  const handleArrowDown = useCallback((lineId: string) => {
    ...
    if (currentIndex < lines.length - 1) {
      ...
      const nextElement = document.querySelector(`[data-line-id="${nextLineId}"] .tiptap-line-editor`) as HTMLElement;
      nextElement?.focus();
    }
  }, [lines]);
  ...
  const handleGrayTextClick = useCallback(() => {
    ...
    setTimeout(() => {
      const newElement = document.querySelector(`[data-line-id="${newLine.id}"] .tiptap-line-editor`) as HTMLElement;
      newElement?.focus();
    }, 10);
  }, [lines, onLinesChange]);
