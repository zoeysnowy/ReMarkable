...
export const TiptapFreeFormEditor = <T,>({
  ...
  const handleLineEnter = useCallback((lineId: string) => {
    ...
    // 标记需要聚焦的新行
    console.log('[TiptapFreeFormEditor] Setting pendingFocus to:', newLine.id);
  ...
  const handleLineShiftEnter = useCallback((lineId: string) => {
    ...
    if (!isDescriptionMode) {
      ...
      // 标记需要聚焦的 description 行
      console.log('[TiptapFreeFormEditor] Setting pendingFocus to description:', descLine.id);
      pendingFocusRef.current = descLine.id;
      ...
      pendingFocusRef.current = titleLineId;
    }
  }, [lines, onLinesChange]);
  ...
  useEffect(() => {
    if (pendingFocusRef.current) {
      const lineId = pendingFocusRef.current;
      console.log('[TiptapFreeFormEditor] useEffect: Attempting to focus:', lineId);
      ...
      const wrapper = document.querySelector(`[data-line-id="${lineId}"]`) as HTMLElement;
      ...
      if (wrapper) {
        ...
        if (element) {
          ...
          console.error('[TiptapFreeFormEditor] ProseMirror element not found in wrapper');
        }
      } else {
        console.error('[TiptapFreeFormEditor] Wrapper not found for lineId:', lineId);
      }
      
      // 清除pending focus
      pendingFocusRef.current = null;
    }
  }, [lines]); // Re-run when lines change
