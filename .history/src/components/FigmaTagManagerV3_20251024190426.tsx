...
const FigmaTagManagerV3: React.FC<FigmaTagManagerProps> = ({
  ...
  useEffect(() => {
    ...
    const handlePaste = (e: ClipboardEvent) => {
      ...
      if (jsonData) {
        try {
          ...
          if (Array.isArray(tagsData) && tagsData.length > 0) {
            ...
            if (isCut) {
              // 🔧 剪切 + 粘贴 = 移动操作（保留原ID）
              console.log('✂️ [Paste] Detected CUT operation - moving tags');
              ...
              setTags(prevTags => {
                ...
                // 2. 在新位置添加标签（保留原ID）
                const idMap = new Map<string, string>(); // 旧parentId -> 新parentId的映射
                ...
                console.log('✅ [Paste] Moved tags:', tagsData.length);
                return remainingTags;
              });
            } else {
              // 📋 复制 + 粘贴 = 新建操作（生成新ID）
              console.log('📋 [Paste] Detected COPY operation - creating new tags');
              ...
              setTags(prevTags => {
                ...
                tagsData.forEach((tagData: any, index: number) => {
                  const newId = `tag-${Date.now()}-${Math.random()}`;
                  idMap.set(tagData.id, newId);
                ...
                console.log('✅ [Paste] Created new tags:', tagsData.length);
                return newTags;
              });
            }
            return; // 成功处理JSON，退出
          ...
          console.warn('⚠️ [Paste] Failed to parse JSON, fallback to text:', error);
        }
      }
      ...
      if (!pastedText) {
        console.log('📋 [Paste] No text data in clipboard');
        return;
      }

      console.log('📋 [Paste] Using text fallback. Raw pasted text:', pastedText);
      console.log('📋 [Paste] Text length:', pastedText.length);
