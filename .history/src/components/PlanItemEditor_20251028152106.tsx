...
export interface PlanItem {
  ...
  notes?: string;
  // UnifiedTimeline 集成
  syncToTimeline?: boolean;  // 是否同步到时间线
...
const PlanItemEditor: React.FC<PlanItemEditorProps> = ({
  ...
  const [syncToTimeline, setSyncToTimeline] = useState(item?.syncToTimeline ?? true);
  ...
  useEffect(() => {
    if (item) {
      ...
      setNotes(item.notes || '');
      setSyncToTimeline(item.syncToTimeline ?? true);
    }
  }, [item]);
  ...
  // 检查是否应该同步到时间线
  const shouldSyncToTimeline = () => {
    return syncToTimeline && startTime && endTime;
  };

  // 创建或更新 UnifiedTimeline Event
  const syncToUnifiedTimeline = useCallback((planItem: PlanItem) => {
    if (!shouldSyncToTimeline()) {
      return;
    }

    const eventData: Event = {
      id: planItem.eventId || `plan-event-${Date.now()}`,
      title: `${planItem.emoji || ''}${planItem.title}`.trim(),
      description: planItem.content || planItem.notes || '',
      ...
      isTask: true,
      category: `priority-${planItem.priority}`,
      remarkableSource: true,
    };

    if (planItem.eventId && onUpdateEvent) {
      // 更新现有事件
      onUpdateEvent(planItem.eventId, eventData);
    } else if (onCreateEvent) {
      // 创建新事件
      onCreateEvent(eventData);
      planItem.eventId = eventData.id;
    }
  }, [onCreateEvent, onUpdateEvent, syncToTimeline]);
  ...
  const handleSave = () => {
    ...
    // 同步到 UnifiedTimeline
    syncToUnifiedTimeline(planItem);
    ...
    <div className="plan-item-editor-overlay" onClick={handleCancel}>
      <div className="plan-item-editor-container" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="plan-editor-main">
          ...
          <div className="plan-editor-header">
            <div className="plan-editor-title-row">
              ...
              {shouldSyncToTimeline() && (
                <span className="plan-sync-badge" title="将同步到时间线">
                  🔄 Timeline
                </span>
              )}
        ...
        <div className="plan-editor-sidebar">
          {/* UnifiedTimeline 同步开关 */}
          <div className="plan-sidebar-section">
            <div className="plan-sync-toggle">
              <label className="plan-sync-label">
                <input
                  type="checkbox"
                  checked={syncToTimeline}
                  onChange={(e) => setSyncToTimeline(e.target.checked)}
                  className="plan-sync-checkbox"
                />
                <span>同步到时间线</span>
              </label>
              {syncToTimeline && (!startTime || !endTime) && (
                <p className="plan-sync-hint">
                  💡 需要设置开始和结束时间才能同步
                </p>
              )}
            </div>
