...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  useEffect(() => {
    ...
    console.log(`%c[🚀 ${timestamp}] PlanManager mounted with DEBUG LOGGING ENABLED`, 
      'background: #FF9800; color: white; padding: 4px 8px; border-radius: 3px; font-weight: bold;');
    console.log(`%c[📊 ${timestamp}] Initial items count: ${items.length}`, 
  ...
  const executeBatchUpdate = useCallback((updatedItems: any[]) => {
    ...
    console.log(`[⚙️ ${timestamp}] executeBatchUpdate started`, {
    ...
    if (crossDeletedIds.length > 0) {
      ...
      console.log(`[🗑️ ${timestamp}] Cross-line deletion detected`, {
        deletedIds: crossDeletedIds,
      });
    ...
    updatedItems.forEach((updatedItem: any) => {
      ...
      if (isEmpty) {
        if (existingItem) {
          ...
          console.log(`[🗑️ ${timestamp}] Empty item deletion`, {
      ...
      if (isChanged) {
        console.log(`[💾 ${timestamp}] Item changed, preparing to save`, {
          itemId: updatedItem.id,
          isNew: !existingItem,
  ...
  const debouncedOnChange = useCallback((updatedItems: any[]) => {
    ...
    console.log(`[📥 ${timestamp}] debouncedOnChange called`, {
      itemCount: updatedItems.length,
      items: updatedItems.map(item => ({
        id: item.id,
        title: item.title,
        hasContent: !!(item.title?.trim() || item.description?.trim()),
      })),
    });
    ...
    onChangeTimerRef.current = setTimeout(() => {
      ...
      console.log(`[⏱️ ${new Date().toISOString().split('T')[1].slice(0, 12)}] Debounce timer fired, executing batch update`);
      ...
      <div className="plan-list-scroll-container" ref={editorContainerRef}>
        <UnifiedSlateEditor
          ...
          renderLinePrefix={(line) => {
            ...
            console.log(`[🎨 ${timestamp}] renderLinePrefix called`, {
              lineId: line.lineId,
              baseLineId,
              foundItem: !!item,
              mode: line.mode,
            ...
            const result = renderLinePrefix(fakeFormLine);
            console.log(`[✅ ${timestamp}] renderLinePrefix result`, {
              lineId: line.lineId,
              hasResult: !!result,
              isCompleted: item.isCompleted,
            });
