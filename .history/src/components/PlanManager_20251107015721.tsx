...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  const executeBatchUpdate = useCallback((updatedItems: any[]) => {
    ...
    if (isDebugEnabled()) {
      window.console.log(`[⚙️ ${timestamp}] executeBatchUpdate started`, {
    ...
    if (crossDeletedIds.length > 0) {
      ...
      if (isDebugEnabled()) {
        window.console.log(`[🗑️ ${timestamp}] Cross-line deletion detected`, {
  ...
  const debouncedOnChange = useCallback((updatedItems: any[]) => {
    ...
    if (isDebugEnabled()) {
      window.console.log(`[📥 ${timestamp}] debouncedOnChange called`, {
