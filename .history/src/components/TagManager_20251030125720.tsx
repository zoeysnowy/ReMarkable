...
const TagManager: React.FC<TagManagerProps> = ({
  ...
  useEffect(() => {
    ...
    console.log('🚀 [TagManager] Component initializing...');
    
    const savedTags = loadTagsFromStorage();
    const savedCounts = loadCheckinCountsFromStorage();
    
    console.log(`📦 [TagManager] Loaded ${savedTags.length} tags from storage`);
    ...
    if (hasLevelCalculated || hasPositionInitialized) {
      console.log('💾 [TagManager] Saving calculated levels and positions to storage...');
    ...
    console.log(`✅ [TagManager] Initialized in ${duration.toFixed(2)}ms`);
  }, []);
  ...
  useEffect(() => {
    const handleGlobalFocus = (e: FocusEvent) => {
      ...
      if (target && target.textContent?.includes('新增标签')) {
        console.log('🎯 GLOBAL FOCUS on gray text detected! (Should not happen in new implementation)');
  ...
  useEffect(() => {
    ...
    const handleCopy = (e: ClipboardEvent) => {
      ...
      console.log('📋 [Copy] Selected tags:', selectedTags.length);
      ...
      try {
        ...
        console.error('📋 [Copy] Error setting clipboard data:', error);
      }
    };
    ...
    const handleCut = (e: ClipboardEvent) => {
      ...
      console.log('✂️ [Cut] Selected tags:', selectedTags.length);
      ...
      try {
        ...
        console.log('✂️ [Cut] Cut to clipboard:', {
          textFormat: textFormat,
          jsonData: jsonData,
          tagsCount: selectedTags.length
        });
        ...
        console.error('✂️ [Cut] Error setting clipboard data:', error);
      }
    };
    ...
    const handleCopyPaste = (tagsData: any[]) => {
      setTags(prevTags => {
        ...
        tagsData.forEach((tagData: any, index: number) => {
          ...
          console.log(`📋 [CopyPaste] Creating tag #${index}:`, { name: newTag.name, color: newTag.color, level: newTag.level });
          newTags.push(newTag);
        });
        
        console.log('✅ [CopyPaste] Created new tags:', tagsData.length);
        return newTags;
      });
    };
    ...
    const handleCutPaste = (tagsData: any[]) => {
      setTags(prevTags => {
        ...
        tagsData.forEach((tagData: any, index: number) => {
          ...
          console.log(`✂️ [CutPaste] Moving tag #${index}:`, { name: movedTag.name, color: movedTag.color, level: movedTag.level });
        ...
        console.log('✅ [CutPaste] Moved tags:', tagsData.length);
    ...
    const handlePaste = (e: ClipboardEvent) => {
      ...
      if (!isRemarkableFormat && (target.contentEditable === 'true' || target.tagName === 'INPUT' || target.tagName === 'TEXTAREA')) {
        console.log('📋 [Paste] Allowing default paste behavior in editable element');
        return;
      }
      
      // 如果是我们的格式，无论在哪里都要处理（包括编辑框）
      console.log('📋 [Paste] Processing paste event');
      console.log('📋 [Paste] Raw pasted text FULL:', pastedText);
      console.log('📋 [Paste] Text starts with marker?', isRemarkableFormat);
      console.log('📋 [Paste] First 200 chars:', pastedText?.substring(0, 200));
      ...
      if (pastedText && pastedText.startsWith('__REMARKABLE_TAGS_JSON__')) {
        try {
          ...
          if (match) {
            ...
            console.log('✅ [Paste] Extracted JSON from Base64 text successfully:', parsedData);
            ...
            if (Array.isArray(tagsData) && tagsData.length > 0) {
              ...
              if (isCut) {
                // ✂️ 剪切 + 粘贴 = 移动操作（保留原ID）
                console.log('✂️ [Paste] Detected CUT operation from Base64');
                handleCutPaste(tagsData);
              } else {
                // 📋 复制 + 粘贴 = 新建操作（生成新ID）
                console.log('📋 [Paste] Detected COPY operation from Base64');
          ...
          console.warn('⚠️ [Paste] Failed to extract Base64 JSON:', error);
        }
      }
      
      // 🔧 Step 2: 回退尝试读取 application/json（备用方案）
      const jsonData = e.clipboardData?.getData('application/json');
      console.log('📋 [Paste] JSON data from clipboard:', jsonData ? 'found' : 'not found');
      
      if (jsonData) {
        try {
          const parsedData = JSON.parse(jsonData);
          console.log('✅ [Paste] Parsed application/json successfully:', parsedData);
          ...
          if (Array.isArray(tagsData) && tagsData.length > 0) {
            ...
            if (isCut) {
              console.log('✂️ [Paste] Detected CUT operation from application/json');
              handleCutPaste(tagsData);
            } else {
              console.log('📋 [Paste] Detected COPY operation from application/json');
              handleCopyPaste(tagsData);
            }
            return; // 成功处理JSON，退出
          }
        } catch (error) {
          console.warn('⚠️ [Paste] Failed to parse application/json, fallback to text:', error);
        }
      }

      // 🔧 Step 3: 最后回退到纯文本格式解析（⚠️ 无法保留颜色信息）
      if (!pastedText) {
        console.log('📋 [Paste] No paste data found');
        return;
      }
      
      // 移除 Base64 标记头（如果存在）
      const cleanText = pastedText.replace(/^__REMARKABLE_TAGS_JSON__.+?__\n/, '');
      console.log('📋 [Paste] Using text fallback. Clean text preview:', cleanText.substring(0, 100));
      
      // ⚠️ 文本格式无法保留颜色信息，只有JSON格式才能完整保留
      // 格式1: # emoji 名称 (带前导空格)
      // 格式2: #emoji名称 (紧凑格式)
      // 格式3: # emoji名称 (无空格)
      const lines = cleanText.split('\n');
      console.log('📋 [Paste] Total lines:', lines.length);
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (match1) {
          emoji = match1[1];
          name = match1[2].trim();
          console.log(`✅ [Paste] Matched pattern1:`, { emoji, name, level });
        } else if (match2) {
          emoji = match2[1];
          name = match2[2].trim();
          console.log(`✅ [Paste] Matched pattern2:`, { emoji, name, level });
        } else if (match3) {
          emoji = undefined;
          name = match3[1].trim();
          console.log(`✅ [Paste] Matched pattern3:`, { name, level });
          ...
          console.log(`⚠️ [Paste] Fallback parsing:`, { emoji, name, level, original: trimmedLine });
        } else {
          console.log(`❌ [Paste] Line doesn't start with #, skipping:`, trimmedLine);
          continue;
        }
      ...
      if (parsedTags.length > 0) {
        console.log('📋 [Paste] Parsed tags:', parsedTags);
        e.preventDefault();
        ...
        setTags(prevTags => {
            ...
            if (parsedTag.level > 0) {
              ...
              for (let i = index - 1; i >= 0; i--) {
                if (parsedTags[i].level < parsedTag.level) {
                  ...
                  console.log(`🔗 [Paste] Found parent for "${parsedTag.name}":`, {
          ...
          console.log('✅ [Paste] Successfully imported tags:', newTags.length - prevTags.length);
          console.log('📊 [Paste] Final tag structure:', newTags.slice(-parsedTags.length).map(t => ({
            name: t.name,
            level: t.level,
            parentId: t.parentId
          })));
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // 🗑️ Delete/Backspace - 批量删除
      if (e.key === 'Delete' || e.key === 'Backspace') {
        e.preventDefault();
        
        if (confirm(`确定要删除选中的 ${selectedTags.length} 个标签吗？`)) {
          console.log('🗑️ [Batch Delete] Deleting tags:', selectedTags.map(t => t.name));
      ...
      // ⬆️⬇️ Shift+Alt+↑/↓ - 批量上下移动
      if (e.shiftKey && e.altKey && (e.key === 'ArrowUp' || e.key === 'ArrowDown')) {
        e.preventDefault();
        
        const direction = e.key === 'ArrowUp' ? -1 : 1;
        console.log(`🔄 [Batch Move] Moving ${selectedTags.length} tags ${direction > 0 ? 'down' : 'up'}`);
        ...
        setTags(prevTags => {
          ...
          if (direction === -1 && minIndex === 0) {
            console.log('⚠️ Already at top');
            return prevTags;
          }
          if (direction === 1 && maxIndex === newTags.length - 1) {
            console.log('⚠️ Already at bottom');
            return prevTags;
          }
      ...
      if (e.shiftKey && e.altKey && e.key.toLowerCase() === 'm') {
        ...
        console.log(`📅 [Batch Calendar] Editing calendar mapping for ${selectedTags.length} tags`);
  ...
  const handleCalendarMappingClick = (tagId: string, event: React.MouseEvent) => {
    ...
    if (shouldBatchUpdate) {
      console.log(`📅 [Smart Batch] Tag ${tagId} is selected with ${selectedTagIds.length - 1} other tags, enabling batch mode`);
      setShowCalendarPicker({
        show: true,
  ...
  const handleCalendarSelect = (calendar: { calendarId: string; calendarName: string; color?: string }) => {
    ...
    if (showCalendarPicker.tagId.startsWith('batch:')) {
      ...
      console.log(`📅 [Batch Calendar] Setting calendar for ${tagIds.length} tags:`, calendar.calendarName);
      ...
      console.log(`✅ [Batch Calendar] Updated ${tagIds.length} tags: ${tagNames}`);
  ...
  const createNewTag = (level: number = 0, afterTagId?: string) => {
    ...
    setTags(prevTags => {
      ...
      if (afterTagId) {
        ...
        if (!afterTag) {
          console.error('❌ After tag not found:', afterTagId);
          return prevTags;
        }
        
        console.log('📍 Found afterTag:', {
        ...
        console.log('🎯 Creating new tag after tagId:', afterTagId, 'afterPosition:', afterPosition, 'newPosition:', newPosition);
        console.log('📋 Current tags positions before shift:', 
          prevTags
        ...
        const shiftedTags = prevTags.map(tag => {
          if ((tag.position || 0) > afterPosition) {
            console.log(`  🔄 Shifting tag "${tag.name}" from position ${tag.position} to ${(tag.position || 0) + 1}`);
        ...
        console.log('✅ Created tag at position', newPosition, 'and shifted', shiftedTags.filter(t => t.position !== prevTags.find(pt => pt.id === t.id)?.position).length, 'tags forward');
        console.log('📊 All tags after creation (sorted by position):', 
          updatedTags
            .sort((a, b) => (a.position || 0) - (b.position || 0))
    ...
    (async () => {
      ...
      if (!calendarMapping) {
        ...
        console.log('🎯 使用默认日历映射:', calendarMapping);
      }
    ...
    const focusNewTag = (retryCount = 0) => {
      ...
      if (element) {
        console.log('🎯 Successfully found and focusing new tag:', newId);
        element.focus();
        return;
      }
      ...
      if (retryCount < 5) {
        console.log(`🔄 Retrying focus for tag ${newId}, attempt ${retryCount + 1}`);
        setTimeout(() => focusNewTag(retryCount + 1), 50);
      } else {
        console.error('❌ Failed to focus new tag after 5 attempts:', newId);
      }
    };
  ...
  const handleNewTagActivation = () => {
    console.log('🚨 handleNewTagActivation called!');
    console.log('🔍 用户点击了灰色文本:', userClickedGrayText);
    console.log('🔍 Call stack:', new Error().stack);
    
    // 只有当用户明确点击了灰色文本时才激活
    if (!userClickedGrayText) {
      console.log('🚨 阻止激活：用户没有明确点击灰色文本');
      return;
    }
  ...
  const handleCancelNewTag = () => {
    console.log('❌ Cancelling new tag creation');
  ...
  const handleTagKeyDown = (e: React.KeyboardEvent, tagId: string, currentLevel: number) => {
    // 立即输出，确保函数被调用
    console.log('🚨 FUNCTION CALLED - handleTagKeyDown');
    ...
    if (e.key === 'Enter') {
      ...
      e.preventDefault();
      // 如果标签名称为空，取消创建（删除这个标签）
      const currentTag = tags.find(tag => tag.id === tagId);
      if (currentTag && (!currentTag.name || currentTag.name.trim() === '')) {
        console.log('🚫 ESC pressed - Canceling tag creation:', tagId);
        setTags(prev => prev.filter(tag => tag.id !== tagId));
        // 失焦当前输入框
        (e.target as HTMLElement).blur();
      } else {
        // 如果标签已有内容，只是失焦
        console.log('🚫 ESC pressed - Blur input:', tagId);
        (e.target as HTMLElement).blur();
      }
    } else if (e.key === 'Tab') {
      e.preventDefault();
      ...
      e.preventDefault();
      console.log('🔼 Shift+Alt+↑ detected for tagId:', tagId);
      console.log('🔼 Current tags state:', tags.map(t => ({id: t.id, position: t.position, name: t.name})));
      // Shift+Alt+↑: 向上移动标签
      moveTagUp(tagId);
      ...
      e.preventDefault();
      console.log('🔧 Manual position fix triggered');
      fixTagPositions();
    }
  };
  ...
  const validateAndFixPositions = (tagsToCheck: ExtendedHierarchicalTag[]): ExtendedHierarchicalTag[] => {
    ...
    if (positions.length !== uniquePositions.length) {
      console.warn('⚠️ Found duplicate positions:', positions);
      console.warn('🔧 Synchronously fixing positions...');
      // 立即修复重复的position
  ...
  const fixTagPositions = () => {
    setTags(prevTags => {
      ...
      console.log('🔧 Fixed tag positions:', fixedTags.map(t => ({id: t.id, position: t.position, name: t.name})));
      return fixedTags;
    });
  };
  ...
  const moveTagUp = (tagId: string) => {
    console.log('🔼 moveTagUp called with tagId:', tagId);
    
    setTags(prevTags => {
      console.log('🔼 Current prevTags:', prevTags.map(t => ({id: t.id, position: t.position, name: t.name})));
      
      // 先验证和修复position
      const validatedTags = validateAndFixPositions(prevTags);
      const sortedTags = [...validatedTags].sort((a, b) => (a.position || 0) - (b.position || 0));
      console.log('🔼 Sorted tags:', sortedTags.map(t => ({id: t.id, position: t.position, name: t.name})));
      
      const currentIndex = sortedTags.findIndex(tag => tag.id === tagId);
      console.log('🔼 Current index:', currentIndex, 'for tagId:', tagId);
      
      if (currentIndex <= 0) {
        console.log('🔼 Tag is already at the top, no movement needed');
      ...
      console.log('🔼 New level for moved tag:', newLevel);
      ...
      const newTags = validatedTags.map(tag => {
        if (tag.id === tagId) {
          ...
          console.log('🔼 Updated current tag:', newTag);
          return newTag;
        } else if (tag.id === previousTag.id) {
          const newTag = { ...tag, position: currentTag.position };
          console.log('🔼 Updated previous tag:', newTag);
          return newTag;
        }
        return tag;
      });
      
      console.log('🔼 Final result:', newTags.map(t => ({id: t.id, position: t.position, name: t.name})));
  ...
  const moveTagDown = (tagId: string) => {
    console.log('🔽 moveTagDown called with tagId:', tagId);
    ...
    setTags(prevTags => {
      ...
      if (currentIndex < 0 || currentIndex >= sortedTags.length - 1) {
        console.log('🔽 Tag is already at the bottom or not found, no movement needed');
        return validatedTags; // 返回修复后的数据
      }
      ...
      const newTags = validatedTags.map(tag => {
        if (tag.id === tagId) {
          ...
          const newTag = { ...tag, position: currentTag.position };
          console.log('🔽 Updated next tag:', newTag);
          return newTag;
        }
        return tag;
      });
      ...
      <ClickTracker 
        ...
        onClickDetected={(event) => {
          ...
          if (event.elementInfo?.textContent?.includes('新增标签')) {
            console.log('🎯 CLICKED ELEMENT CONTAINS GRAY TEXT!', event);
          }
        }}
      >
      */}
            ...
            >
              ...
              <span 
                ...
                onClick={(e) => {
                  ...
                  console.log('🎯 Gray text area clicked at:', {
                ...
                onFocus={(e) => {
                  console.log('🎯 Gray text focused, isCreating:', isCreatingNewTag);
                  console.log('🔍 用户是否明确点击了灰色文本:', userClickedGrayText);
                  
                  // 如果已经在创建模式，直接返回
                  if (isCreatingNewTag) {
                    console.log('✅ Already in creation mode, nothing to do');
                    return;
                  }
                  
                  // 如果用户没有明确点击灰色文本，这是意外的focus事件，忽略
                  if (!userClickedGrayText) {
                    console.log('🚨 Focus事件不是由用户直接点击引起的，忽略');
                    // 移除焦点以防止意外激活
                  ...
                  console.log('✅ Valid focus on placeholder text, activating creation');
                  handleNewTagActivation();
                  e.currentTarget.textContent = '';
                }}
                onBlur={(e) => {
                  console.log('👋 Gray text blurred');
                  const content = e.currentTarget.textContent || '';
                  
                  if (isCreatingNewTag) {
                    if (content.trim() === '') {
                      // 如果没有输入内容，取消创建
                      console.log('❌ Empty content, cancelling creation');
                      handleCancelNewTag();
                      e.currentTarget.textContent = '点击新增标签，Tab/Shift+Tab切换层级，Shift+Alt+↑/↓上下移动标签';
                    } else {
                      // 保存新标签内容，退出创建模式
                      console.log('💾 Saving new tag:', content.trim());
                      setIsCreatingNewTag(false);
                    }
                  } else {
                    // 确保显示占位符文本
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    ...
                    const content = e.currentTarget.textContent || '';
                    if (content.trim() !== '') {
                      console.log('✅ Creating new tag with Enter:', content.trim());
                      setIsCreatingNewTag(false);
                      // 这里可以继续创建下一个标签
                      setTimeout(() => {
                        handleNewTagActivation();
                    ...
                    e.preventDefault();
                    console.log('🚫 Cancelling new tag creation with Escape');
                    handleCancelNewTag();
                    e.currentTarget.textContent = '点击新增标签，Tab/Shift+Tab切换层级，Shift+Alt+↑/↓上下移动标签';
                    e.currentTarget.blur();
                  }
