...
const CalendarSettingsPanel: React.FC<CalendarSettingsPanelProps> = ({
  ...
  // 🔧 修改初始位置：让dragbar显示在TimeCalendar"设置"按钮的下方
  // 假设窗口宽度1920px，面板宽度260px，设置按钮在右上角
  // x: window.innerWidth - 260 - 20 = 右侧留20px边距
  // y: 150px（header 115px + 设置按钮高度 + 一点间距）
  const [position, setPosition] = useState({ 
    x: typeof window !== 'undefined' ? window.innerWidth - 280 : 1640, 
    y: 150 
  });
