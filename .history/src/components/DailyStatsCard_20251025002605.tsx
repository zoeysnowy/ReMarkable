...
export const DailyStatsCard: React.FC<DailyStatsCardProps> = ({
  ...
  // 🔧 监听 localStorage 变化以实时更新
  useEffect(() => {
    const handleStorageChange = () => {
      console.log('📊 [DailyStats] eventsUpdated event received, reloading from localStorage');
      // 🔧 从 localStorage 重新读取最新数据
      try {
        const saved = localStorage.getItem('remarkable-events');
        if (saved) {
          const latestEvents = JSON.parse(saved);
          console.log('📊 [DailyStats] Loaded latest events:', latestEvents.length);
  ...
  const tagStats = useMemo(() => {
    console.log('📊 [DailyStats] useMemo triggered, refreshKey:', refreshKey, 'events count:', events.length);
    ...
    console.log('📊 [DailyStats] Calculating stats for:', {
      date: targetDateStr,
    ...
    let totalDuration = 0;
    let maxDuration = 0; // 跟踪最大时长
    const now = new Date(); // 🔧 当前时间

    dayEvents.forEach(event => {
      const tags = event.tags || (event.tagId ? [event.tagId] : []);
      
      console.log('📊 [DailyStats] Event tags:', {
      ...
      tags.forEach(tagId => {
        ...
        if (duration > 0) {
          ...
          console.log('📊 [DailyStats] Calculated duration:', {
            tagId,
            eventTitle: event.title,
            ...
            isOngoing: end > nowTime
          });
        }
      });
    });

    console.log('📊 [DailyStats] Tag durations map:', Array.from(tagDurations.entries()).map(([id, dur]) => ({
      tagId: id,
      duration: dur,
      tag: TagService.getTagById(id)
    })));

    // 获取扁平标签列表（带parentId字段）
    const flatTags = TagService.getFlatTags();

    console.log('📊 [DailyStats] Flat tags with parentId:', flatTags.map(t => ({
      id: t.id,
      name: t.name,
      emoji: t.emoji,
      parentId: t.parentId,
    ...
    const buildTagStatsFromFlat = (): TagStats[] => {
      ...
      flatTags.forEach(tag => {
        ...
        while (currentParentId) {
          ...
          if (parentStat) {
            ...
            console.log('📊 [DailyStats] Adding child duration to parent:', {
              child: tag.name,
              childDuration: childStat.duration,
              parent: flatTags.find(t => t.id === currentParentId)?.name,
              parentNewTotal: parentStat.duration
            });
      ...
      console.log('📊 [DailyStats] Stats after accumulation:', Array.from(statsMap.values()).map(s => ({
        name: s.name,
        duration: s.duration,
        hasParent: flatTags.find(t => t.id === s.id)?.parentId
      ...
      flatTags.forEach(tag => {
        ...
        console.log('📊 [DailyStats] Processing tag in hierarchy:', {
          tagId: tag.id,
          tagName: tag.name,
          parentId: tag.parentId,
          statDuration: stat?.duration,
          willSkip: !stat || stat.duration === 0
        });
