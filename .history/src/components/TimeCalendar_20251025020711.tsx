...
export const TimeCalendar: React.FC<TimeCalendarProps> = ({
  ...
  // 🎨 Widget模式：强制移除 Toast UI Calendar 的内联背景色
  useEffect(() => {
  ...
  }, [isWidgetMode, currentView]); // 视图切换时重新执行
    ...
    <>
      ...
      <div className={`time-calendar-container ${className}`} style={{ 
        ...
        <ToastUIReactCalendar
          ...
          theme={{
            common: {
              border: isWidgetMode ? 'none' : '1px solid #e9ecef', // 🎨 Widget 模式移除边框
              backgroundColor: isWidgetMode ? 'transparent' : 'white', // 🎨 Widget 模式透明背景
              holiday: {
                color: '#ff4040'
              },
              saturday: {
