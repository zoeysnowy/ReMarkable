...
const PlanManager: React.FC<PlanManagerProps> = ({
  ...
  const handleLinesChange = (newLines: FreeFormLine<Event>[]) => {
    ...
    itemGroups.forEach((group, itemId) => {
      ...
      console.log('[🔍 DEBUG handleLinesChange] 处理 item', {
        itemId,
        '标题行content': titleLine.content.substring(0, 50),
        '标题行data.description': (titleLine.data as any)?.description,
        '描述行存在': !!descLine,
        '描述行content': descLine?.content,
        '描述行data.description': (descLine?.data as any)?.description
      });
      ...
      if (titleLine.data) {
        ...
        if (isChanged) {
          console.log('[🔍 DEBUG handleLinesChange] 检测到变更', {
            itemId: updatedItem.id,
            title: updatedItem.title.substring(0, 30),
            description: updatedItem.description,
            变更原因: !existingItem ? '新建' : '内容变化'
          });
    ...
    // 🔧 批量更新：只更新真正变化的 item
    console.log(`[⚡ 批量更新] 检测到 ${changedItems.length} 个变更，${unchangedItemIds.size} 个未变更`);
