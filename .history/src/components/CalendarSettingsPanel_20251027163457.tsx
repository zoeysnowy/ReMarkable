...
const CalendarSettingsPanel: React.FC<CalendarSettingsPanelProps> = ({
  ...
  const getInitialPosition = () => {
    if (typeof window === 'undefined') return { x: 1640, y: 180 };
    ...
    if (calendarContainer) {
      ...
      return {
        x: rect.right - 280, // 面板宽度260px + 20px边距
            ...
            <div className="filter-list">
              {availableTags.length === 0 ? (
                ...
                availableTags.map(tag => {
                  // 🔧 调试：打印标签的层级信息
                  if (tag.level && tag.level > 0) {
                    console.log(`🏷️ 标签 "${tag.name}" level: ${tag.level}, paddingLeft: ${tag.level * 12}px`);
                  }
                  return (
