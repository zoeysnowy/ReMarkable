...
function App() {
  ...
  const handleStartTimeChange = (newStartTime: number) => {
    ...
    // 修正时间差计算逻辑
    // 如果提前开始时间（newStartTime < oldStartTime），应该增加已计时时长
    // 如果推迟开始时间（newStartTime > oldStartTime），应该减少已计时时长
    const timeDiff = oldStartTime - newStartTime; // 正数=提前开始（增加时长），负数=延后开始（减少时长）
    ...
    if (globalTimer.isRunning) {
      // 计时中：需要考虑当前运行时长
      const currentRunningTime = now - oldStartTime; // 当前这轮运行的时长
      ...
      // 重新设置计时器，startTime 更新为当前时间，elapsedTime 调整
      setGlobalTimer({
        ...globalTimer,
        startTime: now, // 重置为当前时间
