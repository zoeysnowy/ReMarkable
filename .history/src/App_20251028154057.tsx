...
function App() {
  ...
  const handleTimerStop = async () => {
    ...
    try {
      ...
      console.log('✅ [Timer Stop] 事件已保存:', finalEvent);

      // ⚡ 立即切换到时间页面，不等待同步完成（避免卡顿）
      setCurrentPage('time');

      // 🔄 延迟同步到 Outlook（不阻塞 UI，给用户时间开始下一个 Timer）
      if (syncManager) {
        // 🔧 延迟 5 秒再同步，避免阻塞用户开始下一个 Timer
        setTimeout(async () => {
          try {
            await syncManager.recordLocalAction('create', 'event', finalEvent.id, finalEvent);
            console.log('✅ [Timer Stop] 事件已异步同步到 Outlook');
          } catch (error) {
            console.error('❌ [Timer Stop] 异步同步失败:', error);
          }
        }, 5000); // 🔧 延迟 5 秒
      }
    } catch (error) {
  ...
  // 打开编辑器
  const handleOpenPlanEditor = useCallback((item?: PlanItem) => {
    setCurrentPlanItem(item || null);
  ...
  const handleCreateEvent = useCallback((event: Event) => {
    try {
      ...
      console.log('✅ [App] 创建 UnifiedTimeline 事件:', event.title);
    } catch (error) {
  ...
  const handleUpdateEvent = useCallback((eventId: string, updates: Partial<Event>) => {
    try {
      const saved = localStorage.getItem(STORAGE_KEYS.EVENTS);
      const events = saved ? JSON.parse(saved) : [];
      const updated = events.map((e: Event) => 
        e.id === eventId ? { ...e, ...updates } : e
      );
      localStorage.setItem(STORAGE_KEYS.EVENTS, JSON.stringify(updated));
      setAllEvents(updated);
      console.log('✅ [App] 更新 UnifiedTimeline 事件:', eventId);
    } catch (error) {
      console.error('❌ [App] 更新事件失败:', error);
    }
  }, []);
  ...
  const renderCurrentPage = useMemo(() => {
    ...
    switch (currentPage) {
      ...
      case 'plan':
        // FloatingButton 操作配置
        const planFloatingActions: FloatingButtonAction[] = [
          {
            id: 'new-plan',
            label: '新建计划',
            icon: '📝',
              ...
              <div className="plan-items-container">
                {planItems.length === 0 ? (
                  <div className="plan-empty-state">
                    ...
                    <p className="empty-text">还没有计划项</p>
                    <p className="empty-hint">点击右下角按钮创建新计划</p>
                  </div>
                ) : (
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            borderLeft: `4px solid ${priorityColors[item.priority || 'medium']}`,
                          }}
                          onClick={() => handleOpenPlanEditor(item)}
                        >
                          <div className="plan-item-header">
                            {item.emoji && <span className="plan-item-emoji">{item.emoji}</span>}
                            <span className={`plan-item-priority ${item.priority}`}>
                              {item.priority?.toUpperCase()}
                            </span>
                            {item.syncToTimeline && item.startTime && item.endTime && (
                              <span className="plan-item-sync-badge" title="已同步到时间线">
                                🔄
                              </span>
                            )}
                          </div>
                          
                          <h3 className="plan-item-title" style={{ color: item.color }}>
                            {item.bulletStyle === 'checkbox' && (
                              <span className="plan-bullet">
                                {item.isCompleted ? '☑' : '☐'}
                              </span>
                            )}
                            {item.bulletStyle === 'dot' && <span className="plan-bullet">●</span>}
                            {item.bulletStyle === 'number' && <span className="plan-bullet">1.</span>}
                            {item.title || '无标题'}
                          </h3>
                          
                          {item.content && (
                            <p className="plan-item-content">{item.content}</p>
                          )}
                          
                          {item.tags && item.tags.length > 0 && (
                            <div className="plan-item-tags">
                              {item.tags.map((tag) => (
                                <span key={tag} className="plan-item-tag">
                                  #{tag}
                                </span>
                              ))}
                            </div>
                          ...
                          {item.startTime && item.endTime && (
                            <div className="plan-item-time">
                              <span className="plan-time-icon">⏰</span>
                              <span className="plan-time-text">
                          ...
                          {item.duration && item.duration > 0 && (
                            <div className="plan-item-duration">
                              <span className="plan-duration-icon">⏱️</span>
                              <span className="plan-duration-text">
                                已用时: {Math.floor(item.duration / 3600)}h {Math.floor((item.duration % 3600) / 60)}m
                              </span>
                            </div>
                          )}
