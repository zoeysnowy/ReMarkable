...
if (msService) {
  const isAuth = msService.isAuthenticated();
  console.log(`✅ MicrosoftCalendarService 已初始化`);
  console.log(`   登录状态: ${isAuth ? '✅ 已登录' : '❌ 未登录'}`);
  
  if (isAuth) {
    const account = msService.getAccountInfo?.();
    if (account) {
      console.log(`   账户: ${account.name || account.username}`);
      console.log(`   邮箱: ${account.username}`);
    }
  }
} else {
