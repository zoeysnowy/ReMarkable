...
(async function fixOrphanedPendingEvents() {
  ...
  for (const orphanedEvent of orphanedEvents) {
    // 🔧 关键：使用原始 events 数组中已修复的事件对象
    const originalEvent = events.find(e => e.id === orphanedEvent.id);
    
    if (!originalEvent) {
      console.warn(`  ⚠️ 跳过：找不到事件 ${orphanedEvent.id}`);
      continue;
    }
    ...
    console.log(`✓ [${createdCount}/${orphanedEvents.length}] 创建 action: ${originalEvent.title} (calendarId: ${originalEvent.calendarId ? '✓' : '✗'})`);
  }
