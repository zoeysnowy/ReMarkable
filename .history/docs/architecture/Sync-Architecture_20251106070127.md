...
```typescript
// 用户操作
1. 编辑事件标题 → recordLocalAction('update', v1)
2. 修改时间 → recordLocalAction('update', v2)
3. 调整描述 → recordLocalAction('update', v3)

// 同步队列
actionQueue = [
  { type: 'update', entityId: 'event-123', data: v1 },
  { type: 'update', entityId: 'event-123', data: v2 },
  { type: 'update', entityId: 'event-123', data: v3 }
]
```

**问题**: 联网后会发送 3 次 PATCH 请求，浪费 API 配额
...
| 场景 | 规则 | 结果 |
|------|------|------|
| **多次 UPDATE** | 保留时间戳最新的 | 1 次 API 调用 |
| **CREATE → UPDATE** | 合并为 CREATE（最新数据） | 1 次 API 调用 |
| **UPDATE → DELETE** | 保留 DELETE | 1 次 DELETE 调用 |
| **CREATE → DELETE** | 两个都标记为已同步 | 0 次 API 调用（抵消） |

### 性能提升

```typescript
// 场景：离线编辑事件 10 次
优化前：10 个 action → 10 次 API 调用
优化后：10 个 action → 1 次 API 调用
节省：90% API 配额 ✅
```
...
```bash
🔧 [Queue Optimization] Consolidated 10 actions → 2 actions (saved 8 API calls)
  - event-123: 5 updates → 1 update (最新版本)
  - event-456: 3 updates + 1 delete → 1 delete
```
...
6. ✅ **队列合并优化** - 减少 90% 不必要的 API 调用（2025-11-06 新增）
7. ✅ **认证恢复优化** - 页面刷新后自动恢复登录状态（2025-11-06 新增）
