...
Timer 模块是 ReMarkable 的核心功能之一，为用户提供**专注计时**能力：
- ✅ 帮助用户追踪任务的实际耗时
- ✅ 自动创建时间事件，无需手动记录
- ✅ 支持暂停/恢复/取消，灵活应对工作中断
- ✅ 与标签系统深度集成，实现分类计时
...
> **作为** 使用番茄工作法的知识工作者  
> **我希望** 快速开始一个 25 分钟的专注计时  
> **以便** 记录我的工作时间，并在日历中看到我的专注分布
...
| **TimeCalendar** | Timer 事件显示 | Timer 保存事件 → TimeCalendar 渲染 |
| **EventEditModal** | Timer 编辑入口 | Timer → 打开 Modal → 保存回 Timer |
