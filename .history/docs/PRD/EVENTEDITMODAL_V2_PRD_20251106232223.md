...
**交互逻辑**:
```typescript
const handleTimerAction = async (action: 'start' | 'pause' | 'resume') => {
  switch (action) {
    case 'start':
      // 为当前事件启动新的 Timer
...
**多 Timer 支持说明**：
- ✅ 支持多个事件同时计时
- ✅ 每个事件独立管理自己的 Timer 状态
- ✅ 使用 `Map<eventId, TimerState>` 存储所有活动 Timer
- ✅ 用户可以在多个事件间切换，各自计时互不影响
- ✅ 实际进展区域（【下 Section】）只显示当前事件自己的 Timer 数据
...
function shouldShowActualProgress(event: Event, activeTimers: Map<string, TimerState>): boolean {
  // 只要当前事件有 Timer（运行中或已暂停），就显示实际进展
  return activeTimers.has(event.id);
}
```

**数据来源**: 
- `activeTimers.get(event.id).segments[]` - 当前事件的时间片段
- `activeTimers.get(event.id).elapsedTime` - 累积时长
