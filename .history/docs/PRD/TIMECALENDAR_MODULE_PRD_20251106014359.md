...
```typescript
// 如果没有时间，使用 createdAt 作为 fallback
let startDate: Date;
let endDate: Date;

if (!event.startTime || !event.endTime) {
  // 没有时间的 Task：使用 createdAt 作为显示日期
  const createdDate = parseLocalTimeString(event.createdAt);
  startDate = new Date(createdDate);
  startDate.setHours(0, 0, 0, 0); // 设为当天 00:00
  endDate = new Date(createdDate);
  endDate.setHours(23, 59, 59, 999); // 设为当天 23:59
} else {
  startDate = parseLocalTimeString(event.startTime);
  endDate = parseLocalTimeString(event.endTime);
}
```

**关键点**：
1. ✅ **createdAt 永远存在**：所有事件创建时自动设置 `createdAt` 字段
2. ✅ **在 TimeCalendar 中显示**：无时间的 Task 显示在 `createdAt` 对应日期的 Task Bar 上
3. ✅ **在 Plan 页面不显示时间**：只显示标题、标签、描述等内容
4. ✅ **时间可后续添加**：用户通过 FloatingBar 或 @chrono 添加时间后，自动转换为 Event（时间块）
