...
export interface Event {
  ...
  isTask?: boolean;         // 无固定时间段的事件（在日历中显示为 task 条）
...
| `isTask: true` | 无固定时间段的事件（只有日期） | TimeCalendar（显示为 task 条） | 在日历中显示为任务条而非时间块 |
...
interface Event {
  // ...
  isTask?: boolean; // 标记为无固定时间段的事件（Task 类型）
}
```
...
2. **时间判断逻辑**（PlanManager.tsx L747-819）：
   ```typescript
   // 自动判断 isTask
   if (hasStart && hasEnd) {
     // 有开始和结束时间 → Event (time/allday)
     isTask = false;
   } else if (hasStart && !hasEnd) {
     // 只有开始时间 → Task
     isTask = true;
     finalEndTime = finalStartTime; // 设置为同一天
   } else if (!hasStart && hasEnd) {
     // 只有结束时间 → Task
     isTask = true;
...
- **PlanManager 创建事件时**（L654）：
  ...
  const newItem: Event = {
    // ...
    isTask: true, // ✅ 默认创建为 Task 类型（无固定时间段）
...
- **syncToUnifiedTimeline 同步时**（L747-819）：
  ...
  // 有完整时间段 → isTask: false (Event)
  // 缺少时间 → isTask: true (Task)
  ```

**典型场景**：

| 场景 | startTime | endTime | isTask | 日历显示 |
|------|-----------|---------|--------|----------|
| 会议（14:00-15:00） | ✅ | ✅ | `false` | 时间块（占据 14:00-15:00） |
| 全天活动 | ✅ | ✅ | `false` | 全天事件条 |
| 待办事项（今天完成） | ❌ | ❌ | `true` | 任务条（只显示日期） |
