...
interface Contact {
  ...
  tags?: string[];
  note?: string;
  source?: 'outlook' | 'google' | 'icloud' | 'remarkable' | string; // 来源（或首次出现的 eventId）
}
...
function shouldShowAttendees(event: Event): boolean {
  const { attendees } = event;
  
  // 检查 attendees 是否有人且有联系方式
...
// 整个参会人section的显示条件
function shouldShowAttendeesSection(event: Event): boolean {
  return shouldShowOrganizer(event) || shouldShowAttendees(event);
}
```
...
async function searchContacts(query: string): Promise<Contact[]> {
  ...
  // 1. 优先级最高：Outlook/Google/iCloud 联系人
  const platformContacts = await ContactService.searchPlatformContacts(query);
  results.push(...platformContacts.map(c => ({ ...c, source: c.platform })));
  
  // 2. ReMarkable 本地联系人
  const remarkableContacts = await ContactService.searchLocalContacts(query);
  results.push(...remarkableContacts.map(c => ({ ...c, source: 'remarkable' })));
  
  // 3. 历史事件中的参会人/发起人
  const historicalContacts = await EventService.searchHistoricalParticipants(query);
  results.push(...historicalContacts);
...
function formatSource(source: string): string {
  switch (source) {
    case 'outlook': return 'Outlook 联系人';
    case 'google': return 'Google 联系人';
    case 'icloud': return 'iCloud 联系人';
    case 'remarkable': return 'ReMarkable 联系人';
    default: return `来自事件：${getEventTitle(source)}`; // source 是 eventId
  }
}
```
  ...
  >
    ...
    <div className="preview-fields">
      ...
      <EditableField
        ...
        onSave={(value) => updateContact('tags', value.split(','))}
      />
      <EditableField
        label="备注"
  ...
  return (
    <div className="editable-field">
      ...
      {editing ? (
        ...
        >
          {value || <span className="placeholder">点击添加</span>}
        </div>
      )}
    </div>
  );
