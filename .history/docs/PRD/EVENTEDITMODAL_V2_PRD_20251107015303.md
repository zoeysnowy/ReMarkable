...
async function searchContacts(query: string): Promise<Contact[]> {
  ...
  allContacts.forEach(contact => {
    ...
    if (!uniqueMap.has(key)) {
      ...
      // 同一个人存在多个来源，根据优先级确定显示哪个来源标签
      const existing = uniqueMap.get(key)!;
      const newSource = getSourcePriority(contact.source);
      const existingSource = getSourcePriority(existing.source);
...
function getSourcePriority(source: string): number {
  if (source === 'outlook' || source === 'google' || source === 'icloud') return 1;
  if (source === 'remarkable') return 2;
  return 3; // 历史事件中的参会人（source 是 eventId）
}
...
function formatSource(source: string): string {
  switch (source) {
    ...
    default: return '历史参会人'; // source 是具体的 eventId
  }
}
```
