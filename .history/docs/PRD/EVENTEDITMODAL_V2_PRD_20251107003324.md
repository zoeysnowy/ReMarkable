...
function shouldShowPlanSection(event: Event): boolean {
  ...
  if (event.isTimer === true && event.parentEventId) {
    ...
    return parentEvent !== null; // 有父事件就显示父事件的计划安排
  }
...
**UI 提示**（Timer 子事件显示父事件数据时）:
```tsx
{planData.isFromParent && (
  <div className="parent-event-indicator">
    <LinkColorIcon />
    <span>来自父事件：{parentEvent.title}</span>
  </div>
)}
```
...
- Timer 子事件会显示父事件的计划安排，并在顶部显示"来自父事件"提示
...
**UI 提示**（Timer 子事件情况）:
```tsx
{progressData.isTimerChild && (
  <div className="timer-child-indicator">
    <TimerCheckIcon />
    <span>单次计时记录</span>
    {event.parentEventId && (
      <button 
        className="view-parent-btn"
        onClick={() => openParentEvent(event.parentEventId)}
      >
        查看完整历史 →
      </button>
    )}
...
sequenceDiagram
    ...
    User->>TimerChildModal: 点击 "查看完整历史 →" 按钮
    TimerChildModal->>ParentEvent: 关闭当前 Modal，打开父事件 Modal
    ParentEvent->>ParentEvent: 显示所有计时记录的合并日志
    ParentEvent->>ParentEvent: 自动滚动到该 Timer 子事件的起始位置
```

**关键设计点**:

1. **Timer 子事件 Modal 的特殊性**:
   - ✅ 【中 Section】显示父事件的计划安排（参会人、时间、位置）
   - ✅ 顶部提示 "来自父事件：{标题}"，点击可跳转
   - ✅ 【下 Section】只显示这一次计时的数据（非汇总）
   - ✅ 【Slate 编辑区】只显示这一次计时的日志（非合并）

2. **双向导航**:
   ```typescript
   // Timer 子事件 → 父事件
   <button onClick={() => openParentEvent(event.parentEventId)}>
     查看完整历史 →
   </button>
   
   // 父事件 → Timer 子事件
   event.timerChildEvents.map(child => (
     <div onClick={() => scrollToTimerSection(child.id)}>
...
3. **数据显示区别**:
   | 字段 | Timer 子事件 Modal | 父事件 Modal |
   |-----|-------------------|-------------|
   | 标题 | 父事件标题（只读） | 父事件标题（可编辑） |
   | 计划安排 | 显示父事件的计划 | 显示自己的计划 |
   | 实际进展 | 显示单次计时数据 | 显示所有计时汇总 |
   | Slate 日志 | 显示单次日志 | 显示合并日志时间轴 |
