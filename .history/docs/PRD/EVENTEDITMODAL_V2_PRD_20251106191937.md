...
**数据来源**:
```typescript
event.emoji?: string  // 新增字段（用户主动设置的 emoji）
```
...
function getDisplayEmoji(event: Event, globalTimer: any): string {
  ...
  // 优先级 2: event.emoji（用户通过 picker 设置的）
...
- 如果标题中有 emoji，标题 emoji 优先显示，但用户通过 picker 设置的 `event.emoji` 会被保存（可用于覆盖标题 emoji）
