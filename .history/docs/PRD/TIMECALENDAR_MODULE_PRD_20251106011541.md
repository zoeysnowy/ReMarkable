...
2. **自动判断逻辑**（PlanManager.tsx L747-819）：
   ```typescript
   // 判断规则：只有同时有 startTime 和 endTime，且不是全天事件，才是 Event
   ...
   if (hasStart && hasEnd) {
     ...
     // 只有开始时间 → Task（设置 endTime = startTime）
     isTask = true;
   } else if (!hasStart && hasEnd) {
     // 只有结束时间 → Task（设置 startTime = endTime）
     isTask = true;
   } else {
     // 完全没有时间 → Task（使用创建日期）
     isTask = true;
...
| 全天活动 | ✅ | ✅ | `false` | 全天事件条 |
| 待办事项（无时间） | ❌ | ❌ | `true` | 任务条（显示在创建日期） |
| 截止日期（11月6日前） | ❌ | ✅ | `true` | 任务条（显示在截止日期） |
| 开始日期（11月6日起） | ✅ | ❌ | `true` | 任务条（显示在开始日期） |

**关键理解**：
- ✅ **Task（待办事项）**：缺少完整时间段，在日历中显示为任务条（只占据日期，不占据具体时间轴）
- ✅ **Event（日程事件）**：有完整时间段（开始+结束），在日历中显示为时间块（占据具体时间轴）
