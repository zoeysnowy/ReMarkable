...
**数据来源**:
```typescript
event.emoji?: string  // 新增字段
```

**显示逻辑**:
```typescript
function getDisplayEmoji(event: Event): string {
  // 优先级 1: event.emoji
...
**交互**:
- 点击弹出 Emoji Picker（复用现有的 `@emoji-mart/react`）
- 选中后更新 `event.emoji` 字段
...
function getTimerButtonConfig(event: Event, globalTimer: any): TimerButtonConfig {
  const isCurrentEvent = globalTimer?.eventId === event.id;
  const isTimerRunning = globalTimer?.isRunning === true;
  
  if (isCurrentEvent && isTimerRunning) {
    // 当前事件正在计时
    return {
      icon: '⏸️',
      text: '暂停计时',
  ...
  } else if (isCurrentEvent && !isTimerRunning) {
    // 当前事件已暂停
    return {
      icon: '▶️',
      text: '继续计时',
...
function renderAttendees(attendees: Contact[]): ReactNode {
  ...
  if (attendees.length > displayLimit) {
    return `👥 ${displayNames} +${attendees.length - displayLimit}`;
  }
    ...
    <>
      ...
      {event.dueDate && (
        <label className="plan-prompt-row">
          ...
          />
          <span>
            🚩 创建于{createdAgo}，距离ddl还有{ddlRemaining}
          </span>
        </label>
      )}
    </>
...
### Phase 1: 布局重构
- [ ] 修改 `.modal-body` 为 Flex 左右布局
- [ ] 实现 `.modal-left-panel`（40%）
- [ ] 实现 `.modal-right-panel`（60%）
