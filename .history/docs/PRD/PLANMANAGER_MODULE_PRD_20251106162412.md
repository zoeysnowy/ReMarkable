...
```typescript
const now = new Date();
const nowISO = formatTimeForStorage(now); // ✅ 使用 timeUtils 避免时区问题
...
const newItem: Event = {
  ...
  createdAt: nowISO,      // ✅ 记录创建时间（用于 TimeCalendar 显示）
  updatedAt: nowISO,
  source: 'local',
...
- **实现**：添加空 event 检测逻辑
  ...
  changedItems.forEach(updatedItem => {
    // 检查是否完全为空
    const isEmpty = (
      (!updatedItem.title || updatedItem.title.trim() === '') &&
