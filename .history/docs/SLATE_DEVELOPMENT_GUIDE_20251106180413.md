...
4. **跨行删除失效**（2025-11-06 v1.3）
   ...
   - **修复**：
     - ✅ 添加删除检测逻辑：对比 `items` 和 `updatedItems` 的 ID 差异
     - ✅ 找到被删除的 IDs 后批量调用 `onDelete`
   - **位置**：`PlanManager.tsx` L1032-1038
   - **代码示例**：
     ```typescript
     // 🆕 删除检测：找出被移除的 items（跨行删除）
...
| 删除类型 | 触发条件 | 检测位置 | 处理方式 |
|---------|----------|----------|----------|
| **跨行删除** | 用户选择多行按 Backspace/Delete | `onChange` 开头（L1032-1038） | 对比 ID 差异 → 直接调用 `onDelete` |
| **空白删除** | 用户清空所有内容后失焦 | `onChange` 遍历中（L1053-1068） | 检测内容为空 → 加入删除队列 → 批量调用 `onDelete` |

**优雅性特点**：
- ✅ **职责分离**：跨行删除 vs 空白删除，逻辑清晰
- ✅ **统一入口**：所有删除都通过 `onDelete(id)` 执行
- ✅ **批量优化**：空白删除使用队列批量处理
- ✅ **日志规范**：使用 `dbg()` 替代 `console.log`，生产环境可关闭
