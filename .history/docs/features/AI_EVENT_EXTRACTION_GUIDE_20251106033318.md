...
2. 使用阿里云账号登录（支持微信/支付宝）
3. 点击 **"创建 API Key"**
4. 复制生成的 Key（格式：`sk-xxxxxxxxxxxxxx`）

**Step 2: 在 AIConfig.tsx 中配置**
```typescript
// 添加 DashScope 配置选项
export const AI_PROVIDER_OPTIONS = [
  { value: 'ollama', label: 'Ollama (本地)' },
  { value: 'dashscope', label: 'DashScope (云端)', recommended: true },
  { value: 'openai', label: 'OpenAI' },
  // ...
];
```

**Step 3: 实现 DashScope Provider**
```typescript
// src/services/ai/providers/DashScopeProvider.ts
export class DashScopeProvider implements AIProvider {
  ...
  async extractEventInfo(text: string, prompt: string): Promise<ExtractedEventInfo> {
    ...
    const data = await response.json();
    return this.parseResponse(data.output.text);
  }
}
```
...
**Q: 代理服务器频繁崩溃？**
- 检查 Node.js 版本（需要 18+）
- 查看日志文件 `ai-proxy/logs/error.log`

### 💰 费用说明

- 免费额度：无（需要付费）
- 价格：约 ¥0.002/1k tokens（与 DashScope 类似）
- 建议月预算：¥20-50
