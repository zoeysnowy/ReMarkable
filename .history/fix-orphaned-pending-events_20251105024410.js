    ...
    if (!event.calendarId) {
      ...
      if (event.tags && event.tags.length > 0) {
        ...
        if (tag?.calendarMapping?.calendarId) {
          event.calendarId = tag.calendarMapping.calendarId;
          console.log(`  ✓ [${event.title}] 从标签 "${tag.name}" 获取日历`);
          fixedCalendarCount++;
        }
      }
      // 尝试从 tagId 获取
        ...
        if (tag?.calendarMapping?.calendarId) {
          event.calendarId = tag.calendarMapping.calendarId;
          console.log(`  ✓ [${event.title}] 从 tagId "${tag.name}" 获取日历`);
          fixedCalendarCount++;
        }
      }
      
      // 如果还是没有，使用默认日历
      if (!event.calendarId && defaultCalendarId) {
        event.calendarId = defaultCalendarId;
        console.log(`  ✓ [${event.title}] 使用默认日历`);
        fixedCalendarCount++;
      }
    }
  }
