...
ipcMain.handle('widget-move', (event, position) => {
  if (widgetWindow && !widgetWindow.isDestroyed()) {
    try {
      ...
      console.log(`🎯 [Main] Moving widget: (${bounds.x}, ${bounds.y}) -> (${newX}, ${newY}), delta: (${position.x}, ${position.y})`);
      
      // 使用 setBounds 而不是 setPosition，对无边框窗口更可靠
      ...
      console.log(`✅ [Main] Widget moved to: (${newBounds.x}, ${newBounds.y})`);
