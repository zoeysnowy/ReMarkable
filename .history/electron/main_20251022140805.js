...
ipcMain.handle('widget-lock', (event, isLocked) => {
  if (widgetWindow && !widgetWindow.isDestroyed()) {
    ...
    if (isLocked) {
      console.log('✅ Widget locked: Always on top (screen-saver level), all interactions enabled');
    } else {
      console.log('✅ Widget unlocked: Normal window level, can be covered by other apps');
    }
  }
...
ipcMain.handle('widget-move', (event, position) => {
  if (widgetWindow && !widgetWindow.isDestroyed()) {
    try {
      ...
      console.log(`🎯 [Main] Moving widget: (${currentX}, ${currentY}) -> (${newX}, ${newY}), delta: (${position.x}, ${position.y})`);
      
      // 相对移动
      widgetWindow.setPosition(newX, newY);
      
      // 验证移动是否成功
      const [actualX, actualY] = widgetWindow.getPosition();
      console.log(`✅ [Main] Widget moved to: (${actualX}, ${actualY})`);
      
      return { success: true, position: { x: actualX, y: actualY } };
    } catch (error) {
