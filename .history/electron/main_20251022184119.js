...
ipcMain.handle('widget-move', (event, position) => {
  if (widgetWindow && !widgetWindow.isDestroyed()) {
    try {
      ...
      console.log(`🎯 [Main] Moving widget: (${currentBounds.x}, ${currentBounds.y}) -> (${newBounds.x}, ${newBounds.y}), delta: (${position.x}, ${position.y})`);
      console.log(`📐 [Main] 尺寸锁定为: ${currentBounds.width}x${currentBounds.height}`);
      
      // 使用 setBounds 一次性设置位置和尺寸，并禁用动画
      widgetWindow.setBounds(newBounds, false);
      
      // 验证结果
      const resultBounds = widgetWindow.getBounds();
      console.log(`✅ [Main] Widget moved to: (${resultBounds.x}, ${resultBounds.y}), size: ${resultBounds.width}x${resultBounds.height}`);
      
      // 如果尺寸仍然变化了（理论上不应该），再次强制修正
      if (resultBounds.width !== currentBounds.width || resultBounds.height !== currentBounds.height) {
        console.warn(`⚠️ [Main] 尺寸仍然变化了！再次修正`);
      ...
      return { success: true, position: { x: resultBounds.x, y: resultBounds.y } };
    } catch (error) {
