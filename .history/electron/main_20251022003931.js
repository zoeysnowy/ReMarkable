...
ipcMain.handle('widget-lock', (event, isLocked) => {
  if (widgetWindow && !widgetWindow.isDestroyed()) {
    // 🔒 锁定只影响窗口移动和大小，不影响内部交互
    // 不使用 setIgnoreMouseEvents - 那会让窗口完全穿透
    widgetWindow.setMovable(!isLocked);
    widgetWindow.setResizable(!isLocked);
    
    if (isLocked) {
      console.log('✅ Widget locked: Window fixed, interactions enabled');
    } else {
      console.log('✅ Widget unlocked: Window movable');
    }
  }
