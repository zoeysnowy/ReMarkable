...
ipcMain.handle('widget-move', (event, position) => {
  if (widgetWindow && !widgetWindow.isDestroyed()) {
    try {
      ...
      const boundsBefore = widgetWindow.getBounds();
      
      // 获取当前位置（不获取尺寸，避免尺寸被错误读取/设置）
      const currentPos = widgetWindow.getPosition();
      const newX = currentPos[0] + position.x;
      const newY = currentPos[1] + position.y;
      
      console.log(`🎯 [Main] Moving widget: (${currentPos[0]}, ${currentPos[1]}) -> (${newX}, ${newY}), delta: (${position.x}, ${position.y})`);
      console.log(`📐 [Main] 移动前尺寸: size=${sizeBefore[0]}x${sizeBefore[1]}, bounds=${boundsBefore.width}x${boundsBefore.height}`);
      
      // 只设置位置，不触碰尺寸 - 使用 setPosition 而不是 setBounds
      // 这样可以避免在移动时意外改变窗口大小
      widgetWindow.setPosition(newX, newY, false); // 第三个参数 false 表示不进行动画
      
      // 🔍 拖动后记录窗口尺寸
      const sizeAfter = widgetWindow.getSize();
      const boundsAfter = widgetWindow.getBounds();
      console.log(`📐 [Main] 移动后尺寸: size=${sizeAfter[0]}x${sizeAfter[1]}, bounds=${boundsAfter.width}x${boundsAfter.height}`);
      
      // 🔧 检查尺寸是否变化，如果变化则强制恢复原始尺寸
      if (sizeBefore[0] !== sizeAfter[0] || sizeBefore[1] !== sizeAfter[1]) {
        console.warn(`⚠️ [Main] 窗口尺寸在 setPosition 后发生了变化！强制恢复原始尺寸`, {
          widthChange: sizeAfter[0] - sizeBefore[0],
          heightChange: sizeAfter[1] - sizeBefore[1]
        });
        
        // 强制恢复原始尺寸
        widgetWindow.setSize(sizeBefore[0], sizeBefore[1], false);
        
        const sizeRestored = widgetWindow.getSize();
        console.log(`✅ [Main] 尺寸已恢复: ${sizeRestored[0]}x${sizeRestored[1]}`);
      }
      
      // 验证移动是否成功
      const newPos = widgetWindow.getPosition();
      console.log(`✅ [Main] Widget moved to: (${newPos[0]}, ${newPos[1]})`);
      
      return { success: true, position: { x: newPos[0], y: newPos[1] } };
    } catch (error) {
      console.error('❌ [Main] Failed to move widget:', error);
      return { success: false, error: error.message };
    }
  }
  console.warn('⚠️ [Main] Widget window not available');
...
function createWidgetWindow() {
  try {
    ...
    widgetWindow.on('resize', () => {
      ...
      console.log('🔄 [Main] Window resize event fired:', {
        size: `${size[0]}x${size[1]}`,
        bounds: `${bounds.width}x${bounds.height}`,
        stack: new Error().stack // 打印调用栈
      });
    });
    ...
    widgetWindow.on('move', () => {
      ...
      console.log('🚚 [Main] Window move event fired:', `(${pos[0]}, ${pos[1]})`);
    });
